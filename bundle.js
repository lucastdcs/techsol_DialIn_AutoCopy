...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let g=r.querySelector(".cw-hero-grid"),l=r.querySelector(".cw-acc-container"),u=r.querySelector(".cw-results-container"),b=r.querySelector(".cw-search-input"),d=r.querySelector(".cw-status-bar"),C=r.querySelector(".cw-status-text"),S=r.querySelector(".cw-footer-icons");s.forEach(([y,m])=>{let h=o(m.name),v=document.createElement("div");v.className="cw-hero-card",v.id=`hero-${y}`,v.style.setProperty("--hero-color",h.color),v.innerHTML=`
        ...
        `,v.onclick=T=>{if(T.target.closest(".cw-step-btn"))return;let A=t[y]?t[y].count:0;P(y,A>0?-A:1,m)},v.querySelector(".minus").onclick=()=>P(y,-1,m),v.querySelector(".plus").onclick=()=>P(y,1,m),v.dataset.color=h.color,g.appendChild(v)});function L(y,m){let h=o(m.name),v=document.createElement("div");return v.className="cw-task-item",v.dataset.id=y,v.innerHTML=`
...
`).filter(W=>W.trim()!=="").map(W=>`<p style="margin: 0 0 8px 0;">${W}</p>`).join(""):J.tagName==="TEXTAREA"&&(Z=Z.replace(/\n/g,"<br>"));let ue=Z.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(ue===""||ue==="\u2022"||ue.toLowerCase()==="n/a"){let W=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${V}\\}(?:<br>\\s*)?`,"gi");W.test(q)?q=q.replace(W,""):q=q.replace(z,"")}else q=q.replace(z,Z.replace(/\$/g,"$$$$"))}),q=q.replace(/{([A-Z0-9_]+)}/g,""),q=q.replace(/(<br>){3,}/g,"<br><br>"),typeof i<"u"&&i.getOutput&&(q+=i.getOutput()),q}Ze.onclick=()=>{let k=Ft();k?(Nt(k),ee(X("copiado_sucesso"))):ee(X("selecione_substatus"),{error:!0})},Je.onclick=async()=>{let k=_.value,H=Ft();if(!H){ee(X("selecione_substatus"),{error:!0});return}Nt(H),vt();let q=pt(),K=await uo();if(K)try{if(K.focus(),K.innerHTML.trim()==="<p><br></p>"||K.innerHTML.trim()==="<br>"||K.innerText.trim()===""){let ge=document.createRange();ge.selectNodeContents(K);let ne=window.getSelection();ne.removeAllRanges(),ne.addRange(ge),document.execCommand("delete",!1,null)}else if(!K.innerHTML.endsWith("<br><br>")){let ge=document.createRange();ge.selectNodeContents(K),ge.collapse(!1);let ne=window.getSelection();ne.removeAllRanges(),ne.addRange(ge),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,H),po(K),setTimeout(()=>{ee(X("inserido_copiado"))},600);let me=typeof je<"u"&&je?je.checked:!0;if(k&&$e[k]&&me){let ge=$e[k];await ct(ge),await new Promise(ne=>setTimeout(ne,500))}q(),yt(1.5),T.value="",_.innerHTML=`<option value="">${X("select_substatus")}</option>`,_.disabled=!0}catch(de){console.error(de),ee("Erro ao inserir.",{error:!0}),q()}};function yt(k=1.5){k<=1.5&&(Ee.style.display="none",Ce.innerHTML=""),k<=2&&(r.reset(),xe.style.display="none"),k<=3&&(he.style.display="none",Fe.innerHTML="",i.reset(),Ye.style.display="none",Be.style.display="none")}function vt(){if(s=!s,s){let k=c.querySelector(".cw-expand-btn");k&&typeof k.resetState=="function"&&k.resetState()}Ie(s,c,"cw-btn-notes")}return bt("bau"),ft("pt"),vt}var We={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
        ...
        `;let I=_.querySelector("#csa-back-btn");I.onmouseenter=()=>I.style.background="#f1f3f4",I.onmouseleave=()=>I.style.background="none",I.onclick=x;let R=_.querySelector("#csa-insert-btn");R.onmouseenter=()=>{R.style.transform="translateY(-1px)",R.style.boxShadow="0 6px 16px rgba(26,115,232,0.4)"},R.onmouseleave=()=>{R.style.transform="translateY(0)",R.style.boxShadow="0 4px 12px rgba(26,115,232,0.3)"},R.onclick=()=>{R.style.transform="scale(0.96)",te(N,"email"),setTimeout(()=>{R.style.transform="scale(1)",x()},300)}}function x(){s="list",m.style.transform="translateX(0)"}function w(N,j,M=null){let I=document.createElement("button"),R=M?`<span style="margin-right:6px; font-size:14px; opacity:0.9;">${M}</span>`:"";return I.innerHTML=`${R}${N}`,Object.assign(I.style,b),n===j&&o===""?Object.assign(I.style,d):(I.onmouseenter=()=>{I.style.background="#F1F3F4",I.style.borderColor="#DADCE0"},I.onmouseleave=()=>{I.style.background="#FFFFFF",I.style.borderColor="#DADCE0"}),I.onclick=()=>{n=j,o="",T.value="",E(),G()},I}function E(){A.innerHTML="",A.appendChild(w("Smart CRs",t,"\u26A1")),Object.keys(We).forEach(N=>{A.appendChild(w(We[N].title,N))})}function G(){D.innerHTML="";let N=[];if(o.trim()!==""){let $=o.toLowerCase();Object.values(We).forEach(O=>{O.emails.forEach(B=>{(B.name.toLowerCase().includes($)||B.subject.toLowerCase().includes($))&&N.push({type:"email",data:B})})}),Object.entries($e).forEach(([O,B])=>{if(!B)return;(O.replace(/_/g," ").toLowerCase().includes($)||B.toLowerCase().includes($))&&N.push({type:"cr",key:O,code:B})})}else n===t?Object.entries($e).forEach(([$,O])=>{O&&N.push({type:"cr",key:$,code:O})}):We[n]&&We[n].emails.forEach($=>{N.push({type:"email",data:$})});if(N.length===0){D.innerHTML=`
                <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:60px 20px; color:#9AA0A6;">
                ...
                </div>`;return}let M='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1967D2" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>',I='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#EA8600" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon></svg>',R='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#BDC1C6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>';N.forEach($=>{let O=document.createElement("div");if(Object.assign(O.style,S),$.type==="email"){let B=$.data,se=B.subject.length>45?B.subject.substring(0,45)+"...":B.subject;O.innerHTML=`
                    ...
                    <div style="margin-left:12px; opacity:0.6;">${R}</div>
...
`),O=/\d{1,2}\/\d{1,2}/;if($.forEach(ce=>{let fe=ce.match(O);if(fe){let Ee=fe[0],Ne="\u{1F4C5}";/🇧🇷|🇵🇹|PT|BR/i.test(ce)?Ne="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM/i.test(ce)&&(Ne="\u{1F1EA}\u{1F1F8}"),R.some(he=>he.flag===Ne&&he.date===Ee)||R.push({flag:Ne,date:Ee})}}),R.length===0){let ce=(M.text||"").match(/\d{1,2}\/\d{1,2}/g);ce&&[...new Set(ce)].forEach(fe=>R.push({flag:"\u{1F4C5}",date:fe}))}let B="",se='<button id="cw-bau-toggle-btn" class="cw-btn-interactive" style="background:rgba(255,255,255,0.7); border:1px solid rgba(139, 92, 246, 0.4); border-radius:12px; padding:8px 12px; color:#6D28D9; font-size:12px; font-weight:600;">Detalhes</button>';window._cwIsAdmin&&(se=`
              ...
              `),R.length>0?B=`
                  <div style="display:flex; justify-content:space-between; align-items:center;">
                      <div style="flex:1; display:flex; gap:8px;">${R.map(fe=>`
