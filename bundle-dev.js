...
`),O=/\d{1,2}\/\d{1,2}/;if($.forEach(le=>{let fe=le.match(O);if(fe){let Se=fe[0],Ne="\u{1F4C5}";/🇧🇷|🇵🇹|PT|BR/i.test(le)?Ne="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM/i.test(le)&&(Ne="\u{1F1EA}\u{1F1F8}"),G.some(Fe=>Fe.flag===Ne&&Fe.date===Se)||G.push({flag:Ne,date:Se})}}),G.length===0){let le=(L.text||"").match(/\d{1,2}\/\d{1,2}/g);le&&[...new Set(le)].forEach(fe=>G.push({flag:"\u{1F4C5}",date:fe}))}let P="",de='<button id="cw-bau-toggle-btn" class="cw-btn-interactive" style="background:rgba(255,255,255,0.7); border:1px solid rgba(139, 92, 246, 0.4); border-radius:12px; padding:8px 12px; color:#6D28D9; font-size:12px; font-weight:600;">Detalhes</button>';window._cwIsAdmin&&(de=`
              ...
              ${P}
          `,k.after(M);let oe=M.querySelector("#cw-bau-toggle-btn"),ne=M.querySelector("#cw-bau-full");if(oe&&ne&&(oe.onclick=()=>{let le=ne.style.display==="none";ne.style.display=le?"block":"none",oe.textContent=le?"Ocultar":"Detalhes"}),window._cwIsAdmin){let le=M.querySelector(".cw-bau-edit");le&&(le.onclick=()=>B(L))}}let E=f.sort((L,M)=>{let G=y.includes(L.id),$=y.includes(M.id);return G===$?0:G?1:-1});if(E.length===0&&!m){let L=document.createElement("div");Object.assign(L.style,i.emptyState),L.innerHTML=`
