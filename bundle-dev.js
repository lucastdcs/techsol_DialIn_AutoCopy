...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let m=l.querySelector(".cw-hero-grid"),c=l.querySelector(".cw-acc-container"),w=l.querySelector(".cw-results-container"),b=l.querySelector(".cw-search-input"),f=l.querySelector(".cw-status-bar"),y=l.querySelector(".cw-status-text"),T=l.querySelector(".cw-footer-icons");i.forEach(([h,x])=>{let r=o(x.name),d=document.createElement("div");d.className="cw-hero-card",d.id=`hero-${h}`,d.style.setProperty("--hero-color",r.color),d.innerHTML=`
        ...
        `,d.onclick=()=>{c.querySelectorAll(".cw-acc-group.open").forEach(p=>{p!==r&&p.classList.remove("open")}),r.classList.toggle("open")};let S=document.createElement("div");S.className="cw-acc-body",x.tasks.forEach(p=>{let A=q(p.key,p);S.appendChild(A)}),r.appendChild(d),r.appendChild(S),c.appendChild(r)});function R(h,x,r){t[h]||(t[h]={count:0,data:r,brand:o(r.name)}),t[h].count+=x,t[h].count<=0&&delete t[h],B(),$(),e&&e()}function B(){i.forEach(([S])=>{let p=m.querySelector(`#hero-${S}`);if(!p)return;let A=t[S];A?(p.classList.add("active"),p.querySelector(".cw-step-val").textContent=A.count,p.querySelector(".cw-step-val").style.color=p.dataset.color):p.classList.remove("active")}),l.querySelectorAll(".cw-task-item").forEach(S=>{let p=S.dataset.id,A=t[p];A?(S.classList.add("selected"),S.querySelector(".cw-step-val").textContent=A.count):S.classList.remove("selected")});let x=Object.keys(t),r=0,d=[];if(x.forEach(S=>{let p=t[S];r+=p.count;for(let A=0;A<p.count;A++)d.length<6&&d.push(p.brand)}),r>0){f.classList.add("visible");let S=r>1?"A\xE7\xF5es":"A\xE7\xE3o",p=r>1?"definidas":"definida";y.textContent=`${r} ${S} ${p}`,T.innerHTML="",d.forEach(A=>{let O=document.createElement("div");O.className="cw-mini-icon",O.innerHTML=Pe[A.icon]||Pe.default;let v=O.querySelector("svg");v&&(v.style.width="14px",v.style.height="14px"),T.appendChild(O)})}else f.classList.remove("visible")}b.addEventListener("input",h=>{let x=h.target.value.toLowerCase();if(x.length>0){c.style.display="none",w.style.display="block",w.innerHTML="";let r=!1;Object.entries(Re).forEach(([d,S])=>{if(S.name.toLowerCase().includes(x)){r=!0;let p=q(d,S);t[d]&&(p.classList.add("selected"),p.querySelector(".cw-step-val").textContent=t[d].count),w.appendChild(p)}}),r||(w.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else c.style.display="block",w.style.display="none"});function $(){g.innerHTML="";let h=Object.keys(t),x=!1,r=document.getElementById("sub-status"),d="implementation";if(r&&r.value.toLowerCase().includes("education")&&(d="education"),h.length===0){g.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}if(h.length===0){g.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let S=document.createElement("div");S.className="cw-info-banner",S.innerHTML=`
...
`),_.trim()!=="\u2022"&&_.trim()!==""?U.value=_:ot.includes(K.replace("field-",""))?U.value="\u2022 ":U.value="",U.tagName==="TEXTAREA"&&typeof gt=="function"&&gt(U)}});let le=new Set,he=new Set;C.querySelectorAll('input[type="checkbox"], input[type="radio"]').forEach(K=>{let U=we[K.id];U&&U.linkedTask&&(K.checked?le.add(U.linkedTask):he.add(U.linkedTask))}),he.forEach(K=>{le.has(K)||l.toggleTask(K,!1)}),le.forEach(K=>{l.toggleTask(K,!0)})}S.onchange=()=>{let C=S.value;if(ft(1.5),v.innerHTML=`<option value="">${j("select_substatus")}</option>`,!C){v.disabled=!0;return}for(let M in tt){let E=tt[M];if(E.status===C){let V=document.createElement("option");V.value=M,V.textContent=E.name,v.appendChild(V)}}v.disabled=!1},v.onchange=()=>{let C=v.value;if(ft(1.5),!C)return;l.updateSubStatus(C);let M=tt[C];ye.innerHTML="";let E=(D,_,H)=>{let re=document.createElement("label");Object.assign(re.style,s.checkboxLabel),re.onmouseover=()=>re.style.backgroundColor="#e8eaed",re.onmouseout=()=>re.style.backgroundColor="#f8f9fa";let ee=document.createElement("input");return ee.type=_,ee.id=D.id,Object.assign(ee.style,s.checkboxInput),re.appendChild(ee),re.appendChild(document.createTextNode(` ${D.text}`)),H.appendChild(re),ee},V=[],ne="radio";if(C==="NI_Awaiting_Inputs")V=[{id:"quickfill-ni-inicio-manual",text:"In\xEDcio 2/6 (Manual)"},{id:"quickfill-ni-cms-access",text:"In\xEDcio 2/6 (ADV sem acesso ao CMS)"},{id:"quickfill-ni-followup-bau",text:"Follow-up 2/6 (BAU)"},{id:"quickfill-ni-followup-lm",text:"Follow-up 2/6 (LM)"}];else if(C.startsWith("SO_"))ne="checkbox",V=[{id:"quickfill-gtm-install",text:"Instala\xE7\xE3o do GTM"},{id:"quickfill-whatsapp",text:"Convers\xE3o de WhatsApp"},{id:"quickfill-form",text:"Convers\xE3o de Formul\xE1rio (Padr\xE3o)"},{id:"quickfill-ecw4-close",text:"Fechamento ECW4 (P\xF3s 7 dias)"}];else if(C.startsWith("AS_")){ne="checkbox";let D=document.createElement("label");D.textContent=j("cenarios_comuns"),Object.assign(D.style,s.label),ye.appendChild(D),V=[{id:"quickfill-as-no-show",text:"Anunciante n\xE3o compareceu (respondeu e-mail)"},{id:"quickfill-as-insufficient-time",text:"Tempo insuficiente"},{id:"quickfill-as-no-access",text:"Anunciante sem acessos necess\xE1rios"}]}else C.startsWith("IN_")&&(V=[{id:"quickfill-in-nrp-bau",text:"NRP (BAU - 3 tentativas)"},{id:"quickfill-in-nrp-lm",text:"NRP (LM - Sem tentativas)"},{id:"quickfill-in-no-show-bau",text:"No-Show (BAU - 3 tentativas)"},{id:"quickfill-in-2-6-final",text:"Finaliza\xE7\xE3o 2/6 (Sem Resposta)"},{id:"quickfill-in-manual",text:"Outro (Manual)"}]);let le=V.filter(D=>{let _=we[D.id];return!_.type||_.type==="all"||_.type===t});le.forEach((D,_)=>{let H=E(D,ne,ye);ne==="radio"&&(H.name="scenario-radio-group",_===0&&(H.checked=!0))}),le.length>0&&(Me.style.display="block"),M.requiresTasks?(fe.style.display="none",$e.style.display="block",l.selectionElement.style.display="block",Ae.style.display="block"):(fe.style.display="block",$e.style.display="none",l.selectionElement.style.display="none",Ae.style.display="block"),_e.innerHTML="";let he=M.template.match(/{([A-Z0-9_]+)}/g)||[];[...new Set(he)].forEach(D=>{if(["{TAGS_IMPLEMENTED}","{SCREENSHOTS_LIST}","{CONSENTIU_GRAVACAO}","{CASO_PORTUGAL}"].includes(D))return;let _=D.slice(1,-1),H=document.createElement("label"),re=j(_.toLowerCase());if(H.textContent=re!==_.toLowerCase()?re:_.replace(/_/g," ").replace(/\b\w/g,Y=>Y.toUpperCase())+":",Object.assign(H.style,s.label),_==="SPEAKEASY_ID"){let Y=document.createElement("span");Y.innerHTML='<span style="font-size:12px; margin-right:3px;">\u{1F50D}</span>AUTO-BUSCA',Y.style.cssText=`
              ...
              transition: all 0.2s ease;
              letter-spacing: 0.5px;
          `,Y.title="Buscar ID automaticamente no hist\xF3rico do caso",Y.onmouseover=()=>{Y.style.backgroundColor="#d2e3fc",Y.style.boxShadow="0 1px 2px rgba(0,0,0,0.1)"},Y.onmouseout=()=>{Y.style.backgroundColor="#e8f0fe",Y.style.boxShadow="none"},Y.onclick=ue=>{ue.preventDefault(),Ut(`field-${_}`)},H.appendChild(Y)}let ee;ot.includes(_)?(ee=document.createElement("textarea"),Object.assign(ee.style,s.textarea),ee.classList.add("bullet-textarea"),gt(ee)):Ct.includes(_)?(ee=document.createElement("textarea"),Object.assign(ee.style,s.textarea)):(ee=document.createElement("input"),ee.type="text",Object.assign(ee.style,s.input),_==="REASON_COMMENTS"&&(C.startsWith("NI_")||C.startsWith("IN_"))&&(Object.assign(H.style,{display:"none"}),Object.assign(ee.style,{display:"none"}))),_==="ON_CALL"&&t==="lm"&&(Object.assign(H.style,{display:"none"}),Object.assign(ee.style,{display:"none"}),ee.value="N/A"),ee.id=`field-${_}`,_e.appendChild(H),_e.appendChild(ee)});let K=ye.querySelectorAll('input[type="checkbox"], input[type="radio"]');K.length>0&&(K.forEach(D=>{D.removeEventListener("change",bt),D.addEventListener("change",bt)}),bt()),Oe.style.display="block",rt[C]?De.style.display="block":De.style.display="none",Ue.style.display="flex";let U=l.getCheckedElements().map(D=>D.value);a.updateVisibility(C,U)},fe.onclick=()=>{fe.style.display="none",$e.style.display="block",l.selectionElement.style.display="block"};function _t(){let C=v.value;if(!C)return null;let E=tt[C].template.replace(/\n/g,"<br>"),V='style="margin-bottom: 12px; padding-left: 30px;"',ne=[],le="",he=l.getCheckedElements();he.length>0&&he.forEach(U=>{let D=U.value,_=Re[D],H=U.closest().querySelector(".stepper-count"),re=H?parseInt(H.textContent):1;re>1?ne.push(`${_.name} (x${re})`):ne.push(_.name)});let de=l.screenshotsElement;if(de){let U=Array.from(de.querySelectorAll('input[id^="name-"]'));U.length>0&&U.forEach(D=>{let _=D.value,H=D.closest(".cw-screen-card");if(H){let re=H.querySelectorAll('input[id^="screen-"]'),ee=!1,Y="";re.forEach(ue=>{let Rt=ue.closest(".cw-input-group"),Lt=Rt?Rt.querySelector(".cw-input-label"):null,fo=Lt?Lt.textContent:"Evid\xEAncia",Mt=ue.value.trim(),ho=Mt?` ${Mt}`:"";Y+=`<li>${fo} -${ho}</li>`,ee=!0}),ee&&(le+=`<b>${_}</b>`,le+=`<ul ${V}>${Y}</ul>`)}})}if(E.includes("{TAGS_IMPLEMENTED}")?E=E.replace(/{TAGS_IMPLEMENTED}/g,ne.join(", ")||"N/A"):ne.length>0&&(E+=`<br><b>Tags:</b> ${ne.join(", ")}<br>`),E.includes("{SCREENSHOTS_LIST}")?E=E.replace(/{SCREENSHOTS_LIST}/g,le?`${le}`:"N/A"):le!==""&&(E+=`<br>${le}`),n==="pt"&&o){let U=xe.checked?j("sim"):j("nao");E=E.replace(/{CONSENTIU_GRAVACAO}/g,`<br><b>${j("consentiu_gravacao")}</b> ${U}<br><br>`),E=E.replace(/{CASO_PORTUGAL}/g,`<br><b>${j("caso_portugal")}</b> ${j("sim")}<br>`)}else n==="pt"&&!o?(E=E.replace(/{CASO_PORTUGAL}/g,`<br><b>${j("caso_portugal")}</b> ${j("nao")}<br>`),E=E.replace(/{CONSENTIU_GRAVACAO}/g,"")):(E=E.replace(/{CASO_PORTUGAL}/g,""),E=E.replace(/{CONSENTIU_GRAVACAO}/g,""));return _e.querySelectorAll("input, textarea").forEach(U=>{let D=U.id.replace("field-",""),_=new RegExp(`{${D}}`,"g"),H=U.value;if(D==="REASON_COMMENTS"&&(C.startsWith("NI_")||C.startsWith("IN_"))){let Y=ye.querySelector('input[type="radio"]:checked');Y&&we[Y.id]&&(H=we[Y.id]["field-REASON_COMMENTS"])}if(ot.includes(D)&&H.trim()!==""){let Y=H.split(`
`).map(ue=>ue.trim()).filter(ue=>ue!==""&&ue!=="\u2022").map(ue=>ue.startsWith("\u2022 ")?ue.substring(2):ue).map(ue=>`<li>${ue}</li>`).join("");H=Y?`<ul ${V}>${Y}</ul>`:""}else Ct.includes(D)?H=H.split(`
`).filter(Y=>Y.trim()!=="").map(Y=>`<p style="margin: 0 0 8px 0;">${Y}</p>`).join(""):U.tagName==="TEXTAREA"&&(H=H.replace(/\n/g,"<br>"));let re=H.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(re===""||re==="\u2022"||re.toLowerCase()==="n/a"){let Y=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${D}\\}(?:<br>\\s*)?`,"gi");Y.test(E)?E=E.replace(Y,""):E=E.replace(_,"")}else E=E.replace(_,H.replace(/\$/g,"$$$$"))}),E=E.replace(/{([A-Z0-9_]+)}/g,""),E=E.replace(/(<br>){3,}/g,"<br><br>"),typeof a<"u"&&a.getOutput&&(E+=a.getOutput()),E}Ke.onclick=()=>{let C=_t();C?(Ot(C),J(j("copiado_sucesso"))):J(j("selecione_substatus"),{error:!0})},Qe.onclick=async()=>{let C=v.value,M=_t();if(!M){J(j("selecione_substatus"),{error:!0});return}Ot(M),ht();let E=Xe(),V=await io();if(V)try{if(V.focus(),V.innerHTML.trim()==="<p><br></p>"||V.innerHTML.trim()==="<br>"||V.innerText.trim()===""){let de=document.createRange();de.selectNodeContents(V);let K=window.getSelection();K.removeAllRanges(),K.addRange(de),document.execCommand("delete",!1,null)}else if(!V.innerHTML.endsWith("<br><br>")){let de=document.createRange();de.selectNodeContents(V),de.collapse(!1);let K=window.getSelection();K.removeAllRanges(),K.addRange(de),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,M),ao(V),setTimeout(()=>{J(j("inserido_copiado"))},600);let le=typeof Ge<"u"&&Ge?Ge.checked:!0;if(C&&rt[C]&&le){let de=rt[C];await Yt(de),await new Promise(K=>setTimeout(K,500))}E(),ft(1.5),S.value="",v.innerHTML=`<option value="">${j("select_substatus")}</option>`,v.disabled=!0}catch(ne){console.error(ne),J("Erro ao inserir.",{error:!0}),E()}};function ft(C=1.5){C<=1.5&&(Me.style.display="none",ye.innerHTML=""),C<=2&&(Ae.style.display="none",l.reset(),fe.style.display="none"),C<=3&&(Oe.style.display="none",_e.innerHTML="",a.reset(),Ue.style.display="none",De.style.display="none")}function ht(){if(i=!i,i){let C=u.querySelector(".cw-expand-btn");C&&typeof C.resetState=="function"&&C.resetState()}Ee(i,u,"cw-btn-notes")}return ut("bau"),mt("pt"),ht}var Ve={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
                    <p>Ol\xE1,</p>
                    <br>
