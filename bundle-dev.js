...
.cw-pill.collapsed {
    ...
    height: 56px !important; /* For\xE7a altura igual largura */
...
/* ADICIONE OU ATUALIZE ESTA REGRA ESPEC\xCDFICA */
.cw-pill.collapsed .cw-brand-btn {
    position: absolute; /* Remove do fluxo para n\xE3o somar margens */
    top: 50%; 
    left: 50%;
    /* Centraliza e aplica a rota\xE7\xE3o */
...
`).filter(B=>B.trim()!=="").map(B=>`<p style="margin: 0 0 8px 0;">${B}</p>`).join(""):K.tagName==="TEXTAREA"&&(Y=Y.replace(/\n/g,"<br>"));let se=Y.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(se===""||se==="\u2022"||se.toLowerCase()==="n/a"){let B=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${D}\\}(?:<br>\\s*)?`,"gi");B.test(E)?E=E.replace(B,""):E=E.replace(I,"")}else E=E.replace(I,Y.replace(/\$/g,"$$$$"))}),E=E.replace(/{([A-Z0-9_]+)}/g,""),E=E.replace(/(<br>){3,}/g,"<br><br>"),typeof i<"u"&&i.getOutput&&(E+=i.getOutput()),E}Ke.onclick=()=>{let C=Lt();C?(It(C),oe(H("copiado_sucesso"))):oe(H("selecione_substatus"),{error:!0})},Qe.onclick=async()=>{let C=S.value,M=Lt();if(!M){oe(H("selecione_substatus"),{error:!0});return}It(M),yt();let E=dt(),$=await po();if($)try{if($.focus(),$.innerHTML.trim()==="<p><br></p>"||$.innerHTML.trim()==="<br>"||$.innerText.trim()===""){let de=document.createRange();de.selectNodeContents($);let Q=window.getSelection();Q.removeAllRanges(),Q.addRange(de),document.execCommand("delete",!1,null)}else if(!$.innerHTML.endsWith("<br><br>")){let de=document.createRange();de.selectNodeContents($),de.collapse(!1);let Q=window.getSelection();Q.removeAllRanges(),Q.addRange(de),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,M),co($),setTimeout(()=>{oe(H("inserido_copiado"))},600);let ce=typeof ze<"u"&&ze?ze.checked:!0;if(C&&Pe[C]&&ce){let de=Pe[C];await lt(de),await new Promise(Q=>setTimeout(Q,500))}E(),xt(1.5),w.value="",S.innerHTML=`<option value="">${H("select_substatus")}</option>`,S.disabled=!0}catch(ae){console.error(ae),oe("Erro ao inserir.",{error:!0}),E()}};function xt(C=1.5){C<=1.5&&(Me.style.display="none",Ae.innerHTML=""),C<=2&&(l.reset(),ue.style.display="none"),C<=3&&(xe.style.display="none",_e.innerHTML="",i.reset(),We.style.display="none",Fe.style.display="none")}function yt(){if(r=!r,r){let C=g.querySelector(".cw-expand-btn");C&&typeof C.resetState=="function"&&C.resetState()}Te(r,g,"cw-btn-notes")}return gt("bau"),bt("pt"),yt}var $e={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
        ...
        `,y.querySelector("#csa-back-btn").onclick=T;let V=y.querySelector("#csa-insert-btn");V.onclick=()=>{V.style.transform="scale(0.96)",k(R,"email"),setTimeout(()=>{V.style.transform="scale(1)",T()},300)}}function T(){r="list",j.style.transform="translateX(0)"}function ne(R,P,X=null){let V=document.createElement("button");return V.innerHTML=X?`<span style="margin-right:4px">${X}</span>${R}`:R,Object.assign(V.style,p),n===P&&o===""&&Object.assign(V.style,b),V.onclick=()=>{n=P,o="",s.value="",z(),L()},V}function z(){c.innerHTML="",c.appendChild(ne("Smart CRs",t,"\u26A1")),Object.keys($e).forEach(R=>{c.appendChild(ne($e[R].title,R))})}function L(){w.innerHTML="";let R=[];if(o.trim()!==""){let W=o.toLowerCase();Object.values($e).forEach(_=>{_.emails.forEach(J=>{(J.name.toLowerCase().includes(W)||J.subject.toLowerCase().includes(W))&&R.push({type:"email",data:J})})}),Object.entries(Pe).forEach(([_,J])=>{if(!J)return;(_.replace(/_/g," ").toLowerCase().includes(W)||J.toLowerCase().includes(W))&&R.push({type:"cr",key:_,code:J})})}else n===t?Object.entries(Pe).forEach(([W,_])=>{_&&R.push({type:"cr",key:W,code:_})}):$e[n]&&$e[n].emails.forEach(W=>{R.push({type:"email",data:W})});if(R.length===0){w.innerHTML='<div style="text-align:center; padding:60px 20px; color:#9aa0a6;"><div style="font-size:24px;">\u{1F50D}</div><div style="font-size:14px; margin-top:8px;">Nada encontrado.</div></div>';return}let X='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#1a73e8" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"></path><polyline points="22,6 12,13 2,6"></polyline></svg>',V='<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#fbbc04" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"></polygon></svg>',he='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#dadce0" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>';R.forEach(W=>{let _=document.createElement("div");if(Object.assign(_.style,N),W.type==="email"){let J=W.data,re=J.subject.length>40?J.subject.substring(0,40)+"...":J.subject;_.innerHTML=`
                ...
                `,_.onclick=()=>S(J)}else{let J=W.key.replace(/_/g," ").replace("AS ","AS - ").replace("NI ","NI - ");_.innerHTML=`
                    <div style="width:36px; height:36px; border-radius:8px; background:#FEF7E0; display:flex; align-items:center; justify-content:center; flex-shrink:0; margin-right:12px;">${V}</div>
                    <div style="flex-grow:1; min-width:0;">
