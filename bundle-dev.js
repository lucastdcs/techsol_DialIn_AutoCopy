...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let f=i.querySelector(".cw-hero-grid"),b=i.querySelector(".cw-acc-container"),u=i.querySelector(".cw-results-container"),w=i.querySelector(".cw-search-input"),h=i.querySelector(".cw-status-bar"),x=i.querySelector(".cw-status-text"),C=i.querySelector(".cw-footer-icons");o.forEach(([y,g])=>{let l=n(g.name),c=document.createElement("div");c.className="cw-hero-card",c.id=`hero-${y}`,c.style.setProperty("--hero-color",l.color),c.innerHTML=`
        ...
        `,c.onclick=m=>{if(m.target.closest(".cw-step-btn"))return;let S=t[y]?t[y].count:0;T(y,S>0?-S:1,g)},c.querySelector(".minus").onclick=()=>T(y,-1,g),c.querySelector(".plus").onclick=()=>T(y,1,g),c.dataset.color=l.color,f.appendChild(c)});function E(y,g){let l=n(g.name),c=document.createElement("div");return c.className="cw-task-item",c.dataset.id=y,c.innerHTML=`
        ...
        `,c.onclick=m=>{if(m.target.closest(".cw-step-btn"))return;let S=t[y]?t[y].count:0;T(y,S>0?-S:1,g)},c.querySelector(".minus").onclick=()=>T(y,-1,g),c.querySelector(".plus").onclick=()=>T(y,1,g),c}Object.entries(r).forEach(([y,g])=>{let l=document.createElement("div");l.className="cw-acc-group";let c=document.createElement("div");c.className="cw-acc-header",c.innerHTML=`
            <div class="cw-acc-title">
                <div class="cw-acc-dot" style="background:${g.brand.color}"></div>
                ${y}
            </div>
            <div class="cw-acc-icon">\u25BC</div>
        `,c.onclick=()=>{b.querySelectorAll(".cw-acc-group.open").forEach(S=>{S!==l&&S.classList.remove("open")}),l.classList.toggle("open")};let m=document.createElement("div");m.className="cw-acc-body",g.tasks.forEach(S=>{let A=E(S.key,S);m.appendChild(A)}),l.appendChild(c),l.appendChild(m),b.appendChild(l)});function T(y,g,l){t[y]||(t[y]={count:0,data:l,brand:n(l.name)}),t[y].count+=g,t[y].count<=0&&delete t[y],j(),V(),e&&e()}function j(){o.forEach(([m])=>{let S=f.querySelector(`#hero-${m}`);if(!S)return;let A=t[m];A?(S.classList.add("active"),S.querySelector(".cw-step-val").textContent=A.count,S.querySelector(".cw-step-val").style.color=S.dataset.color):S.classList.remove("active")}),i.querySelectorAll(".cw-task-item").forEach(m=>{let S=m.dataset.id,A=t[S];A?(m.classList.add("selected"),m.querySelector(".cw-step-val").textContent=A.count):m.classList.remove("selected")});let g=Object.keys(t),l=0,c=[];if(g.forEach(m=>{let S=t[m];l+=S.count;for(let A=0;A<S.count;A++)c.length<6&&c.push(S.brand)}),l>0){h.classList.add("visible");let m=l>1?"A\xE7\xF5es":"A\xE7\xE3o",S=l>1?"definidas":"definida";x.textContent=`${l} ${m} ${S}`,C.innerHTML="",c.forEach(A=>{let N=document.createElement("div");N.className="cw-mini-icon",N.innerHTML=_e[A.icon]||_e.default;let $=N.querySelector("svg");$&&($.style.width="14px",$.style.height="14px"),C.appendChild(N)})}else h.classList.remove("visible")}w.addEventListener("input",y=>{let g=y.target.value.toLowerCase();if(g.length>0){b.style.display="none",u.style.display="block",u.innerHTML="";let l=!1;Object.entries(Se).forEach(([c,m])=>{if(m.name.toLowerCase().includes(g)){l=!0;let S=E(c,m);t[c]&&(S.classList.add("selected"),S.querySelector(".cw-step-val").textContent=t[c].count),u.appendChild(S)}}),l||(u.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else b.style.display="block",u.style.display="none"});function V(){p.innerHTML="";let y=Object.keys(t),g=!1,l="implementation";if(y.length===0){p.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let c=document.createElement("div");c.className="cw-info-banner",c.innerHTML=`
...
`).filter(ee=>ee.trim()!=="").map(ee=>`<p style="margin: 0 0 8px 0;">${ee}</p>`).join(""):B.tagName==="TEXTAREA"&&(q=q.replace(/\n/g,"<br>"));let Z=q.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(Z===""||Z==="\u2022"||Z.toLowerCase()==="n/a"){let ee=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${L}\\}(?:<br>\\s*)?`,"gi");ee.test(v)?v=v.replace(ee,""):v=v.replace(k,"")}else v=v.replace(k,q.replace(/\$/g,"$$$$"))}),v=v.replace(/{([A-Z0-9_]+)}/g,""),v=v.replace(/(<br>){3,}/g,"<br><br>"),typeof i<"u"&&i.getOutput&&(v+=i.getOutput()),v}Ge.onclick=()=>{let d=Ct();d?(xt(d),Q(z("copiado_sucesso"))):Q(z("selecione_substatus"),{error:!0})},Fe.onclick=async()=>{let d=O.value,G=Ct();if(!G){Q(z("selecione_substatus"),{error:!0});return}xt(G),dt();let v=Je(),R=await Bt();if(R)try{if(R.focus(),R.innerHTML.trim()==="<p><br></p>"||R.innerHTML.trim()==="<br>"||R.innerText.trim()===""){let ae=document.createRange();ae.selectNodeContents(R);let H=window.getSelection();H.removeAllRanges(),H.addRange(ae),document.execCommand("delete",!1,null)}else if(!R.innerHTML.endsWith("<br><br>")){let ae=document.createRange();ae.selectNodeContents(R),ae.collapse(!1);let H=window.getSelection();H.removeAllRanges(),H.addRange(ae),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,G),jt(R),setTimeout(()=>{Q(z("inserido_copiado"))},600);let oe=typeof Ie<"u"&&Ie?Ie.checked:!0;if(d&&Ke[d]&&oe){let ae=Ke[d];await Mt(ae),await new Promise(H=>setTimeout(H,500))}v(),ct(1.5),A.value="",O.innerHTML=`<option value="">${z("select_substatus")}</option>`,O.disabled=!0}catch(X){console.error(X),Q("Erro ao inserir.",{error:!0}),v()}};function ct(d=1.5){d<=1.5&&(Te.style.display="none",me.innerHTML=""),d<=2&&(be.style.display="none",s.reset(),le.style.display="none"),d<=3&&(fe.style.display="none",ye.innerHTML="",i.reset(),Me.style.display="none",ke.style.display="none")}function dt(){if(r=!r,r){let d=p.querySelector(".cw-expand-btn");d&&typeof d.resetState=="function"&&d.resetState()}we(r,p,"cw-btn-notes")}return it("bau"),st("pt"),dt}var Re={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
                    <p>Ol\xE1,</p>
                    <br>
