...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let u=c.querySelector(".cw-hero-grid"),l=c.querySelector(".cw-acc-container"),v=c.querySelector(".cw-results-container"),b=c.querySelector(".cw-search-input"),f=c.querySelector(".cw-status-bar"),A=c.querySelector(".cw-status-text"),T=c.querySelector(".cw-footer-icons");i.forEach(([h,x])=>{let r=o(x.name),d=document.createElement("div");d.className="cw-hero-card",d.id=`hero-${h}`,d.style.setProperty("--hero-color",r.color),d.innerHTML=`
        ...
        `,d.onclick=()=>{l.querySelectorAll(".cw-acc-group.open").forEach(p=>{p!==r&&p.classList.remove("open")}),r.classList.toggle("open")};let S=document.createElement("div");S.className="cw-acc-body",x.tasks.forEach(p=>{let y=q(p.key,p);S.appendChild(y)}),r.appendChild(d),r.appendChild(S),l.appendChild(r)});function R(h,x,r){e[h]||(e[h]={count:0,data:r,brand:o(r.name)}),e[h].count+=x,e[h].count<=0&&delete e[h],j(),U(),t&&t()}function j(){i.forEach(([S])=>{let p=u.querySelector(`#hero-${S}`);if(!p)return;let y=e[S];y?(p.classList.add("active"),p.querySelector(".cw-step-val").textContent=y.count,p.querySelector(".cw-step-val").style.color=p.dataset.color):p.classList.remove("active")}),c.querySelectorAll(".cw-task-item").forEach(S=>{let p=S.dataset.id,y=e[p];y?(S.classList.add("selected"),S.querySelector(".cw-step-val").textContent=y.count):S.classList.remove("selected")});let x=Object.keys(e),r=0,d=[];if(x.forEach(S=>{let p=e[S];r+=p.count;for(let y=0;y<p.count;y++)d.length<6&&d.push(p.brand)}),r>0){f.classList.add("visible");let S=r>1?"A\xE7\xF5es":"A\xE7\xE3o",p=r>1?"definidas":"definida";A.textContent=`${r} ${S} ${p}`,T.innerHTML="",d.forEach(y=>{let O=document.createElement("div");O.className="cw-mini-icon",O.innerHTML=Pe[y.icon]||Pe.default;let w=O.querySelector("svg");w&&(w.style.width="14px",w.style.height="14px"),T.appendChild(O)})}else f.classList.remove("visible")}b.addEventListener("input",h=>{let x=h.target.value.toLowerCase();if(x.length>0){l.style.display="none",v.style.display="block",v.innerHTML="";let r=!1;Object.entries(Re).forEach(([d,S])=>{if(S.name.toLowerCase().includes(x)){r=!0;let p=q(d,S);e[d]&&(p.classList.add("selected"),p.querySelector(".cw-step-val").textContent=e[d].count),v.appendChild(p)}}),r||(v.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else l.style.display="block",v.style.display="none"});function U(){m.innerHTML="";let h=Object.keys(e),x=!1,r=document.getElementById("sub-status"),d="implementation";if(r&&r.value.toLowerCase().includes("education")&&(d="education"),h.length===0){m.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}if(h.length===0){m.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let S=document.createElement("div");S.className="cw-info-banner",S.innerHTML=`
