...
`).filter(te=>te.trim()!=="").map(te=>`<p style="margin: 0 0 8px 0;">${te}</p>`).join(""):re.tagName==="TEXTAREA"&&(se=se.replace(/\n/g,"<br>"));let ye=se.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(ye===""||ye==="\u2022"||ye.toLowerCase()==="n/a"){let te=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${Z}\\}(?:<br>\\s*)?`,"gi");te.test(F)?F=F.replace(te,""):F=F.replace(X,"")}else F=F.replace(X,se.replace(/\$/g,"$$$$"))}),F=F.replace(/{([A-Z0-9_]+)}/g,""),F=F.replace(/(<br>){3,}/g,"<br><br>"),typeof r<"u"&&r.getOutput&&(F+=r.getOutput()),F}rt.onclick=()=>{let g=oo();g?(gt(g),W(ne("copiado_sucesso"))):W(ne("selecione_substatus"),{error:!0})},lt.onclick=async()=>{let g=d.value,P=oo();if(!P){W(ne("selecione_substatus"),{error:!0});return}gt(P),zt();let F=It(),Y=await Lt();if(Y)try{if(Y.focus(),Y.innerHTML.trim()==="<p><br></p>"||Y.innerHTML.trim()==="<br>"||Y.innerText.trim()===""){let pe=document.createRange();pe.selectNodeContents(Y);let ee=window.getSelection();ee.removeAllRanges(),ee.addRange(pe),document.execCommand("delete",!1,null)}else if(!Y.innerHTML.endsWith("<br><br>")){let pe=document.createRange();pe.selectNodeContents(Y),pe.collapse(!1);let ee=window.getSelection();ee.removeAllRanges(),ee.addRange(pe),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,P),Ft(Y),setTimeout(()=>{W(ne("inserido_copiado"))},600);let he=typeof Ke<"u"&&Ke?Ke.checked:!0;if(g&&Qe[g]&&he){let pe=Qe[g];await Tt(pe),await new Promise(ee=>setTimeout(ee,500))}F(),ht(1.5),_.value="",d.innerHTML=`<option value="">${ne("select_substatus")}</option>`,d.disabled=!0}catch(ie){console.error(ie),W("Erro ao inserir.",{error:!0}),F()}};function ht(g=1.5){g<=1.5&&(de.style.display="none",qe.innerHTML=""),g<=2&&(c.reset(),Ce.style.display="none"),g<=3&&(we.style.display="none",Ne.innerHTML="",r.reset(),Ve.style.display="none",Ye.style.display="none")}let Wo=null,yt="";function no(g){let P=document.getElementById("cw-btn-notes");if(!P)return;let F=P.querySelector(".cw-dot-dirty");if(g){if(!F){let Y=document.createElement("div");Y.className="cw-dot-dirty",P.appendChild(Y)}}else F&&F.remove()}async function Yo(){let g=await eo(),P=g.activeTasks.length>0;if(!P){let F=Object.keys(g.formData);for(let Y of F){let ie=g.formData[Y];if(ie&&ie.trim().length>3&&ie!=="\u2022 "){P=!0;break}}}if(no(P),P){let F=JSON.stringify(g);F!==yt&&(DraftService.saveEmergency(g),yt=F)}else yt!==""&&(DraftService.clearEmergency(),yt="")}Wo=setInterval(Yo,5e3),setTimeout(()=>{let g=DraftService.getEmergency();if(g){let P=document.createElement("button");P.innerHTML="\u26A0\uFE0F Recuperar trabalho n\xE3o salvo",P.style.cssText="width:100%; background:#FFF3E0; color:#B06000; border:none; padding:8px; font-size:12px; cursor:pointer; font-weight:600; border-bottom:1px solid #FFE0B2;",P.onclick=()=>{to(g),P.remove(),W("Trabalho recuperado!"),SoundManager.playSuccess()},b.insertBefore(P,b.firstChild),no(!0)}},1e3);function zt(){if(i=!i,i){let g=l.querySelector(".cw-expand-btn");g&&typeof g.resetState=="function"&&g.resetState()}Me(i,l,"cw-btn-notes")}return ft("bau"),xt("pt"),zt}var tt={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
 ...
 D) Nombre del sitio web.`,"autenticar la cuenta del anunciante en el cases, si corresponde.","T\xE9rminos y condi\xE7\xF5es.","Informar las Task solicitadas y AM.","Cerrar contenido sensible.","Confirmaci\xF3n de copia de seguridad y acceso de administrador a las ferramentas.","Resumen de llamada."],fim:["Ofrecer ayuda adicional.","Dejar de compartir la pantalla.","Pasos siguientes  (Si se le har\xE1 seguimiento al caso).","Encuesta de Satisfa\xE7\xE3o.","Informar al cliente que el equipo de QA ir\xE1 a realizar pruebas en los siguientes dias."]},"EN BAU":{color:"#ff0011",inicio:["Example 1","Example 2"],fim:["Example 3","Example 4"]}};function Do(){let e="v2.6 (Context HD)",t="csa-local-styles";if(!document.getElementById(t)){let d=document.createElement("style");d.id=t,d.innerHTML=`
  ...
  `,f.onfocus=()=>{h.style.boxShadow="0 4px 12px rgba(26,115,232,0.15)",h.style.border="1px solid #1a73e8"},f.onblur=()=>{h.style.boxShadow="0 2px 6px rgba(0,0,0,0.04)",h.style.border="1px solid transparent"},h.appendChild(x),h.appendChild(f),b.appendChild(h),u.appendChild(b);let E=document.createElement("div");E.style.cssText="flex: 1; overflow-y: auto; padding: 0 24px 40px 24px; scroll-behavior: smooth;",u.appendChild(E);let A=null;function H(){if(A)return;A=document.createElement("div"),A.style.cssText=`
      ...
      `,L.title="Copiar URL",v.onmouseenter=()=>{v.style.transform="translateY(-2px)",v.style.boxShadow="0 8px 20px rgba(0,0,0,0.08)",v.style.borderColor="rgba(0,0,0,0.05)",v.style.borderLeft=`4px solid ${d.color}`,L.style.opacity="1",L.style.background="#F1F3F4"},v.onmouseleave=()=>{v.style.transform="translateY(0)",v.style.boxShadow="0 1px 3px rgba(0,0,0,0.05)",v.style.border="1px solid transparent",L.style.opacity="0",L.style.background="transparent"},v.onclick=()=>{!U&&m&&zo(R,m),window.open(R.url,"_blank")},L.onclick=D=>{D.stopPropagation(),K.playClick(),navigator.clipboard.writeText(R.url),!U&&m&&zo(R,m),W("Link copiado!")},v.appendChild(y),v.appendChild(q),v.appendChild(L),v}f.addEventListener("input",R=>{t=R.target.value,O()});function N(){n=!n,Me(n,a,"cw-btn-links")}return document.body.appendChild(a),w(),O(),N}var Ue=[];function Jt(e){Ue=e}var un=["lucaste","ricardogi"],mn=60*1e3;window._cwIsAdmin=!1;window._cwCurrentUser=null;function Go(){let e="v4.9",t=!1,n=null,o=null;function i(m){if(!m)return"";try{let v=new Date(m);return isNaN(v.getTime())?String(m):v.toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}).replace(","," \xE0s")}catch{return String(m)}}if(!document.getElementById("cw-broadcast-hd-css")){let m=document.createElement("style");m.id="cw-broadcast-hd-css",m.innerHTML=`
        @keyframes cw-pulse {
...
`),I=/\d{1,2}\/\d{1,2}/;if(V.forEach(ue=>{let me=ue.match(I);if(me){let Ie=me[0],ce="\u{1F4C5}",oe=/🇧🇷|🇵🇹|PT|BR|BRASIL|BRAZIL|PORTUGAL|LISBOA|SAO PAULO|SÃO PAULO/i.test(ue),le=/🇪🇸|🇲🇽|ES|LATAM|ESPANHA|SPAIN|MEXICO|MÉXICO|MADRID|BARCELONA/i.test(ue);oe?ce="\u{1F1E7}\u{1F1F7}":le&&(ce="\u{1F1EA}\u{1F1F8}"),D.some(Oe=>Oe.flag===ce&&Oe.date===Ie)||D.push({flag:ce,date:Ie})}}),D.length===0){let ue=(k.text||"").match(/\d{1,2}\/\d{1,2}/g);ue&&[...new Set(ue)].forEach(me=>D.push({flag:"\u{1F4C5}",date:me}))}let $="",J='<button id="cw-bau-toggle-btn" class="cw-btn-interactive" style="background:rgba(255,255,255,0.7); border:1px solid rgba(139, 92, 246, 0.4); border-radius:12px; padding:8px 12px; color:#6D28D9; font-size:12px; font-weight:600;">Detalhes</button>';window._cwIsAdmin&&(J=`
        ...
        `,G.appendChild(k),G.appendChild(L),T.appendChild(G);let D=document.createElement("div");D.id="cw-planner-status",Object.assign(D.style,l.statusBadge),T.appendChild(D);let V=document.createElement("div");Object.assign(V.style,{padding:"0 4px",marginTop:"12px"});let I=document.createElement("div");I.textContent="Arraste para simular o hor\xE1rio:",I.style.cssText="font-size:12px; color:#5F6368; text-align:center; margin-bottom:12px;";let $=document.createElement("div");Object.assign($.style,l.timelineContainer);let J=document.createElement("div");Object.assign(J.style,l.timelineTrack);let xe=document.createElement("div");Object.assign(xe.style,l.dayZone),J.appendChild(xe);let fe=document.createElement("input");fe.type="range",fe.min="0",fe.max="1439",fe.step="15",fe.style.cssText="position:absolute; top:20px; left:0; width:100%; -webkit-appearance:none; background:transparent; z-index:2; cursor:pointer;";let ue=document.createElement("div");ue.style.cssText="position:absolute; top:36px; width:100%; display:flex; justify-content:space-between; font-size:10px; font-weight:600; color:#9AA0A6; padding:0 2px;",ue.innerHTML="<span>00h</span><span>06h</span><span>12h</span><span>18h</span><span>24h</span>",$.appendChild(J),$.appendChild(fe),$.appendChild(ue),V.appendChild(I),V.appendChild($),T.appendChild(V);let me=k.querySelector("#cw-time-input-br"),Ie=L.querySelector("#cw-time-display-client"),ce=L.querySelector("#cw-client-label");function oe(){let le=Zt.find(Ne=>Ne.id===o);ce.textContent=`${le.flag} ${le.label} (${le.zone})`;let de=r.getHours(),Oe=r.getMinutes(),qe=`${String(de).padStart(2,"0")}:${String(Oe).padStart(2,"0")}`;me.value=qe,fe.value=de*60+Oe;let we=r.toLocaleTimeString("pt-BR",{timeZone:le.zone,hour:"2-digit",minute:"2-digit"});Ie.textContent=we;let _e=parseInt(we.split(":")[0]);_e>=9&&_e<17?(D.style.background=c.successBg,D.style.color=c.success,D.innerHTML='<span style="font-size:16px">\u2705</span> Hor\xE1rio Comercial Ideal'):_e>=8&&_e<9||_e>=17&&_e<19?(D.style.background=c.warningBg,D.style.color=c.warning,D.innerHTML='<span style="font-size:16px">\u26A0\uFE0F</span> Hor\xE1rio Limite (Aten\xE7\xE3o)'):(D.style.background=c.errorBg,D.style.color=c.error,D.innerHTML='<span style="font-size:16px">\u26D4</span> Fora de Hor\xE1rio')}fe.oninput=le=>{let de=parseInt(le.target.value);r.setHours(Math.floor(de/60)),r.setMinutes(de%60),oe()},me.oninput=le=>{let[de,Oe]=le.target.value.split(":");de&&Oe&&(r.setHours(parseInt(de)),r.setMinutes(parseInt(Oe)),oe())},oe()}function U(){R(),n||(n=setInterval(R,6e4))}function m(){n&&(clearInterval(n),n=null)}function v(){t=!t,Me(t,p,"cw-btn-timezone"),t?O("live"):m()}return document.body.appendChild(p),v}function fn(){if(window.techSolInitialized){Ht();return}window.techSolInitialized=!0;let e="v5.0";console.log(`\u{1F680} TechSol Suite Initializing (${e})...`);try{go();try{K.initGlobalListeners(),K.playStartup()}catch(r){console.warn("\xC1udio bloqueado:",r)}ke.fetchTips(),Ht();let t=No(),n=_o(),o=Do(),i=Bo(),s=Vo(),a=Go();To({toggleNotes:t,toggleEmail:n,toggleScript:o,toggleLinks:i,toggleTimezone:s,broadcastControl:a}),setTimeout(()=>{ke.logEvent("App","Start","Session Start"),Po(),setTimeout(()=>{Ho(e)},500)},2500)}catch(t){console.error("Erro fatal na inicializa\xE7\xE3o:",t),W("Erro cr\xEDtico ao iniciar o Case Wizard.",{error:!0})}}fn();})();
