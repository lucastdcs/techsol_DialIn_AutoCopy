...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
        ...
        `,document.head.appendChild(f)}let i=document.createElement("div");i.className="cw-zen-container";let s=document.createElement("div");Object.assign(s.style,{display:"none"});let b=document.createElement("div");b.className="cw-screens-container",s.appendChild(b),i.innerHTML=`
    ...
    `;let y=i.querySelector(".cw-hero-grid"),v=i.querySelector(".cw-acc-container"),m=i.querySelector(".cw-results-container"),E=i.querySelector(".cw-search-input"),C=i.querySelector(".cw-status-bar"),x=i.querySelector(".cw-status-text"),A=i.querySelector(".cw-footer-icons");o.forEach(([f,u])=>{let l=n(u.name),c=document.createElement("div");c.className="cw-hero-card",c.id=`hero-${f}`,c.style.setProperty("--hero-color",l.color),c.innerHTML=`
        ...
        `,c.onclick=p=>{if(p.target.closest(".cw-step-btn"))return;let h=t[f]?t[f].count:0;T(f,h>0?-h:1,u)},c.querySelector(".minus").onclick=()=>T(f,-1,u),c.querySelector(".plus").onclick=()=>T(f,1,u),c.dataset.color=l.color,y.appendChild(c)});function w(f,u){let l=n(u.name),c=document.createElement("div");return c.className="cw-task-item",c.dataset.id=f,c.innerHTML=`
        ...
        `,c.onclick=()=>{v.querySelectorAll(".cw-acc-group.open").forEach(h=>{h!==l&&h.classList.remove("open")}),l.classList.toggle("open")};let p=document.createElement("div");p.className="cw-acc-body",u.tasks.forEach(h=>{let S=w(h.key,h);p.appendChild(S)}),l.appendChild(c),l.appendChild(p),v.appendChild(l)});function T(f,u,l){t[f]||(t[f]={count:0,data:l,brand:n(l.name)}),t[f].count+=u,t[f].count<=0&&delete t[f],j(),V(),e&&e()}function j(){o.forEach(([p])=>{let h=y.querySelector(`#hero-${p}`);if(!h)return;let S=t[p];S?(h.classList.add("active"),h.querySelector(".cw-step-val").textContent=S.count,h.querySelector(".cw-step-val").style.color=h.dataset.color):h.classList.remove("active")}),i.querySelectorAll(".cw-task-item").forEach(p=>{let h=p.dataset.id,S=t[h];S?(p.classList.add("selected"),p.querySelector(".cw-step-val").textContent=S.count):p.classList.remove("selected")});let u=Object.keys(t),l=0,c=[];if(u.forEach(p=>{let h=t[p];l+=h.count;for(let S=0;S<h.count;S++)c.length<6&&c.push(h.brand)}),l>0){C.classList.add("visible");let p=l>1?"A\xE7\xF5es":"A\xE7\xE3o",h=l>1?"definidas":"definida";x.textContent=`${l} ${p} ${h}`,A.innerHTML="",c.forEach(S=>{let N=document.createElement("div");N.className="cw-mini-icon",N.innerHTML=_e[S.icon]||_e.default;let $=N.querySelector("svg");$&&($.style.width="14px",$.style.height="14px"),A.appendChild(N)})}else C.classList.remove("visible")}E.addEventListener("input",f=>{let u=f.target.value.toLowerCase();if(u.length>0){v.style.display="none",m.style.display="block",m.innerHTML="";let l=!1;Object.entries(Se).forEach(([c,p])=>{if(p.name.toLowerCase().includes(u)){l=!0;let h=w(c,p);t[c]&&(h.classList.add("selected"),h.querySelector(".cw-step-val").textContent=t[c].count),m.appendChild(h)}}),l||(m.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else v.style.display="block",m.style.display="none"});function V(){b.innerHTML="";let f=Object.keys(t),u=!1,l="implementation";if(f.length===0){b.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let c=document.createElement("div");c.className="cw-info-banner",c.innerHTML=`
