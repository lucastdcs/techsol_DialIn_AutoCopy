...
`).filter(B=>B.trim()!=="").map(B=>`<p style="margin: 0 0 8px 0;">${B}</p>`).join(""):W.tagName==="TEXTAREA"&&($=$.replace(/\n/g,"<br>"));let le=$.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(le===""||le==="\u2022"||le.toLowerCase()==="n/a"){let B=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${D}\\}(?:<br>\\s*)?`,"gi");B.test(E)?E=E.replace(B,""):E=E.replace(_,"")}else E=E.replace(_,$.replace(/\$/g,"$$$$"))}),E=E.replace(/{([A-Z0-9_]+)}/g,""),E=E.replace(/(<br>){3,}/g,"<br><br>"),typeof a<"u"&&a.getOutput&&(E+=a.getOutput()),E}Ke.onclick=()=>{let C=_t();C?(Ot(C),ne(P("copiado_sucesso"))):ne(P("selecione_substatus"),{error:!0})},Qe.onclick=async()=>{let C=w.value,M=_t();if(!M){ne(P("selecione_substatus"),{error:!0});return}Ot(M),ft();let E=Xe(),H=await ro();if(H)try{if(H.focus(),H.innerHTML.trim()==="<p><br></p>"||H.innerHTML.trim()==="<br>"||H.innerText.trim()===""){let pe=document.createRange();pe.selectNodeContents(H);let K=window.getSelection();K.removeAllRanges(),K.addRange(pe),document.execCommand("delete",!1,null)}else if(!H.innerHTML.endsWith("<br><br>")){let pe=document.createRange();pe.selectNodeContents(H),pe.collapse(!1);let K=window.getSelection();K.removeAllRanges(),K.addRange(pe),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,M),so(H),setTimeout(()=>{ne(P("inserido_copiado"))},600);let ce=typeof ze<"u"&&ze?ze.checked:!0;if(C&&st[C]&&ce){let pe=st[C];await Kt(pe),await new Promise(K=>setTimeout(K,500))}E(),bt(1.5),S.value="",w.innerHTML=`<option value="">${P("select_substatus")}</option>`,w.disabled=!0}catch(oe){console.error(oe),ne("Erro ao inserir.",{error:!0}),E()}};function bt(C=1.5){C<=1.5&&(Me.style.display="none",Ae.innerHTML=""),C<=2&&(l.reset(),ue.style.display="none"),C<=3&&(he.style.display="none",_e.innerHTML="",a.reset(),Ue.style.display="none",Fe.style.display="none")}function ft(){if(i=!i,i){let C=u.querySelector(".cw-expand-btn");C&&typeof C.resetState=="function"&&C.resetState()}Te(i,u,"cw-btn-notes")}return pt("bau"),ut("pt"),ft}var Ve={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
                    <p>Ol\xE1,</p>
                    <br>
