...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
        ...
        `,document.head.appendChild(v)}let l=document.createElement("div");l.className="cw-zen-container";let m=document.createElement("div");Object.assign(m.style,{display:"none"});let b=document.createElement("div");b.className="cw-screens-container",m.appendChild(b),l.innerHTML=`
    ...
    `;let g=l.querySelector(".cw-hero-grid"),c=l.querySelector(".cw-acc-container"),f=l.querySelector(".cw-results-container"),A=l.querySelector(".cw-search-input"),x=l.querySelector(".cw-status-bar"),T=l.querySelector(".cw-status-text"),S=l.querySelector(".cw-footer-icons");s.forEach(([v,w])=>{let r=o(w.name),u=document.createElement("div");u.className="cw-hero-card",u.id=`hero-${v}`,u.style.setProperty("--hero-color",r.color),u.innerHTML=`
        ...
        `,u.onclick=d=>{if(d.target.closest(".cw-step-btn"))return;let p=t[v]?t[v].count:0;R(v,p>0?-p:1,w)},u.querySelector(".minus").onclick=()=>R(v,-1,w),u.querySelector(".plus").onclick=()=>R(v,1,w),u.dataset.color=r.color,g.appendChild(u)});function q(v,w){let r=o(w.name),u=document.createElement("div");return u.className="cw-task-item",u.dataset.id=v,u.innerHTML=`
        ...
        `,u.onclick=d=>{if(d.target.closest(".cw-step-btn"))return;let p=t[v]?t[v].count:0;R(v,p>0?-p:1,w)},u.querySelector(".minus").onclick=()=>R(v,-1,w),u.querySelector(".plus").onclick=()=>R(v,1,w),u}Object.entries(i).forEach(([v,w])=>{let r=document.createElement("div");r.className="cw-acc-group";let u=document.createElement("div");u.className="cw-acc-header",u.innerHTML=`
            <div class="cw-acc-title">
                <div class="cw-acc-dot" style="background:${w.brand.color}"></div>
                ${v}
            </div>
            <div class="cw-acc-icon">\u25BC</div>
        `,u.onclick=()=>{c.querySelectorAll(".cw-acc-group.open").forEach(p=>{p!==r&&p.classList.remove("open")}),r.classList.toggle("open")};let d=document.createElement("div");d.className="cw-acc-body",w.tasks.forEach(p=>{let h=q(p.key,p);d.appendChild(h)}),r.appendChild(u),r.appendChild(d),c.appendChild(r)});function R(v,w,r){t[v]||(t[v]={count:0,data:r,brand:o(r.name)}),t[v].count+=w,t[v].count<=0&&delete t[v],V(),K(),e&&e()}function V(){s.forEach(([d])=>{let p=g.querySelector(`#hero-${d}`);if(!p)return;let h=t[d];h?(p.classList.add("active"),p.querySelector(".cw-step-val").textContent=h.count,p.querySelector(".cw-step-val").style.color=p.dataset.color):p.classList.remove("active")}),l.querySelectorAll(".cw-task-item").forEach(d=>{let p=d.dataset.id,h=t[p];h?(d.classList.add("selected"),d.querySelector(".cw-step-val").textContent=h.count):d.classList.remove("selected")});let w=Object.keys(t),r=0,u=[];if(w.forEach(d=>{let p=t[d];r+=p.count;for(let h=0;h<p.count;h++)u.length<6&&u.push(p.brand)}),r>0){x.classList.add("visible");let d=r>1?"A\xE7\xF5es":"A\xE7\xE3o",p=r>1?"definidas":"definida";T.textContent=`${r} ${d} ${p}`,S.innerHTML="",u.forEach(h=>{let C=document.createElement("div");C.className="cw-mini-icon",C.innerHTML=Ve[h.icon]||Ve.default;let y=C.querySelector("svg");y&&(y.style.width="14px",y.style.height="14px"),S.appendChild(C)})}else x.classList.remove("visible")}A.addEventListener("input",v=>{let w=v.target.value.toLowerCase();if(w.length>0){c.style.display="none",f.style.display="block",f.innerHTML="";let r=!1;Object.entries(Re).forEach(([u,d])=>{if(d.name.toLowerCase().includes(w)){r=!0;let p=q(u,d);t[u]&&(p.classList.add("selected"),p.querySelector(".cw-step-val").textContent=t[u].count),f.appendChild(p)}}),r||(f.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else c.style.display="block",f.style.display="none"});function K(){b.innerHTML="";let v=Object.keys(t),w=!1,r=document.getElementById("sub-status"),u="implementation";if(r&&r.value.toLowerCase().includes("education")&&(u="education"),v.length===0){b.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}if(v.length===0){b.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let d=document.createElement("div");d.className="cw-info-banner",d.innerHTML=`
        ...
        `,b.appendChild(d),v.forEach(p=>{let h=t[p].data,C=t[p].count,y=t[p].brand,O=h.screenshots?h.screenshots[u]||[]:["Link da Evid\xEAncia"];if(O.length>0){w=!0;for(let se=1;se<=C;se++){let Z=document.createElement("div");Z.className="cw-screen-card",Z.style.setProperty("--brand-color",y.color),Z.style.setProperty("--brand-bg",y.bg),Z.style.setProperty("--brand-shadow",y.color+"40");let G=document.createElement("div");G.className="cw-card-header";let B=document.createElement("div");B.className="cw-card-icon",B.innerHTML=Ve[y.icon]||Ve.default;let J=document.createElement("div");J.style.cssText="flex:1; display:flex; align-items:center; gap:8px;";let I=document.createElement("input");I.className="cw-card-title-input",I.id=`name-${p}-${se}`,I.value=`${h.name}${C>1?" #"+se:""}`,I.title="Clique para renomear esta task";let Y=document.createElement("span");Y.className="cw-edit-hint",Y.innerHTML="\u270E Renomear",J.appendChild(I),J.appendChild(Y),G.appendChild(B),G.appendChild(J),Z.appendChild(G),O.forEach((re,M)=>{let $=document.createElement("div");$.className="cw-input-group";let te=document.createElement("label");te.className="cw-input-label",te.textContent=re.replace(/📷|:|•/g,"").trim();let _=document.createElement("input");_.className="cw-input-field",_.id=`screen-${p}-${se}-${M}`,_.placeholder="Cole o link aqui...",_.setAttribute("autocomplete","off"),_.addEventListener("input",()=>{_.value.trim().length>5?_.classList.add("filled"):_.classList.remove("filled")});let X=document.createElement("div");X.className="cw-input-check",X.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>',$.appendChild(te),$.appendChild(_),$.appendChild(X),Z.appendChild($)}),b.appendChild(Z)}}}),m.style.display=w?"block":"none"}return{selectionElement:l,screenshotsElement:m,updateSubStatus:()=>K(),getCheckedElements:()=>Object.keys(t).map(v=>({value:v,closest:()=>({querySelector:()=>({textContent:t[v].count})})})),toggleTask:(v,w=!0)=>{let r=t[v];w&&!r?R(v,1,Re[v]):!w&&r&&R(v,-r.count,Re[v])},setMode:v=>{n=v,K()},reset:()=>{for(let v in t)delete t[v];A.value="",c.style.display="block",f.style.display="none",V(),K()}}}function go(){let e="v3.6.0",t="bau",n="pt",o=!1,s=!1,i={input:{width:"100%",padding:"8px",borderRadius:"8px",border:"1px solid #dadce0",fontSize:"14px",marginBottom:"12px",boxSizing:"border-box",fontFamily:"'Google Sans', Roboto, sans-serif",transition:"border-color 0.2s ease, box-shadow 0.2s ease"},textarea:{width:"100%",padding:"8px",borderRadius:"8px",border:"1px solid #dadce0",fontSize:"14px",marginBottom:"12px",boxSizing:"border-box",fontFamily:"'Google Sans', Roboto, sans-serif",transition:"border-color 0.2s ease, box-shadow 0.2s ease",height:"100px",resize:"vertical"},h3:{fontSize:"14px",fontWeight:"600",color:"#202124",margin:"0 0 12px 0"},label:{display:"block",fontSize:"14px",fontWeight:"500",color:"#3c4043",marginBottom:"8px",marginTop:"16px"},checkboxLabel:{display:"flex",alignItems:"center",marginBottom:"10px",fontSize:"14px",fontWeight:"400",cursor:"pointer",padding:"8px",background:"#f8f9fa",borderRadius:"6px",transition:"background-color 0.2s ease, box-shadow 0.2s ease",userSelect:"none"},checkboxInput:{width:"auto",marginRight:"10px",marginBottom:"0",cursor:"pointer",accentColor:"#1a73e8"},buttonBase:{flex:"1 1 0",padding:"10px 0",color:"#fff",border:"none",borderRadius:"8px",fontSize:"14px",fontWeight:"500",cursor:"pointer",marginTop:"16px"},stepBlock:{borderTop:"1px solid #eee",paddingTop:"12px",marginTop:"12px"},radioContainer:{display:"flex",gap:"15px",marginBottom:"10px"},optionalBtn:{width:"100%",padding:"10px",background:"white",border:"1px dashed #1a73e8",color:"#1a73e8",borderRadius:"6px",cursor:"pointer",fontWeight:"500",fontSize:"13px",marginBottom:"10px",transition:"all 0.2s"},helpLink:{fontSize:"12px",color:"#1a73e8",textDecoration:"none",cursor:"pointer",fontWeight:"500",display:"flex",alignItems:"center",gap:"4px",marginLeft:"auto",transition:"color 0.2s"}},a=uo(),l=mo(()=>{let L=l.getCheckedElements().map(k=>k.value);y&&y.value&&a.updateVisibility(y.value,L)}),m=document.createElement("div");m.id="autofill-popup",m.classList.add("cw-module-window"),Object.assign(m.style,we,{right:"100px",width:"400px",transition:"width 0.3s cubic-bezier(0.4, 0.0, 0.2, 1)"});let g=Ee(m,"Case Notes Assistant",e,"Gera notas padronizadas automaticamente. Selecione o status, as tasks realizadas e preencha os detalhes para inserir no caso.",{popup:m,googleLine:null},()=>yt());m.appendChild(g);let c=document.createElement("div");Object.assign(c.style,{padding:"0 16px 16px 16px",overflowY:"auto",flexGrow:"1"}),m.appendChild(c);let f=document.createElement("div");f.textContent="created by lucaste@",Object.assign(f.style,Wt),m.appendChild(f);let A=document.createElement("div");A.id="step-lang-type";let x=document.createElement("label");Object.assign(x.style,i.label),A.appendChild(x);let T=document.createElement("div");Object.assign(T.style,{display:"flex",borderRadius:"8px",border:"1px solid #dadce0",overflow:"hidden",marginBottom:"16px"});let S=document.createElement("div");S.textContent="Portugu\xEAs",S.classList.add("no-drag"),Object.assign(S.style,fe);let q=document.createElement("div");q.textContent="Espa\xF1ol",q.classList.add("no-drag"),Object.assign(q.style,fe),S.onclick=()=>bt("pt"),q.onclick=()=>bt("es"),T.appendChild(S),T.appendChild(q),A.appendChild(T),c.appendChild(A);let R=document.createElement("div");R.id="step-0-case-type";let V=document.createElement("label");Object.assign(V.style,i.label),R.appendChild(V);let K=document.createElement("div");Object.assign(K.style,{display:"flex",borderRadius:"8px",border:"1px solid #dadce0",overflow:"hidden",marginBottom:"16px"});let v=document.createElement("div");v.textContent="BAU",v.classList.add("no-drag"),Object.assign(v.style,fe);let w=document.createElement("div");w.textContent="LM",w.classList.add("no-drag"),Object.assign(w.style,fe),v.onclick=()=>gt("bau"),w.onclick=()=>gt("lm"),K.appendChild(v),K.appendChild(w),R.appendChild(K),c.appendChild(R);let r=document.createElement("div");r.id="step-1-selection";let u=document.createElement("label");u.className="cw-input-label",u.textContent="Status Principal";let d=document.createElement("select");d.id="main-status",d.className="cw-select",d.innerHTML=`
  ...
  `;let p=document.createElement("div");p.style.cssText="display: flex; justify-content: space-between; align-items: baseline; margin-top: 16px; margin-bottom: 6px;";let h=document.createElement("label");h.className="cw-input-label",h.textContent="Sub-status",h.style.marginBottom="0";let C=document.createElement("a");C.href="https://seu-link-do-guia-aqui.com",C.target="_blank",C.className="cw-info-link",C.innerHTML='<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px; transform:translateY(1px)"><circle cx="12" cy="12" r="10"></circle><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"></path><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>Guia de Substatus',Object.assign(C.style,i.helpLink),p.appendChild(h),p.appendChild(C);let y=document.createElement("select");y.id="sub-status",y.className="cw-select",y.disabled=!0,y.innerHTML='<option value="" disabled selected hidden>Aguardando status principal...</option>',r.appendChild(u),r.appendChild(d),r.appendChild(p),r.appendChild(y),c.appendChild(r);let O=document.createElement("div");O.id="step-1-1-portugal",Object.assign(O.style,i.stepBlock,{display:"none"});let se=document.createElement("label");Object.assign(se.style,i.label),O.appendChild(se);let Z=document.createElement("div");Object.assign(Z.style,i.radioContainer);let G=document.createElement("div");Object.assign(G.style,{display:"flex",alignItems:"center"});let B=document.createElement("input");B.type="radio",B.name="portugal-group",B.value="sim",Object.assign(B.style,i.checkboxInput);let J=document.createElement("label");J.htmlFor="portugal-sim",Object.assign(J.style,{cursor:"pointer"}),G.appendChild(B),G.appendChild(J);let I=document.createElement("div");Object.assign(I.style,{display:"flex",alignItems:"center"});let Y=document.createElement("input");Y.type="radio",Y.name="portugal-group",Y.value="nao",Y.checked=!0,Object.assign(Y.style,i.checkboxInput);let re=document.createElement("label");re.htmlFor="portugal-nao",Object.assign(re.style,{cursor:"pointer"}),I.appendChild(Y),I.appendChild(re),Z.appendChild(G),Z.appendChild(I),O.appendChild(Z),c.appendChild(O);function M(E){o=E,E?$.style.display="block":$.style.display="none"}B.onchange=()=>M(!0),Y.onchange=()=>M(!1);let $=document.createElement("div");$.id="step-1-2-consent",Object.assign($.style,i.stepBlock,{display:"none"});let te=document.createElement("label");Object.assign(te.style,i.label),$.appendChild(te);let _=document.createElement("div");Object.assign(_.style,i.radioContainer);let X=document.createElement("div");Object.assign(X.style,{display:"flex",alignItems:"center"});let xe=document.createElement("input");xe.type="radio",xe.name="consent-group",xe.value="Sim",xe.checked=!0,Object.assign(xe.style,i.checkboxInput);let ce=document.createElement("label");ce.htmlFor="consent-sim",Object.assign(ce.style,{cursor:"pointer"}),X.appendChild(xe),X.appendChild(ce);let oe=document.createElement("div");Object.assign(oe.style,{display:"flex",alignItems:"center"});let Oe=document.createElement("input");Oe.type="radio",Oe.name="consent-group",Oe.value="N\xE3o",Object.assign(Oe.style,i.checkboxInput);let Me=document.createElement("label");Me.htmlFor="consent-nao",Object.assign(Me.style,{cursor:"pointer"}),oe.appendChild(Oe),oe.appendChild(Me),_.appendChild(X),_.appendChild(oe),$.appendChild(_),c.appendChild($);let Le=document.createElement("div");Le.id="step-1-5-snippets",Object.assign(Le.style,i.stepBlock,{display:"none"});let nt=document.createElement("h3");Object.assign(nt.style,i.h3),nt.textContent="Cen\xE1rios Comuns";let Ae=so(E=>{let L=document.querySelector("textarea");L&&(L.value=E,L.dispatchEvent(new Event("input")),L.style.transition="background-color 0.2s",L.style.backgroundColor="#e8f0fe",setTimeout(()=>L.style.backgroundColor="#fff",300))});Ae.id="snippet-container",Le.appendChild(nt),Le.appendChild(Ae),c.appendChild(Le);let ye=document.createElement("div");ye.id="step-3-form",Object.assign(ye.style,i.stepBlock,{display:"none"});let mt=document.createElement("h3");Object.assign(mt.style,i.h3),ye.appendChild(mt);let _e=document.createElement("div");_e.id="dynamic-form-fields-container",ye.appendChild(_e);let ge=document.createElement("button");ge.textContent="+ Gostaria de selecionar uma task?",Object.assign(ge.style,i.optionalBtn),ge.onmouseover=()=>ge.style.background="#e8f0fe",ge.onmouseout=()=>ge.style.background="white",ge.onclick=()=>{ge.style.display="none",Fe.style.display="block",l.selectionElement.style.display="block"};let Fe=document.createElement("h3");Object.assign(Fe.style,i.h3,{marginTop:"20px"});let Rt=l.selectionElement;Object.assign(Rt.style,{marginBottom:"20px"}),ye.appendChild(ge),ye.appendChild(Fe),ye.appendChild(Rt),ye.appendChild(a.element),ye.appendChild(l.screenshotsElement),c.appendChild(ye);let De=document.createElement("div");De.id="step-4-email",Object.assign(De.style,{display:"none",marginTop:"16px",paddingTop:"12px",borderTop:"1px solid #eee"});let Ge=document.createElement("label");Ge.style.display="flex",Ge.style.alignItems="center",Ge.style.cursor="pointer",Ge.style.fontSize="14px";let ze=document.createElement("input");ze.type="checkbox",ze.checked=!0,Object.assign(ze.style,i.checkboxInput),Ge.appendChild(ze),Ge.appendChild(document.createTextNode("Preencher email automaticamente?")),De.appendChild(Ge),c.appendChild(De);let We=document.createElement("div");Object.assign(We.style,{display:"none",gap:"8px",padding:"0"}),c.appendChild(We);let Ke=document.createElement("button");Object.assign(Ke.style,i.buttonBase,{backgroundColor:"#5f6368"}),Ke.textContent="Copiar";let Qe=document.createElement("button");Object.assign(Qe.style,i.buttonBase,{backgroundColor:"#1a73e8"}),Qe.textContent="Preencher",We.appendChild(Ke),We.appendChild(Qe);let Ze=document.createElement("div");Object.assign(Ze.style,Ye),Ze.className="no-drag",Ze.title="Redimensionar",m.appendChild(Ze),Xe(m,Ze),document.body.appendChild(m);function gt(E){t=E;let L=Je();Object.assign(v.style,fe),Object.assign(w.style,fe),E==="bau"?(Object.assign(v.style,L),C.href="https://docs.google.com/presentation/d/1Jra4de31rYaFZ0JMH_CiVzr2S3F8J1qcUcJRaw0Lw5Q/edit?slide=id.g2fd98a4e46f_11_2657#slide=id.g2fd98a4e46f_11_2657"):(Object.assign(w.style,L),C.href="https://docs.google.com/presentation/d/1CzrmfatUVReZWKAbwxHxcB3YXsH834xO6pPhTsZ7yjM/edit?slide=id.g1f1e626a09c_2_2243&resourcekey=0-KmtR4noMvEx3PODZk_ZvQw#slide=id.g1f1e626a09c_2_2243"),y.value&&y.dispatchEvent(new Event("change"))}function j(E){try{if(Ne&&Ne[n]&&Ne[n][E])return Ne[n][E];if(Ne&&Ne.pt&&Ne.pt[E])return Ne.pt[E]}catch{}return E}function vo(){x.textContent=j("idioma"),V.textContent=j("fluxo"),u.textContent=j("status_principal"),h.textContent=j("substatus"),nt.textContent=j("cenarios_comuns"),Fe.textContent=j("selecione_tasks"),mt.textContent=j("preencha_detalhes"),Ke.textContent=j("copiar"),Qe.textContent=j("preencher"),d.querySelector('option[value=""]')&&(d.querySelector('option[value=""]').textContent=j("select_status")),y.querySelector('option[value=""]')&&(y.querySelector('option[value=""]').textContent=j("select_substatus")),se.textContent=j("caso_portugal"),J.textContent=j("sim"),re.textContent=j("nao"),te.textContent=j("consentiu_gravacao"),ce.textContent=j("sim"),Me.textContent=j("nao"),_e.querySelectorAll("label").forEach(E=>{let L=E.nextElementSibling.id.replace("field-",""),k=j(L.toLowerCase());k!==L.toLowerCase()?E.textContent=k:E.textContent=L.replace(/_/g," ").replace(/\b\w/g,P=>P.toUpperCase())+":"}),ge.textContent="+ "+(n==="pt"?"Gostaria de selecionar uma task?":"Quisiera seleccionar una tarea?")}function bt(E){n=E;let L=Je();Object.assign(S.style,fe),Object.assign(q.style,fe),E==="pt"?(Object.assign(S.style,L),O.style.display="block",M(o)):(Object.assign(q.style,L),O.style.display="none",$.style.display="none"),vo(),y.value&&y.dispatchEvent(new Event("change"))}function ft(E){(E.value.trim()===""||E.value.trim()==="\u2022")&&(E.value="\u2022 "),E.onkeydown=function(L){if(L.key==="Enter"){L.preventDefault();let k=this.selectionStart,P=this.selectionEnd,ie=this.value,pe=ie.lastIndexOf(`
`,k-1)+1,ve=ie.substring(pe,k),ue=ve.trim()==="\u2022"||ve.trim()===""?`
`:`
\u2022 `;this.value=ie.substring(0,k)+ue+ie.substring(P),this.selectionStart=this.selectionEnd=k+ue.length}else if(L.key==="Backspace"){let k=this.selectionStart;if(k===this.selectionEnd&&k>0){let P=this.value.substring(0,k);P.endsWith(`
\u2022 `)?(L.preventDefault(),this.value=P.substring(0,k-3)+this.value.substring(this.selectionEnd),this.selectionStart=this.selectionEnd=k-3):P==="\u2022 "&&(L.preventDefault(),this.value="",this.selectionStart=this.selectionEnd=0)}}}}function ht(){let E=typeof Ae<"u"?Ae:document.getElementById("snippet-container");if(!E)return;let L=E.querySelectorAll('input[type="checkbox"]:checked, input[type="radio"]:checked'),k={},P=new Set;L.forEach(Q=>{let W=Q.id,F=Se[W];if(F)for(let N in F)N==="linkedTask"?P.add(F.linkedTask):N!=="type"&&(k[N]||(k[N]=[]),k[N].includes(F[N])||k[N].push(F[N]))});let ie=new Set;Object.values(Se).forEach(Q=>{Object.keys(Q).forEach(W=>{W!=="linkedTask"&&W!=="type"&&ie.add(W)})}),ie.forEach(Q=>{let W=document.getElementById(Q);if(W){let F=k[Q]||[],N="";ot.includes(Q.replace("field-",""))?(N=F.map(U=>U.startsWith("\u2022 ")?U:"\u2022 "+U).join(`
`),N===""?N="\u2022 ":N.endsWith(`
\u2022 `)||(N+=`
\u2022 `)):N=F.join(`

`),N.trim()!=="\u2022"&&N.trim()!==""?W.value=N:ot.includes(Q.replace("field-",""))?W.value="\u2022 ":W.value="",W.tagName==="TEXTAREA"&&typeof ft=="function"&&ft(W)}});let pe=new Set,ve=new Set;E.querySelectorAll('input[type="checkbox"], input[type="radio"]').forEach(Q=>{let W=Se[Q.id];W&&W.linkedTask&&(Q.checked?pe.add(W.linkedTask):ve.add(W.linkedTask))}),ve.forEach(Q=>{pe.has(Q)||l.toggleTask(Q,!1)}),pe.forEach(Q=>{l.toggleTask(Q,!0)})}d.onchange=()=>{let E=d.value;if(xt(1.5),y.innerHTML=`<option value="">${j("select_substatus")}</option>`,!E){y.disabled=!0;return}for(let L in tt){let k=tt[L];if(k.status===E){let P=document.createElement("option");P.value=L,P.textContent=k.name,y.appendChild(P)}}y.disabled=!1},y.onchange=()=>{let E=y.value;if(xt(1.5),!E)return;l.updateSubStatus(E);let L=tt[E];Ae.innerHTML="";let k=(F,N,U)=>{let de=document.createElement("label");Object.assign(de.style,i.checkboxLabel),de.onmouseover=()=>de.style.backgroundColor="#e8eaed",de.onmouseout=()=>de.style.backgroundColor="#f8f9fa";let ne=document.createElement("input");return ne.type=N,ne.id=F.id,Object.assign(ne.style,i.checkboxInput),de.appendChild(ne),de.appendChild(document.createTextNode(` ${F.text}`)),U.appendChild(de),ne},P=[],ie="radio";if(E==="NI_Awaiting_Inputs")P=[{id:"quickfill-ni-inicio-manual",text:"In\xEDcio 2/6 (Manual)"},{id:"quickfill-ni-cms-access",text:"In\xEDcio 2/6 (ADV sem acesso ao CMS)"},{id:"quickfill-ni-followup-bau",text:"Follow-up 2/6 (BAU)"},{id:"quickfill-ni-followup-lm",text:"Follow-up 2/6 (LM)"}];else if(E.startsWith("SO_"))ie="checkbox",P=[{id:"quickfill-gtm-install",text:"Instala\xE7\xE3o do GTM"},{id:"quickfill-whatsapp",text:"Convers\xE3o de WhatsApp"},{id:"quickfill-form",text:"Convers\xE3o de Formul\xE1rio (Padr\xE3o)"},{id:"quickfill-ecw4-close",text:"Fechamento ECW4 (P\xF3s 7 dias)"}];else if(E.startsWith("AS_")){ie="checkbox";let F=document.createElement("label");F.textContent=j("cenarios_comuns"),Object.assign(F.style,i.label),Ae.appendChild(F),P=[{id:"quickfill-as-no-show",text:"Anunciante n\xE3o compareceu (respondeu e-mail)"},{id:"quickfill-as-insufficient-time",text:"Tempo insuficiente"},{id:"quickfill-as-no-access",text:"Anunciante sem acessos necess\xE1rios"}]}else E.startsWith("IN_")?P=[{id:"quickfill-in-nrp-bau",text:"NRP (BAU - 3 tentativas)"},{id:"quickfill-in-nrp-lm",text:"NRP (LM - Sem tentativas)"},{id:"quickfill-in-no-show-bau",text:"No-Show (BAU - 3 tentativas)"},{id:"quickfill-in-2-6-final",text:"Finaliza\xE7\xE3o 2/6 (Sem Resposta)"},{id:"quickfill-in-manual",text:"Outro (Manual)"}]:E.startsWith("DC_")&&(ie="radio",P=[{id:"quickfill-dc-lm-no-access",text:"LM - Sem acessos (Reagendar em BAU)"}]);let pe=P.filter(F=>{let N=Se[F.id];return!N.type||N.type==="all"||N.type===t});pe.forEach((F,N)=>{let U=k(F,ie,Ae);ie==="radio"&&(U.name="scenario-radio-group",N===0&&(U.checked=!0))}),pe.length>0&&(Le.style.display="block"),L.requiresTasks?(ge.style.display="none",Fe.style.display="block",l.selectionElement.style.display="block"):(ge.style.display="block",Fe.style.display="none",l.selectionElement.style.display="none"),_e.innerHTML="";let ve=L.template.match(/{([A-Z0-9_]+)}/g)||[];[...new Set(ve)].forEach(F=>{if(["{TAGS_IMPLEMENTED}","{SCREENSHOTS_LIST}","{CONSENTIU_GRAVACAO}","{CASO_PORTUGAL}"].includes(F))return;let N=F.slice(1,-1),U=document.createElement("label"),de=j(N.toLowerCase());if(U.textContent=de!==N.toLowerCase()?de:N.replace(/_/g," ").replace(/\b\w/g,z=>z.toUpperCase())+":",Object.assign(U.style,i.label),N==="SPEAKEASY_ID"){let z=document.createElement("button");z.innerHTML='<span style="font-size:12px; margin-right:4px;">\u2728</span>Auto Busca',z.style.cssText=`
              font-family: 'Google Sans', Roboto, sans-serif;
              font-size: 11px;
              font-weight: 500;
          ...
          `,z.title="Localizar Speakeasy ID no hist\xF3rico",z.onmouseover=()=>{z.style.backgroundColor="#c2e7ff",z.style.boxShadow="0 1px 2px rgba(0,0,0,0.1)"},z.onmouseout=()=>{z.style.backgroundColor="#d3e3fd",z.style.boxShadow="none"},z.onmousedown=()=>{z.style.backgroundColor="#a8c7fa",z.style.transform="scale(0.96)"},z.onmouseup=()=>z.style.transform="scale(1)",z.onclick=be=>{be.preventDefault(),Qt(`field-${N}`)},U.appendChild(z)}let ne;ot.includes(N)?(ne=document.createElement("textarea"),Object.assign(ne.style,i.textarea),ne.classList.add("bullet-textarea"),ft(ne)):Tt.includes(N)?(ne=document.createElement("textarea"),Object.assign(ne.style,i.textarea)):(ne=document.createElement("input"),ne.type="text",Object.assign(ne.style,i.input),N==="REASON_COMMENTS"&&(E.startsWith("NI_")||E.startsWith("IN_"))&&(Object.assign(U.style,{display:"none"}),Object.assign(ne.style,{display:"none"}))),N==="ON_CALL"&&t==="lm"&&(Object.assign(U.style,{display:"none"}),Object.assign(ne.style,{display:"none"}),ne.value="N/A"),ne.id=`field-${N}`,_e.appendChild(U),_e.appendChild(ne)});let Q=Ae.querySelectorAll('input[type="checkbox"], input[type="radio"]');Q.length>0&&(Q.forEach(F=>{F.removeEventListener("change",ht),F.addEventListener("change",ht)}),ht()),ye.style.display="block",Pe[E]?De.style.display="block":De.style.display="none",We.style.display="flex";let W=l.getCheckedElements().map(F=>F.value);a.updateVisibility(E,W)},ge.onclick=()=>{ge.style.display="none",Fe.style.display="block",l.selectionElement.style.display="block"};function Mt(){let E=y.value;if(!E)return null;let k=tt[E].template.replace(/\n/g,"<br>"),P='style="margin-bottom: 12px; padding-left: 30px;"',ie=[],pe="",ve=l.getCheckedElements();ve.length>0&&ve.forEach(W=>{let F=W.value,N=Re[F],U=W.closest().querySelector(".stepper-count"),de=U?parseInt(U.textContent):1;de>1?ie.push(`${N.name} (x${de})`):ie.push(N.name)});let ue=l.screenshotsElement;if(ue){let W=Array.from(ue.querySelectorAll('input[id^="name-"]'));W.length>0&&W.forEach(F=>{let N=F.value,U=F.closest(".cw-screen-card");if(U){let de=U.querySelectorAll('input[id^="screen-"]'),ne=!1,z="";de.forEach(be=>{let Lt=be.closest(".cw-input-group"),Ft=Lt?Lt.querySelector(".cw-input-label"):null,Ao=Ft?Ft.textContent:"Evid\xEAncia",Dt=be.value.trim(),wo=Dt?` ${Dt}`:"";z+=`<li>${Ao} -${wo}</li>`,ne=!0}),ne&&(pe+=`<b>${N}</b>`,pe+=`<ul ${P}>${z}</ul>`)}})}if(k.includes("{TAGS_IMPLEMENTED}")?k=k.replace(/{TAGS_IMPLEMENTED}/g,ie.join(", ")||"N/A"):ie.length>0&&(k+=`<br><b>Tags:</b> ${ie.join(", ")}<br>`),k.includes("{SCREENSHOTS_LIST}")?k=k.replace(/{SCREENSHOTS_LIST}/g,pe?`${pe}`:"N/A"):pe!==""&&(k+=`<br>${pe}`),n==="pt"&&o){let W=xe.checked?j("sim"):j("nao");k=k.replace(/{CONSENTIU_GRAVACAO}/g,`<br><b>${j("consentiu_gravacao")}</b> ${W}<br><br>`),k=k.replace(/{CASO_PORTUGAL}/g,`<br><b>${j("caso_portugal")}</b> ${j("sim")}<br>`)}else n==="pt"&&!o?(k=k.replace(/{CASO_PORTUGAL}/g,`<br><b>${j("caso_portugal")}</b> ${j("nao")}<br>`),k=k.replace(/{CONSENTIU_GRAVACAO}/g,"")):(k=k.replace(/{CASO_PORTUGAL}/g,""),k=k.replace(/{CONSENTIU_GRAVACAO}/g,""));return _e.querySelectorAll("input, textarea").forEach(W=>{let F=W.id.replace("field-",""),N=new RegExp(`{${F}}`,"g"),U=W.value;if(F==="REASON_COMMENTS"&&(E.startsWith("NI_")||E.startsWith("IN_"))){let z=Ae.querySelector('input[type="radio"]:checked');z&&Se[z.id]&&(U=Se[z.id]["field-REASON_COMMENTS"])}if(ot.includes(F)&&U.trim()!==""){let z=U.split(`
`).map(be=>be.trim()).filter(be=>be!==""&&be!=="\u2022").map(be=>be.startsWith("\u2022 ")?be.substring(2):be).map(be=>`<li>${be}</li>`).join("");U=z?`<ul ${P}>${z}</ul>`:""}else Tt.includes(F)?U=U.split(`
`).filter(z=>z.trim()!=="").map(z=>`<p style="margin: 0 0 8px 0;">${z}</p>`).join(""):W.tagName==="TEXTAREA"&&(U=U.replace(/\n/g,"<br>"));let de=U.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(de===""||de==="\u2022"||de.toLowerCase()==="n/a"){let z=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${F}\\}(?:<br>\\s*)?`,"gi");z.test(k)?k=k.replace(z,""):k=k.replace(N,"")}else k=k.replace(N,U.replace(/\$/g,"$$$$"))}),k=k.replace(/{([A-Z0-9_]+)}/g,""),k=k.replace(/(<br>){3,}/g,"<br><br>"),typeof a<"u"&&a.getOutput&&(k+=a.getOutput()),k}Ke.onclick=()=>{let E=Mt();E?(It(E),ae(j("copiado_sucesso"))):ae(j("selecione_substatus"),{error:!0})},Qe.onclick=async()=>{let E=y.value,L=Mt();if(!L){ae(j("selecione_substatus"),{error:!0});return}It(L),yt();let k=dt(),P=await po();if(P)try{if(P.focus(),P.innerHTML.trim()==="<p><br></p>"||P.innerHTML.trim()==="<br>"||P.innerText.trim()===""){let ue=document.createRange();ue.selectNodeContents(P);let Q=window.getSelection();Q.removeAllRanges(),Q.addRange(ue),document.execCommand("delete",!1,null)}else if(!P.innerHTML.endsWith("<br><br>")){let ue=document.createRange();ue.selectNodeContents(P),ue.collapse(!1);let Q=window.getSelection();Q.removeAllRanges(),Q.addRange(ue),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,L),co(P),setTimeout(()=>{ae(j("inserido_copiado"))},600);let pe=typeof ze<"u"&&ze?ze.checked:!0;if(E&&Pe[E]&&pe){let ue=Pe[E];await lt(ue),await new Promise(Q=>setTimeout(Q,500))}k(),xt(1.5),d.value="",y.innerHTML=`<option value="">${j("select_substatus")}</option>`,y.disabled=!0}catch(ie){console.error(ie),ae("Erro ao inserir.",{error:!0}),k()}};function xt(E=1.5){E<=1.5&&(Le.style.display="none",Ae.innerHTML=""),E<=2&&(l.reset(),ge.style.display="none"),E<=3&&(ye.style.display="none",_e.innerHTML="",a.reset(),We.style.display="none",De.style.display="none")}function yt(){if(s=!s,s){let E=m.querySelector(".cw-expand-btn");E&&typeof E.resetState=="function"&&E.resetState()}Te(s,m,"cw-btn-notes")}return gt("bau"),bt("pt"),yt}var $e={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
                    <p>Ol\xE1,</p>
                    <br>
            ...
            <div style="padding:24px;">
                ...
                <div>
                    ...
                    <div style="font-size:14px; color:#3c4043; line-height:1.6; padding:0 4px;">${I.body.replace(/\n/g,"<br>")}</div>
                </div>
            </div>
        ...
        `;let M=y.querySelector("#csa-back-btn");M.onmouseenter=()=>M.style.background="#f1f3f4",M.onmouseleave=()=>M.style.background="none",M.onclick=Z;let $=y.querySelector("#csa-insert-btn");$.onmouseenter=()=>{$.style.transform="translateY(-1px)",$.style.boxShadow="0 6px 16px rgba(26,115,232,0.4)"},$.onmouseleave=()=>{$.style.transform="translateY(0)",$.style.boxShadow="0 4px 12px rgba(26,115,232,0.3)"},$.onclick=()=>{$.style.transform="scale(0.96)",O(I,"email"),setTimeout(()=>{$.style.transform="scale(1)",Z()},300)}}function Z(){s="list",w.style.transform="translateX(0)"}function G(I,Y,re=null){let M=document.createElement("button"),$=re?`<span style="margin-right:6px; font-size:14px; opacity:0.9;">${re}</span>`:"";return M.innerHTML=`${$}${I}`,Object.assign(M.style,A),n===Y&&o===""?Object.assign(M.style,x):(M.onmouseenter=()=>{M.style.background="#F1F3F4",M.style.borderColor="#DADCE0"},M.onmouseleave=()=>{M.style.background="#FFFFFF",M.style.borderColor="#DADCE0"}),M.onclick=()=>{n=Y,o="",d.value="",B(),J()},M}function B(){p.innerHTML="",p.appendChild(G("Smart CRs",t,"\u26A1")),Object.keys($e).forEach(I=>{p.appendChild(G($e[I].title,I))})}function J(){h.innerHTML="";let I=[];if(o.trim()!==""){let te=o.toLowerCase();Object.values($e).forEach(_=>{_.emails.forEach(X=>{(X.name.toLowerCase().includes(te)||X.subject.toLowerCase().includes(te))&&I.push({type:"email",data:X})})}),Object.entries(Pe).forEach(([_,X])=>{if(!X)return;(_.replace(/_/g," ").toLowerCase().includes(te)||X.toLowerCase().includes(te))&&I.push({type:"cr",key:_,code:X})})}else n===t?Object.entries(Pe).forEach(([te,_])=>{_&&I.push({type:"cr",key:te,code:_})}):$e[n]&&$e[n].emails.forEach(te=>{I.push({type:"email",data:te})});if(I.length===0){h.innerHTML=`
                <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:60px 20px; color:#9AA0A6;">
                    ...
                    <div style="flex-grow:1; min-width:0; display:flex; flex-direction:column; gap:2px;">
                        <div style="font-size:14px; font-weight:600; color:#202124; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${X.name}</div>
                ...
                `,_.onclick=()=>se(X)}else{let X=te.key.replace(/_/g," ").replace("AS ","AS - ").replace("NI ","NI - ");_.innerHTML=`
                    ...
                    <div style="flex-grow:1; min-width:0; display:flex; flex-direction:column; gap:2px;">
                        <div style="font-size:14px; font-weight:600; color:#202124; margin-bottom:0; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${X}</div>
 ...
 D) Nombre del sitio web.`,"autenticar la cuenta del anunciante en el cases, si corresponde.","T\xE9rminos y condi\xE7\xF5es.","Informar las Task solicitadas y AM.","Cerrar contenido sensible.","Confirmaci\xF3n de copia de seguridad y acceso de administrador a las ferramentas.","Resumen de llamada."],fim:["Ofrecer ayuda adicional.","Dejar de compartir la pantalla.","Pasos siguientes  (Si se le har\xE1 seguimiento al caso).","Encuesta de Satisfa\xE7\xE3o.","Informar al cliente que el equipo de QA ir\xE1 a realizar pruebas en los siguientes dias."]},"EN BAU":{color:"#ff0011",inicio:["Example 1","Example 2"],fim:["Example 3","Example 4"]}};function ho(){let e="v2.1 (Apple Motion)",t={progressBarContainer:{height:"4px",background:"#f1f3f4",width:"100%",position:"relative",overflow:"hidden"},progressBarFill:{height:"100%",background:"linear-gradient(90deg, #4285F4, #34A853)",width:"0%",transition:"width 0.5s cubic-bezier(0.25, 0.8, 0.25, 1)",borderRadius:"0 2px 2px 0"},contentArea:{padding:"16px",overflowY:"auto",flexGrow:"1",background:"#f8f9fa",scrollBehavior:"smooth"},card:{background:"#ffffff",border:"1px solid #dadce0",borderRadius:"12px",padding:"16px",marginBottom:"16px",transition:"transform 0.2s ease, box-shadow 0.2s ease",boxShadow:"0 1px 2px rgba(0,0,0,0.02)"},cardTitle:{fontSize:"13px",fontWeight:"700",color:"#5f6368",textTransform:"uppercase",letterSpacing:"0.6px",marginBottom:"12px",display:"flex",alignItems:"center",justifyContent:"space-between",userSelect:"none"},itemRow:{display:"flex",alignItems:"flex-start",padding:"10px 8px",cursor:"pointer",borderRadius:"8px",transition:"background-color 0.15s ease, opacity 0.3s ease",color:"#202124",fontSize:"14px",lineHeight:"1.5",marginBottom:"2px"},itemCompleted:{opacity:"0.5",textDecoration:"line-through",color:"#5f6368"},checkbox:{minWidth:"20px",height:"20px",borderRadius:"6px",border:"2px solid #dadce0",marginRight:"14px",marginTop:"1px",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)",background:"#fff"},footer:{padding:"12px 16px",borderTop:"1px solid #eee",background:"#fff",display:"flex",justifyContent:"space-between",alignItems:"center"},resetBtn:{background:"transparent",border:"none",color:"#d93025",fontSize:"12px",fontWeight:"600",cursor:"pointer",padding:"6px 12px",borderRadius:"20px",transition:"background 0.2s ease",display:"flex",alignItems:"center",gap:"4px"}},n={},o="PT",s="BAU",i=!1,a=document.createElement("div");a.id="call-script-popup",a.classList.add("cw-module-window"),Object.assign(a.style,we,{right:"auto",left:"50%",width:"400px",height:"650px",display:"flex",flexDirection:"column",transform:"translateX(-50%) scale(0.05)"});let l={popup:a,googleLine:null};function m(){i=!i,Te(i,a,"cw-btn-script")}let b=Ee(a,"Call Script",e,"Guia interativo para condu\xE7\xE3o de chamadas.",l,()=>{m()});a.appendChild(b);let g=document.createElement("div");Object.assign(g.style,t.progressBarContainer);let c=document.createElement("div");Object.assign(c.style,t.progressBarFill),g.appendChild(c),a.appendChild(g);let f=document.createElement("div");f.id="csa-content",Object.assign(f.style,t.contentArea),a.appendChild(f);let A=document.createElement("div");Object.assign(A.style,t.footer);let x=document.createElement("span");x.textContent="by lucaste@",Object.assign(x.style,{fontSize:"10px",color:"#bdc1c6"});let T=document.createElement("button");T.innerHTML=`
    ...
    Resetar Script
  `,Object.assign(T.style,t.resetBtn),T.onmouseenter=()=>T.style.background="#fce8e6",T.onmouseleave=()=>T.style.background="transparent",T.onclick=()=>{T.style.transform="scale(0.9)",setTimeout(()=>T.style.transform="scale(1)",150);for(let C in n)delete n[C];u()},A.appendChild(x),A.appendChild(T),a.appendChild(A);let S=document.createElement("div");Object.assign(S.style,{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px",gap:"8px"});let q=document.createElement("div");Object.assign(q.style,{display:"flex",borderRadius:"8px",border:"1px solid #dadce0",overflow:"hidden",background:"#fff"});let R=document.createElement("div");R.textContent="BAU";let V=document.createElement("div");V.textContent="LT",Object.assign(R.style,fe),Object.assign(V.style,fe),q.appendChild(R),q.appendChild(V);let K=document.createElement("select");Object.assign(K.style,Ut,{marginBottom:"0",width:"auto",minWidth:"90px",paddingTop:"6px",paddingBottom:"6px",paddingRight:"30px",height:"32px",backgroundPosition:"right 8px center"}),K.innerHTML='<option value="PT">PT</option><option value="ES">ES</option><option value="EN">EN</option>',K.value=o,S.appendChild(q),S.appendChild(K),f.appendChild(S);let v=document.createElement("div");v.id="csa-checklist-area",f.appendChild(v);let w=document.createElement("div");Object.assign(w.style,Ye),w.className="no-drag",w.title="Redimensionar",a.appendChild(w),Xe(a,w),document.body.appendChild(a);function r(C){return C}function u(){v.innerHTML="";let C=`${o} ${s}`,y=fo[C];if(!y){v.innerHTML=`<div style="padding: 30px; text-align: center; color: #bdc1c6; display: flex; flex-direction: column; align-items: center; gap: 10px;">
        <div style="font-size: 24px;">\u2615</div>
        <div>Script n\xE3o configurado.</div>
      </div>`,c.style.width="0%";return}let O=y.color||"#1a73e8",se=0,Z=0;["inicio","fim"].forEach(G=>{y[G]&&(se+=y[G].length)}),["inicio","fim"].forEach((G,B)=>{let J=y[G];if(!J||J.length===0)return;let I=document.createElement("div");Object.assign(I.style,t.card);let Y=document.createElement("div");Object.assign(Y.style,t.cardTitle);let re=G==="inicio"?"Abertura":"Fechamento";o.includes("ES")&&(re=G==="inicio"?"Apertura":"Cierre"),o.includes("EN")&&(re=G==="inicio"?"Opening":"Closing"),Y.textContent=re;let M=document.createElement("span");M.style.fontSize="11px",M.style.opacity="0.7",M.style.fontWeight="500",M.style.background="#f1f3f4",M.style.padding="2px 8px",M.style.borderRadius="10px",Y.appendChild(M),I.appendChild(Y);let $=0;J.forEach((te,_)=>{let X=`${C}-${G}-${_}`,xe=!!n[X];xe&&(Z++,$++);let ce=document.createElement("div");Object.assign(ce.style,t.itemRow);let oe=document.createElement("div");Object.assign(oe.style,t.checkbox);let Oe=document.createElement("span");Oe.innerHTML=te,Oe.style.flex="1",xe?(Object.assign(ce.style,t.itemCompleted),oe.style.background=O,oe.style.borderColor=O,oe.style.transform="scale(1)",oe.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>'):(ce.style.textDecoration="none",ce.style.opacity="1",oe.style.background="transparent",oe.style.borderColor="#dadce0",oe.style.transform="scale(1)",oe.innerHTML=""),ce.onclick=()=>{let Me=!n[X];n[X]=Me,ee.playClick(),Me?(oe.style.transform="scale(1.2)",setTimeout(()=>oe.style.transform="scale(1)",150),Object.assign(ce.style,t.itemCompleted),oe.style.background=O,oe.style.borderColor=O,oe.innerHTML='<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>'):(ce.style.textDecoration="none",ce.style.opacity="1",oe.style.background="transparent",oe.style.borderColor="#dadce0",oe.innerHTML=""),d(C,y)},ce.onmouseenter=()=>{n[X]||(ce.style.background="#f1f3f4",oe.style.borderColor=O)},ce.onmouseleave=()=>{n[X]||(ce.style.background="transparent",oe.style.borderColor="#dadce0")},ce.appendChild(oe),ce.appendChild(Oe),I.appendChild(ce)}),$===J.length&&J.length>0&&(M.style.color="#1e8e3e",M.style.background="#e6f4ea",I.style.boxShadow="inset 4px 0 0 #1e8e3e, 0 1px 3px rgba(0,0,0,0.05)"),M.textContent=`${$}/${J.length}`,v.appendChild(I)}),p(se,Z)}function d(C,y){let O=0,se=0;["inicio","fim"].forEach(Z=>{let G=y[Z]||[];O+=G.length;let B=0;G.forEach((J,I)=>{n[`${C}-${Z}-${I}`]&&(se++,B++)})}),p(O,se),setTimeout(()=>u(),200)}function p(C,y){let O=C===0?0:y/C*100;c.style.width=`${O}%`,O===100?c.style.background="#34A853":c.style.background="linear-gradient(90deg, #4285F4, #34A853)"}function h(C){s=C;let y=Je();Object.assign(R.style,fe),Object.assign(V.style,fe),Object.assign(C==="BAU"?R.style:V.style,y),u()}return R.onclick=()=>h("BAU"),V.onclick=()=>h("LT"),K.addEventListener("change",C=>{o=C.target.value,u()}),h(s),m}var ut={lm:{label:"LM Forms",links:[{name:"Relat\xF3rio de Ocorr\xEAncias",url:"https://docs.google.com/forms/d/e/1FAIpQLSc6CamPehrREeVr7yCWMyqFETrFYYezNcLb_13W4yZDQkfY6Q/viewform",desc:"Reportar problemas operacionais | Aviso de pausas"},{name:"Chamadas Excedidas (>50min)",url:"https://docs.google.com/forms/d/e/1FAIpQLSfE8EMHNJMTKYeA6XM2RZjZ9AQ4LhGk1Dwm_WLu3kcMdKMikA/viewform",desc:"Registro de chamadas longas"},{name:"Relat\xF3rio de Bugs",url:"https://docs.google.com/forms/d/e/1FAIpQLSfkqRqT2Kbf08IStz31fQPE84MDOtGxk7cetJmc3xzShXIXRA/viewform",desc:"Erros de sistema/ferramenta"},{name:"Suporte LM",url:"https://script.google.com/a/macros/google.com/s/AKfycbxYMlFCMZvqgHMIImeS_u-lNZPiertXmem-5m9Fox3jvZaq0ZOQDoc5ma96ltSvWHY/exec",desc:"Enviar casos para BAU/Solicitar Descarte/Abrir Monitoria para AMs"}]},qa:{label:"QA",links:[{name:"Elogios",url:"https://docs.google.com/forms/d/e/1FAIpQLSezY5K-trQDv0LkL5IoTlV0Tl0oOqGTEszylmgcbMRXcC9Weg/viewform",desc:"Feedback positivo dos Anunciantes"},{name:"Casos Complexos",url:"https://docs.google.com/forms/d/e/1FAIpQLSe26q1LEloFNRfOAVZtA7DCOQTqdu1BAEeWuxtK6oPwZhLp-A/viewform?resourcekey=0-c1N4h8gntza2gQowqYAqMw",desc:"Casos complicados de atender"}]},suporte:{label:"Central de Ajuda",links:[{name:"Suporte Google Ads",url:"https://support.google.com/google-ads/",desc:"Oficial"},{name:"Suporte GA4",url:"https://support.google.com/analytics/",desc:"Oficial"},{name:"Suporte Merchant Center",url:"https://support.google.com/merchants/gethelp",desc:"Oficial"},{name:"Doc. CSP",url:"https://developers.google.com/tag-platform/tag-manager/web/csp?hl=pt-br.",desc:"Doc. oficial sobre CSP"},{name:"Doc. Enhanced Conversion",url:"https://support.google.com/google-ads/answer/9888656?hl=en",desc:"Como funcionam as convers\xF5es otimizadas?"},{name:"Doc. CoMo",url:"https://developers.google.com/tag-platform/security/concepts/consent-mode?hl=pt-br",desc:"Doc. oficial sobre Consent Mode"},{name:"Cursos SkillShop",url:"https://skillshop.withgoogle.com/intl/pt-BR_ALL/",desc:"Cursos sobre as ferramentas do Google."}]},outros:{label:"Diversos",links:[{name:"Solicitar Grava\xE7\xE3o",url:"https://support.google.com/policies/contact/sar",desc:"Form para solicitar grava\xE7\xE3o da chamada."},{name:"Escala\xE7\xE3o de Sellers",url:"https://forms.gle/HWMhML56eE4CPZCs5",desc:"Form para escala\xE7\xE3o de Sellers, compartilhado pelo gpozzi@."},{name:"SOP Split & Transfer",url:"https://sites.google.com/corp/google.com/technicalsolutions/case-handling_1/out-of-scope?authuser=0#h.oq8p11yo96dw",desc:"Doc. com instru\xE7\xE3o para Split de casos."}]}};function xo(){let e="v2.5.0 HD",t="lm",n="",o={bgApp:"#F8F9FA",bgSurface:"#FFFFFF",borderSubtle:"rgba(0, 0, 0, 0.08)",textPrimary:"#202124",textSecondary:"#5F6368",primary:"#1A73E8",primaryBg:"#E8F0FE",shadowCard:"0 1px 3px rgba(0,0,0,0.06), 0 1px 2px rgba(0,0,0,0.04)",shadowHover:"0 4px 12px rgba(0,0,0,0.08), 0 2px 4px rgba(0,0,0,0.04)",transition:"all 0.2s cubic-bezier(0.2, 0.0, 0.2, 1)"},s={width:"100%",height:"40px",padding:"0 12px 0 40px",borderRadius:"10px",border:"1px solid transparent",background:"#FFFFFF",fontSize:"14px",color:o.textPrimary,boxSizing:"border-box",outline:"none",transition:o.transition,boxShadow:"0 2px 5px rgba(0,0,0,0.03)",backgroundImage:`url('data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="%239AA0A6" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>')`,backgroundRepeat:"no-repeat",backgroundPosition:"12px center"},i={display:"flex",flexWrap:"wrap",justifyContent:"center",gap:"8px",padding:"4px 0 12px 0"},a={padding:"6px 14px",borderRadius:"100px",border:"1px solid #DADCE0",background:"#FFFFFF",color:o.textSecondary,fontSize:"13px",fontWeight:"500",cursor:"pointer",whiteSpace:"nowrap",transition:o.transition,display:"inline-flex",alignItems:"center",justifyContent:"center",marginBottom:"0"},l={background:o.primaryBg,color:o.primary,borderColor:"transparent",fontWeight:"600",boxShadow:"0 1px 2px rgba(26, 115, 232, 0.15)"},m={display:"flex",alignItems:"center",padding:"12px 16px",marginBottom:"8px",borderRadius:"12px",background:o.bgSurface,border:"1px solid transparent",boxShadow:o.shadowCard,cursor:"pointer",transition:"transform 0.25s cubic-bezier(0.2, 0.8, 0.2, 1), box-shadow 0.2s, border-color 0.2s",opacity:"0",transform:"translateY(10px)"},b={width:"36px",height:"36px",borderRadius:"10px",background:"#F1F3F4",color:o.textSecondary,display:"flex",alignItems:"center",justifyContent:"center",marginRight:"16px",flexShrink:"0",transition:"background 0.2s, color 0.2s"},g=document.createElement("div");g.id="feedback-popup",g.classList.add("cw-module-window"),Object.assign(g.style,we,{right:"100px",width:"440px",height:"600px",background:o.bgApp});let c={lm:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M19 3h-4.18C14.4 1.84 13.3 1 12 1c-1.3 0-2.4.84-2.82 2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-7 0c.55 0 1 .45 1 1s-.45 1-1 1-1-.45-1-1 .45-1 1-1zm2 14H7v-2h7v2zm3-4H7v-2h10v2zm0-4H7V7h10v2z"/></svg>',qa:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm-2 16l-4-4 1.41-1.41L10 14.17l6.59-6.59L18 9l-8 8z"/></svg>',suporte:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M21 5c-1.11-.35-2.33-.5-3.5-.5-1.95 0-4.05.4-5.5 1.5-1.45-1.1-3.55-1.5-5.5-1.5S2.45 4.9 1 6v14.65c0 .25.25.5.5.5.1 0 .15-.05.25-.05C3.1 20.45 5.05 20 6.5 20c1.95 0 4.05.4 5.5 1.5 1.35-.85 3.8-1.5 5.5-1.5 1.65 0 3.35.3 4.75 1.05.1.05.15.05.25.05.25 0 .5-.25.5-.5V6c-.6-.45-1.25-.75-2-1zm0 13.5c-1.1-.35-2.3-.5-3.5-.5-1.7 0-4.15.65-5.5 1.5V8c1.35-.85 3.8-1.5 5.5-1.5 1.2 0 2.4.15 3.5.5v11.5z"/></svg>',outros:'<svg width="18" height="18" viewBox="0 0 24 24" fill="currentColor"><path d="M20.5 11H19V7c0-1.1-.9-2-2-2h-4V3.5C13 2.12 11.88 1 10.5 1S8 2.12 8 3.5V5H4c-1.1 0-1.99.9-1.99 2v3.8H3.5c-1.49 0-2.61 1.12-2.61 2.5s1.12 2.5 2.61 2.5H2v4c0 1.1.9 2 2 2h4v1.5c0 1.49 1.12 2.61 2.5 2.61s2.5-1.12 2.5-2.61V19h4c1.1 0 2-.9 2-2v-4h1.5c1.49 0 2.61-1.12 2.61-2.5S21.99 11 20.5 11z"/></svg>'},f={popup:g,googleLine:null,focusElement:null},A=!1,x=Ee(g,"Links \xDAteis",e,"Acesso r\xE1pido a formul\xE1rios internos e documenta\xE7\xE3o.",f,()=>w());g.appendChild(x);let T=document.createElement("div");Object.assign(T.style,{padding:"20px 24px 12px 24px",display:"flex",flexDirection:"column",gap:"16px",borderBottom:`1px solid ${o.borderSubtle}`,flexShrink:"0",backgroundColor:o.bgApp});let S=document.createElement("input");S.type="text",S.placeholder="Pesquisar formul\xE1rios e links...",Object.assign(S.style,s),f.focusElement=S,S.onfocus=()=>{S.style.borderColor=o.primary,S.style.backgroundColor="#fff",S.style.boxShadow="0 0 0 4px rgba(26, 115, 232, 0.15)"},S.onblur=()=>{S.style.borderColor="transparent",S.style.backgroundColor="#fff",S.style.boxShadow="0 2px 5px rgba(0,0,0,0.03)"};let q=document.createElement("div");Object.assign(q.style,i),T.appendChild(S),T.appendChild(q),g.appendChild(T);let R=document.createElement("div");Object.assign(R.style,{padding:"16px 24px",overflowY:"auto",flexGrow:"1",backgroundColor:o.bgApp}),g.appendChild(R);let V=document.createElement("div");Object.assign(V.style,{padding:"12px 24px",borderTop:`1px solid ${o.borderSubtle}`,fontSize:"11px",color:o.textSecondary,textAlign:"right",background:o.bgApp}),V.innerHTML="created by lucaste@",g.appendChild(V),document.body.appendChild(g);function K(){q.innerHTML="",Object.keys(ut).forEach(r=>{let u=ut[r],d=document.createElement("button"),p=c[r]||"";d.innerHTML=`
        <span style="display:inline-flex; align-items:center; margin-right:6px; opacity:0.9;">
            ${p}
        </span> 
        ${u.label}
      `,Object.assign(d.style,a),t===r&&n===""?Object.assign(d.style,l):(d.onmouseenter=()=>{d.style.background="#F1F3F4",d.style.borderColor="#DADCE0"},d.onmouseleave=()=>{d.style.background="#FFFFFF",d.style.borderColor="#DADCE0"}),d.onclick=()=>{t=r,n="",S.value="",K(),v()},q.appendChild(d)})}function v(){R.innerHTML="";let r=[],u=n.trim()!=="";if(u?Object.entries(ut).forEach(([d,p])=>{let h=p.links.filter(C=>C.name.toLowerCase().includes(n.toLowerCase())||C.desc.toLowerCase().includes(n.toLowerCase()));h.forEach(C=>{C._catIcon=c[d],C._categoryName=p.label}),r=[...r,...h]}):(r=ut[t].links,r.forEach(d=>d._catIcon=c[t])),r.length===0){R.innerHTML=`
        <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:60px 20px; color:#9AA0A6;">
            <div style="font-size:32px; margin-bottom:12px; opacity:0.5;">\u{1F50D}</div>
            <div style="font-size:14px; font-weight:500;">Nenhum link encontrado</div>
        </div>`;return}r.forEach((d,p)=>{let h=document.createElement("div");Object.assign(h.style,m);let C=document.createElement("div");Object.assign(C.style,b),C.innerHTML=d._catIcon||'<svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor"><path d="M3.9 12c0-1.71 1.39-3.1 3.1-3.1h4V7H7c-2.76 0-5 2.24-5 5s2.24 5 5 5h4v-1.9H7c-1.71 0-3.1-1.39-3.1-3.1zM8 13h8v-2H8v2zm9-6h-4v1.9h4c1.71 0 3.1 1.39 3.1 3.1s-1.39 3.1-3.1 3.1h-4V17h4c2.76 0 5-2.24 5-5s-2.24-5-5-5z"/></svg>',h.appendChild(C);let y=document.createElement("div");y.style.flexGrow="1",y.style.display="flex",y.style.flexDirection="column",y.style.gap="2px";let O=I=>{if(!u)return I;let Y=new RegExp(`(${n})`,"gi");return I.replace(Y,'<span style="color:#1a73e8; font-weight:700;">$1</span>')},se=`<div style="font-size:14px; font-weight:600; color:${o.textPrimary};">${O(d.name)}</div>`,Z=`<div style="font-size:12px; color:${o.textSecondary}; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">${O(d.desc)}</div>`;y.innerHTML=se+Z,h.appendChild(y);let G=document.createElement("div");G.style.display="flex",G.style.alignItems="center",G.style.gap="8px",G.style.opacity="0.4",G.style.transition="opacity 0.2s";let B=document.createElement("div");B.title="Copiar Link",B.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>',Object.assign(B.style,{width:"32px",height:"32px",borderRadius:"8px",display:"flex",alignItems:"center",justifyContent:"center",color:o.textSecondary,cursor:"pointer",transition:"all 0.2s ease"}),B.onclick=I=>{ee.playClick(),I.stopPropagation(),navigator.clipboard.writeText(d.url),B.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>',B.style.color="#188038",B.style.background="#E6F4EA",setTimeout(()=>{B.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path></svg>',B.style.color=o.textSecondary,B.style.background="transparent"},1500)},B.onmouseenter=()=>{B.style.background="#F1F3F4"},B.onmouseleave=()=>{B.style.background="transparent"},G.appendChild(B);let J=document.createElement("div");J.innerHTML='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"></polyline></svg>',Object.assign(J.style,{display:"flex",alignItems:"center",justifyContent:"center",color:"#DADCE0"}),G.appendChild(J),h.appendChild(G),h.onclick=()=>window.open(d.url,"_blank"),h.onmouseenter=()=>{h.style.transform="translateY(-2px)",h.style.boxShadow=o.shadowHover,G.style.opacity="1",C.style.background="#E8F0FE",C.style.color="#1967D2",J.style.color="#1A73E8"},h.onmouseleave=()=>{h.style.transform="translateY(0)",h.style.boxShadow=o.shadowCard,G.style.opacity="0.4",C.style.background="#F1F3F4",C.style.color=o.textSecondary,J.style.color="#DADCE0"},R.appendChild(h),requestAnimationFrame(()=>{setTimeout(()=>{h.style.opacity="1",h.style.transform="translateY(0)"},p*30)})})}S.addEventListener("input",r=>{n=r.target.value,n!==""?Array.from(q.children).forEach(u=>{Object.assign(u.style,a),u.style.opacity="0.6"}):K(),v()});function w(){A=!A,Te(A,g,"cw-btn-links")}return K(),v(),w}var Ue=[];function _t(e){Ue=e}function yo(){let e="v2.5 (Emoji Fix)",t=!1,n=null,o=60*1e3;function s(r){if(!r)return"";try{let u=new Date(r);return isNaN(u.getTime())?String(r):u.toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"}).replace(","," \xE0s")}catch{return String(r)}}let i={critical:{bg:"#FEF2F2",border:"#FECACA",text:"#991B1B",icon:'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path><line x1="12" y1="9" x2="12" y2="13"></line><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>'},info:{bg:"#EFF6FF",border:"#BFDBFE",text:"#1E40AF",icon:'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"></circle><line x1="12" y1="16" x2="12" y2="12"></line><line x1="12" y1="8" x2="12.01" y2="8"></line></svg>'},success:{bg:"#F0FDF4",border:"#BBF7D0",text:"#166534",icon:'<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>'}},a={feedContainer:{padding:"24px",overflowY:"auto",flexGrow:"1",background:"#FAFAFA",display:"flex",flexDirection:"column",gap:"20px"},card:{background:"#FFFFFF",borderRadius:"16px",border:"1px solid rgba(0,0,0,0.06)",boxShadow:"0 4px 12px rgba(0,0,0,0.05), 0 1px 2px rgba(0,0,0,0.02)",overflow:"hidden",transition:"all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1)",position:"relative",width:"100%",boxSizing:"border-box",flexShrink:"0"},cardHistory:{background:"#FFFFFF",borderRadius:"16px",border:"1px solid rgba(0,0,0,0.04)",boxShadow:"none",opacity:"0.8",filter:"grayscale(0.3)",marginBottom:"16px",flexShrink:"0"},cardHeader:{padding:"12px 20px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:"1px solid rgba(0,0,0,0.04)",fontSize:"12px",fontWeight:"600",letterSpacing:"0.5px",textTransform:"uppercase"},msgTitle:{padding:"20px 20px 8px 20px",fontSize:"16px",fontWeight:"700",color:"#202124",letterSpacing:"-0.01em",lineHeight:"1.4"},metaContainer:{padding:"0 20px 12px 20px",display:"flex",alignItems:"center",gap:"8px",fontSize:"12px",color:"#5f6368"},cardBody:{padding:"0 20px 24px 20px",fontSize:"14px",color:"#3c4043",lineHeight:"1.6",whiteSpace:"pre-wrap",fontFamily:"'Google Sans', Roboto, sans-serif",wordBreak:"break-word",overflowWrap:"break-word"},emojiImg:"height: 20px; vertical-align: text-bottom; margin: 0 2px;",dismissBtn:{width:"28px",height:"28px",borderRadius:"50%",border:"1px solid rgba(0,0,0,0.1)",background:"#fff",color:"#5f6368",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s ease",marginLeft:"12px"},markAllBtn:{fontSize:"12px",color:"#1a73e8",cursor:"pointer",fontWeight:"600",background:"transparent",border:"none",padding:"8px",transition:"opacity 0.2s"},emptyState:{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 0",color:"#bdc1c6",gap:"16px",textAlign:"center"},historyDivider:{display:"flex",alignItems:"center",justifyContent:"center",margin:"10px 0 20px 0",cursor:"pointer",color:"#1a73e8",fontSize:"13px",fontWeight:"500",gap:"8px",padding:"10px",borderRadius:"8px",transition:"background 0.2s"},historyContainer:{display:"none",flexDirection:"column",gap:"16px",paddingTop:"10px",borderTop:"1px dashed rgba(0,0,0,0.1)"}},l="cw-scrollbar-style";if(!document.getElementById(l)){let r=document.createElement("style");r.id=l,r.innerHTML=".cw-nice-scroll::-webkit-scrollbar { width: 5px; } .cw-nice-scroll::-webkit-scrollbar-track { background: transparent; } .cw-nice-scroll::-webkit-scrollbar-thumb { background-color: rgba(0,0,0,0.1); border-radius: 10px; }",document.head.appendChild(r)}function m(r){if(!r||typeof r!="string")return"";let u=r,d=/(https?:\/\/[^\s]+)|(www\.[^\s]+)/g;return u=u.replace(d,p=>{let h=p;return h.startsWith("http")||(h="http://"+h),`<a href="${h}" target="_blank" style="color:#1967d2; text-decoration:underline;">${p}</a>`}),u=u.replace(/\*\*(.*?)\*\*/g,"<b>$1</b>"),u=u.replace(/_(.*?)_/g,"<i>$1</i>"),u=u.replace(/\n/g,"<br>"),u=Yt(u),u=u.replace(/@todos|@all/gi,'<span style="background:#e8f0fe; color:#1967d2; padding:1px 5px; border-radius:4px; font-weight:600; font-size:12px;">@todos</span>'),u}let b=document.createElement("div");b.id="broadcast-popup",b.classList.add("cw-module-window"),Object.assign(b.style,we,{right:"auto",left:"50%",width:"450px",height:"650px",display:"flex",flexDirection:"column",transform:"translateX(-50%) scale(0.05)"});let g={popup:b,googleLine:null};function c(){if(t=!t,Te(t,b,"cw-btn-broadcast"),t){let r=document.getElementById("cw-btn-broadcast");r&&r.classList.remove("has-new"),T()}}let f=Ee(b,"Operations Feed",e,"Atualiza\xE7\xF5es oficiais da opera\xE7\xE3o.",g,()=>c()),A=f.querySelector(".cw-header-actions")||f.lastElementChild;if(A){let r=document.createElement("button");r.textContent="Limpar tudo",Object.assign(r.style,a.markAllBtn),r.onclick=u=>{u.stopPropagation(),ee.playSuccess();let d=Ue.map(p=>p.id);localStorage.setItem("cw_read_broadcasts",JSON.stringify(d)),R(),S()},A.insertBefore(r,A.firstChild)}b.appendChild(f);let x=document.createElement("div");x.className="cw-nice-scroll",Object.assign(x.style,a.feedContainer),b.appendChild(x);async function T(){let r=document.getElementById("cw-update-status");t&&(r||(r=document.createElement("div"),r.id="cw-update-status",r.style.cssText="padding: 6px; text-align: center; font-size: 11px; color: #5f6368; background: #f8f9fa; border-bottom: 1px solid #e0e0e0;",x.parentNode.insertBefore(r,x)),r.innerHTML="\u23F3 Verificando atualiza\xE7\xF5es...",r.style.display="block");let u=Ue.map(p=>p.id),d=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]");try{let p=await He.fetchData();p&&p.broadcast&&(t&&r&&(p.broadcast.some(C=>!u.includes(C.id))?(r.innerHTML="\u2705 Novos avisos sincronizados!",r.style.backgroundColor="#e6f4ea",r.style.color="#137333"):r.innerHTML="\u{1F539} Tudo atualizado.",setTimeout(()=>{r&&(r.style.display="none")},1500)),u.length>0&&p.broadcast.filter(y=>!u.includes(y.id)).filter(y=>!d.includes(y.id)).length>0&&(console.log("\u{1F514} Novo aviso detectado! Tocando som."),ee.playNotification()),_t(p.broadcast),S(),t&&R())}catch(p){console.error("Erro no update:",p),t&&r&&(r.innerHTML="\u26A0\uFE0F Falha na conex\xE3o.",r.style.backgroundColor="#fce8e6")}}function S(){let r=document.getElementById("cw-btn-broadcast");if(!r)return;let u=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]");if(Ue.some(p=>!u.includes(p.id))){if(r.classList.add("has-new"),!r.querySelector(".cw-badge")){let p=document.createElement("div");p.className="cw-badge",Object.assign(p.style,{position:"absolute",top:"8px",right:"8px",width:"8px",height:"8px",backgroundColor:"#d93025",borderRadius:"50%",border:"1px solid #fff",zIndex:"10"}),r.appendChild(p)}}else{r.classList.remove("has-new");let p=r.querySelector(".cw-badge");p&&p.remove()}}let q=He.getCachedBroadcasts();q.length>0&&(_t(q),R()),T(),n||(n=setInterval(T,o));function R(){x.innerHTML="";let r=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]"),u=[...Ue].sort((h,C)=>{let y=r.includes(h.id),O=r.includes(C.id);return y===O?0:y?1:-1});if(u.every(h=>r.includes(h.id))){let h=document.createElement("div");Object.assign(h.style,a.emptyState),h.innerHTML=`
           ...
           `,x.appendChild(h)}let d=u.filter(h=>!r.includes(h.id)),p=u.filter(h=>r.includes(h.id));if(d.forEach(h=>x.appendChild(V(h,!1))),p.length>0){let h=document.createElement("div");Object.assign(h.style,a.historyDivider),h.innerHTML=`<span>Visualizar ${p.length} avisos anteriores</span><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="transition:transform 0.3s"><polyline points="6 9 12 15 18 9"></polyline></svg>`;let C=document.createElement("div");Object.assign(C.style,a.historyContainer),p.forEach(O=>C.appendChild(V(O,!0)));let y=!1;h.onclick=()=>{ee.playClick(),y=!y,C.style.display=y?"flex":"none",h.querySelector("svg").style.transform=y?"rotate(180deg)":"rotate(0deg)",h.querySelector("span").textContent=y?"Ocultar hist\xF3rico":`Visualizar ${p.length} avisos anteriores`},x.appendChild(h),x.appendChild(C)}}function V(r,u){let d=document.createElement("div");Object.assign(d.style,u?a.cardHistory:a.card);let p=i[r.type]||i.info,h=document.createElement("div");Object.assign(h.style,a.cardHeader,{background:p.bg,color:p.text,borderBottom:`1px solid ${p.border}`});let C=document.createElement("div");if(Object.assign(C.style,{display:"flex",alignItems:"center",gap:"6px"}),C.innerHTML=`${p.icon} <span>${r.type.toUpperCase()}</span>`,h.appendChild(C),u){let O=document.createElement("span");O.textContent=s(r.date),O.style.opacity="0.7",h.appendChild(O)}else{let O=document.createElement("button");O.title="Marcar como lido",Object.assign(O.style,a.dismissBtn),O.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>',O.onmouseenter=()=>{O.style.color="#1e8e3e",O.style.background="#e6f4ea",O.style.borderColor="#1e8e3e"},O.onmouseleave=()=>{O.style.color="#5f6368",O.style.background="#fff",O.style.borderColor="rgba(0,0,0,0.1)"},O.onclick=se=>{se.stopPropagation(),ee.playClick(),d.style.transform="translateX(20px)",d.style.opacity="0",setTimeout(()=>{let Z=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]");Z.push(r.id),localStorage.setItem("cw_read_broadcasts",JSON.stringify(Z)),R(),S()},250)},h.appendChild(O)}if(d.appendChild(h),r.title){let O=document.createElement("div");Object.assign(O.style,a.msgTitle),O.textContent=r.title,d.appendChild(O)}if(!u){let O=document.createElement("div");Object.assign(O.style,a.metaContainer),O.innerHTML=`<span style="font-weight:600">${r.author}</span> \u2022 <span>${s(r.date)}</span>`,d.appendChild(O)}let y=document.createElement("div");return Object.assign(y.style,a.cardBody),y.innerHTML=m(r.text),d.appendChild(y),d}R();let K=document.createElement("div");Object.assign(K.style,Ye),K.className="no-drag",b.appendChild(K),Xe(b,K),document.body.appendChild(b);let v=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]"),w=Ue.some(r=>!v.includes(r.id));return{toggle:c,hasUnread:w}}function Mo(){if(window.techSolInitialized){Et();return}window.techSolInitialized=!0,console.log("\u{1F680} TechSol Suite Initializing...");try{Vt();try{ee.initGlobalListeners(),ee.playStartup()}catch(i){console.warn("\xC1udio n\xE3o p\xF4de ser iniciado automaticamente:",i)}He.fetchTips(),Et();let e=go(),t=bo(),n=ho(),o=xo(),s=yo();io({toggleNotes:e,toggleEmail:t,toggleScript:n,toggleLinks:o,broadcastControl:s})}catch(e){console.error("Erro fatal na inicializa\xE7\xE3o:",e),ae("Erro cr\xEDtico ao iniciar o Case Wizard.",{error:!0})}}Mo();})();
