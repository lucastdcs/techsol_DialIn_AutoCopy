...
`).filter(ee=>ee.trim()!=="").map(ee=>`<p style="margin: 0 0 8px 0;">${ee}</p>`).join(""):F.tagName==="TEXTAREA"&&(V=V.replace(/\n/g,"<br>"));let Z=V.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(Z===""||Z==="\u2022"||Z.toLowerCase()==="n/a"){let ee=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${G}\\}(?:<br>\\s*)?`,"gi");ee.test(f)?f=f.replace(ee,""):f=f.replace($,"")}else f=f.replace($,V.replace(/\$/g,"$$$$"))}),f=f.replace(/{([A-Z0-9_]+)}/g,""),f=f.replace(/(<br>){3,}/g,"<br><br>"),typeof i<"u"&&i.getOutput&&(f+=i.getOutput()),f}Me.onclick=()=>{let u=yt();u?(gt(u),K(M("copiado_sucesso"))):K(M("selecione_substatus"),{error:!0})},De.onclick=async()=>{let u=T.value,N=yt();if(!N){K(M("selecione_substatus"),{error:!0});return}gt(N),lt();let f=Qe(),D=await Ft();if(D)try{if(D.focus(),D.innerHTML.trim()==="<p><br></p>"||D.innerHTML.trim()==="<br>"||D.innerText.trim()===""){let B=document.createRange();B.selectNodeContents(D);let Q=window.getSelection();Q.removeAllRanges(),Q.addRange(B),document.execCommand("delete",!1,null)}else if(!D.innerHTML.endsWith("<br><br>")){let B=document.createRange();B.selectNodeContents(D),B.collapse(!1);let Q=window.getSelection();Q.removeAllRanges(),Q.addRange(B),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,N),Gt(D),setTimeout(()=>{K(M("inserido_copiado"))},600);let ie=typeof Te<"u"&&Te?Te.checked:!0;if(u&&Ye[u]&&ie){let B=Ye[u];await Nt(B),await new Promise(Q=>setTimeout(Q,500))}f(),rt(1.5),w.value="",T.innerHTML=`<option value="">${M("select_substatus")}</option>`,T.disabled=!0}catch(X){console.error(X),K("Erro ao inserir.",{error:!0}),f()}};function rt(u=1.5){u<=1.5&&(Ae.style.display="none",ue.innerHTML=""),u<=2&&(me.style.display="none",s.reset(),le.style.display="none"),u<=3&&(be.style.display="none",he.innerHTML="",i.reset(),Ne.style.display="none",Ee.style.display="none")}function lt(){r=!r,Se(r,p,"cw-btn-notes")}return nt("bau"),at("pt"),lt}var ke={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
      ...
      `;let z=y.querySelector("#csa-back-btn");z.onmouseover=()=>z.style.backgroundColor="#f1f3f4",z.onmouseout=()=>z.style.backgroundColor="transparent",z.onclick=O;let R=y.querySelector("#csa-insert-btn");R.onmouseover=()=>R.style.backgroundColor="#174ea6",R.onmouseout=()=>R.style.backgroundColor="#1a73e8",R.onclick=async()=>{R.style.transform="scale(0.96)",A();let oe=Qe();try{await bt(I),oe()}catch(ne){console.error(ne),oe()}setTimeout(()=>{R.style.transform="scale(1)",O()},300)}}function O(){o="list",P.style.transform="translateX(0)"}function H(){l.innerHTML="",Object.keys(ke).forEach(I=>{let J=ke[I],j=document.createElement("button");j.textContent=J.title,Object.assign(j.style,S),t===I&&n===""&&Object.assign(j.style,x),j.onclick=()=>{t=I,n="",d.value="",H(),T()},l.appendChild(j)})}function T(){c.innerHTML="";let I=[];if(n.trim()!==""?Object.values(ke).forEach(z=>{let R=z.emails.filter(oe=>oe.name.toLowerCase().includes(n.toLowerCase()));I=[...I,...R]}):ke[t]&&(I=ke[t].emails),I.length===0){c.innerHTML='<div style="text-align:center; padding:60px 20px; color:#9aa0a6;"><div style="font-size:24px; margin-bottom:8px;">\u{1F50D}</div><div style="font-size:14px;">Nenhum template encontrado.</div></div>';return}let J='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="22" y1="2" x2="11" y2="13"></line><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon></svg>',j='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>';I.forEach(z=>{let R=document.createElement("div");Object.assign(R.style,m);let oe=z.subject.length>50?z.subject.substring(0,50)+"...":z.subject;R.innerHTML=`
