...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let w=l.querySelector(".cw-hero-grid"),u=l.querySelector(".cw-acc-container"),O=l.querySelector(".cw-results-container"),h=l.querySelector(".cw-search-input"),f=l.querySelector(".cw-status-bar"),x=l.querySelector(".cw-status-text"),E=l.querySelector(".cw-footer-icons");s.forEach(([m,b])=>{let i=o(b.name),c=document.createElement("div");c.className="cw-hero-card",c.id=`hero-${m}`,c.style.setProperty("--hero-color",i.color),c.innerHTML=`
        ...
        `,c.onclick=S=>{if(S.target.closest(".cw-step-btn"))return;let d=t[m]?t[m].count:0;R(m,d>0?-d:1,b)},c.querySelector(".minus").onclick=()=>R(m,-1,b),c.querySelector(".plus").onclick=()=>R(m,1,b),c.dataset.color=i.color,w.appendChild(c)});function I(m,b){let i=o(b.name),c=document.createElement("div");return c.className="cw-task-item",c.dataset.id=m,c.innerHTML=`
        ...
        `,c.onclick=S=>{if(S.target.closest(".cw-step-btn"))return;let d=t[m]?t[m].count:0;R(m,d>0?-d:1,b)},c.querySelector(".minus").onclick=()=>R(m,-1,b),c.querySelector(".plus").onclick=()=>R(m,1,b),c}Object.entries(r).forEach(([m,b])=>{let i=document.createElement("div");i.className="cw-acc-group";let c=document.createElement("div");c.className="cw-acc-header",c.innerHTML=`
            <div class="cw-acc-title">
                <div class="cw-acc-dot" style="background:${b.brand.color}"></div>
                ${m}
            </div>
            <div class="cw-acc-icon">\u25BC</div>
        `,c.onclick=()=>{u.querySelectorAll(".cw-acc-group.open").forEach(d=>{d!==i&&d.classList.remove("open")}),i.classList.toggle("open")};let S=document.createElement("div");S.className="cw-acc-body",b.tasks.forEach(d=>{let y=I(d.key,d);S.appendChild(y)}),i.appendChild(c),i.appendChild(S),u.appendChild(i)});function R(m,b,i){t[m]||(t[m]={count:0,data:i,brand:o(i.name)}),t[m].count+=b,t[m].count<=0&&delete t[m],j(),V(),e&&e()}function j(){s.forEach(([S])=>{let d=w.querySelector(`#hero-${S}`);if(!d)return;let y=t[S];y?(d.classList.add("active"),d.querySelector(".cw-step-val").textContent=y.count,d.querySelector(".cw-step-val").style.color=d.dataset.color):d.classList.remove("active")}),l.querySelectorAll(".cw-task-item").forEach(S=>{let d=S.dataset.id,y=t[d];y?(S.classList.add("selected"),S.querySelector(".cw-step-val").textContent=y.count):S.classList.remove("selected")});let b=Object.keys(t),i=0,c=[];if(b.forEach(S=>{let d=t[S];i+=d.count;for(let y=0;y<d.count;y++)c.length<6&&c.push(d.brand)}),i>0){f.classList.add("visible");let S=i>1?"A\xE7\xF5es":"A\xE7\xE3o",d=i>1?"definidas":"definida";x.textContent=`${i} ${S} ${d}`,E.innerHTML="",c.forEach(y=>{let k=document.createElement("div");k.className="cw-mini-icon",k.innerHTML=je[y.icon]||je.default;let v=k.querySelector("svg");v&&(v.style.width="14px",v.style.height="14px"),E.appendChild(k)})}else f.classList.remove("visible")}h.addEventListener("input",m=>{let b=m.target.value.toLowerCase();if(b.length>0){u.style.display="none",O.style.display="block",O.innerHTML="";let i=!1;Object.entries(_e).forEach(([c,S])=>{if(S.name.toLowerCase().includes(b)){i=!0;let d=I(c,S);t[c]&&(d.classList.add("selected"),d.querySelector(".cw-step-val").textContent=t[c].count),O.appendChild(d)}}),i||(O.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else u.style.display="block",O.style.display="none"});function V(){g.innerHTML="";let m=Object.keys(t),b=!1,i=document.getElementById("sub-status"),c="implementation";if(i&&i.value.toLowerCase().includes("education")&&(c="education"),m.length===0){g.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}if(m.length===0){g.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let S=document.createElement("div");S.className="cw-info-banner",S.innerHTML=`
...
`).filter(re=>re.trim()!=="").map(re=>`<p style="margin: 0 0 8px 0;">${re}</p>`).join(""):$.tagName==="TEXTAREA"&&(U=U.replace(/\n/g,"<br>"));let se=U.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(se===""||se==="\u2022"||se.toLowerCase()==="n/a"){let re=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${F}\\}(?:<br>\\s*)?`,"gi");re.test(A)?A=A.replace(re,""):A=A.replace(L,"")}else A=A.replace(L,U.replace(/\$/g,"$$$$"))}),A=A.replace(/{([A-Z0-9_]+)}/g,""),A=A.replace(/(<br>){3,}/g,"<br><br>"),typeof a<"u"&&a.getOutput&&(A+=a.getOutput()),A}Ke.onclick=()=>{let C=Nt();C?(Tt(C),oe(P("copiado_sucesso"))):oe(P("selecione_substatus"),{error:!0})},Qe.onclick=async()=>{let C=v.value,D=Nt();if(!D){oe(P("selecione_substatus"),{error:!0});return}Tt(D),bt();let A=Xe(),H=await oo();if(H)try{if(H.focus(),H.innerHTML.trim()==="<p><br></p>"||H.innerHTML.trim()==="<br>"||H.innerText.trim()===""){let de=document.createRange();de.selectNodeContents(H);let X=window.getSelection();X.removeAllRanges(),X.addRange(de),document.execCommand("delete",!1,null)}else if(!H.innerHTML.endsWith("<br><br>")){let de=document.createRange();de.selectNodeContents(H),de.collapse(!1);let X=window.getSelection();X.removeAllRanges(),X.addRange(de),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,D),to(H),setTimeout(()=>{oe(P("inserido_copiado"))},600);let le=typeof Ge<"u"&&Ge?Ge.checked:!0;if(C&&at[C]&&le){let de=at[C];await $t(de),await new Promise(X=>setTimeout(X,500))}A(),gt(1.5),S.value="",v.innerHTML=`<option value="">${P("select_substatus")}</option>`,v.disabled=!0}catch(te){console.error(te),oe("Erro ao inserir.",{error:!0}),A()}};function gt(C=1.5){C<=1.5&&(Me.style.display="none",ye.innerHTML=""),C<=2&&(ve.style.display="none",l.reset(),be.style.display="none"),C<=3&&(ke.style.display="none",Ne.innerHTML="",a.reset(),Ve.style.display="none",De.style.display="none")}function bt(){if(s=!s,s){let C=p.querySelector(".cw-expand-btn");C&&typeof C.resetState=="function"&&C.resetState()}Ae(s,p,"cw-btn-notes")}return dt("bau"),pt("pt"),bt}var Pe={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
                    <p>Ol\xE1,</p>
                    <br>
 ...
 D) Nombre del sitio web.`,"autenticar la cuenta del anunciante en el cases, si corresponde.","T\xE9rminos y condi\xE7\xF5es.","Informar las Task solicitadas y AM.","Cerrar contenido sensible.","Confirmaci\xF3n de copia de seguridad y acceso de administrador a las ferramentas.","Resumen de llamada."],fim:["Ofrecer ayuda adicional.","Dejar de compartir la pantalla.","Pasos siguientes  (Si se le har\xE1 seguimiento al caso).","Encuesta de Satisfa\xE7\xE3o.","Informar al cliente que el equipo de QA ir\xE1 a realizar pruebas en los siguientes dias."]},"EN BAU":{color:"#ff0011",inicio:["Example 1","Example 2"],fim:["Example 3","Example 4"]}};function lo(){let e="v2.1 (Apple Motion)",t={progressBarContainer:{height:"4px",background:"#f1f3f4",width:"100%",position:"relative",overflow:"hidden"},progressBarFill:{height:"100%",background:"linear-gradient(90deg, #4285F4, #34A853)",width:"0%",transition:"width 0.5s cubic-bezier(0.25, 0.8, 0.25, 1)",borderRadius:"0 2px 2px 0"},contentArea:{padding:"16px",overflowY:"auto",flexGrow:"1",background:"#f8f9fa",scrollBehavior:"smooth"},card:{background:"#ffffff",border:"1px solid #dadce0",borderRadius:"12px",padding:"16px",marginBottom:"16px",transition:"transform 0.2s ease, box-shadow 0.2s ease",boxShadow:"0 1px 2px rgba(0,0,0,0.02)"},cardTitle:{fontSize:"13px",fontWeight:"700",color:"#5f6368",textTransform:"uppercase",letterSpacing:"0.6px",marginBottom:"12px",display:"flex",alignItems:"center",justifyContent:"space-between",userSelect:"none"},itemRow:{display:"flex",alignItems:"flex-start",padding:"10px 8px",cursor:"pointer",borderRadius:"8px",transition:"background-color 0.15s ease, opacity 0.3s ease",color:"#202124",fontSize:"14px",lineHeight:"1.5",marginBottom:"2px"},itemCompleted:{opacity:"0.5",textDecoration:"line-through",color:"#5f6368"},checkbox:{minWidth:"20px",height:"20px",borderRadius:"6px",border:"2px solid #dadce0",marginRight:"14px",marginTop:"1px",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)",background:"#fff"},footer:{padding:"12px 16px",borderTop:"1px solid #eee",background:"#fff",display:"flex",justifyContent:"space-between",alignItems:"center"},resetBtn:{background:"transparent",border:"none",color:"#d93025",fontSize:"12px",fontWeight:"600",cursor:"pointer",padding:"6px 12px",borderRadius:"20px",transition:"background 0.2s ease",display:"flex",alignItems:"center",gap:"4px"}},n={},o="PT",s="BAU",r=!1,a=document.createElement("div");a.id="call-script-popup",Object.assign(a.style,we,{right:"auto",left:"50%",width:"400px",height:"650px",display:"flex",flexDirection:"column",opacity:"0",pointerEvents:"none"});let l={popup:a,googleLine:null};function p(){r=!r,Ae(r,a,"cw-btn-script")}let g=Ce(a,"Call Script",e,"Guia interativo para condu\xE7\xE3o de chamadas.",l,()=>{p()});a.appendChild(g);let w=document.createElement("div");Object.assign(w.style,t.progressBarContainer);let u=document.createElement("div");Object.assign(u.style,t.progressBarFill),w.appendChild(u),a.appendChild(w);let O=document.createElement("div");O.id="csa-content",Object.assign(O.style,t.contentArea),a.appendChild(O);let h=document.createElement("div");Object.assign(h.style,t.footer);let f=document.createElement("span");f.textContent="by lucaste@",Object.assign(f.style,{fontSize:"10px",color:"#bdc1c6"});let x=document.createElement("button");x.innerHTML=`
  ...
  `,Object.assign(x.style,t.resetBtn),x.onmouseenter=()=>x.style.background="#fce8e6",x.onmouseleave=()=>x.style.background="transparent",x.onclick=()=>{x.style.transform="scale(0.9)",setTimeout(()=>x.style.transform="scale(1)",150);for(let k in n)delete n[k];c()},h.appendChild(f),h.appendChild(x),a.appendChild(h);let E=document.createElement("div");Object.assign(E.style,{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px",gap:"8px"});let I=document.createElement("div");Object.assign(I.style,{display:"flex",borderRadius:"8px",border:"1px solid #dadce0",overflow:"hidden",background:"#fff"});let R=document.createElement("div");R.textContent="BAU";let j=document.createElement("div");j.textContent="LT",Object.assign(R.style,me),Object.assign(j.style,me),I.appendChild(R),I.appendChild(j);let V=document.createElement("select");Object.assign(V.style,Pt,{marginBottom:"0",width:"auto",minWidth:"90px",paddingTop:"6px",paddingBottom:"6px",paddingRight:"30px",height:"32px",backgroundPosition:"right 8px center"}),V.innerHTML='<option value="PT">PT</option><option value="ES">ES</option><option value="EN">EN</option>',V.value=o,E.appendChild(I),E.appendChild(V),O.appendChild(E);let m=document.createElement("div");m.id="csa-checklist-area",O.appendChild(m);let b=document.createElement("div");Object.assign(b.style,We),b.className="no-drag",b.title="Redimensionar",a.appendChild(b),Ye(a,b),document.body.appendChild(a);function i(k){return k}function c(){m.innerHTML="";let k=`${o} ${s}`,v=ro[k];if(!v){m.innerHTML=`<div style="padding: 30px; text-align: center; color: #bdc1c6; display: flex; flex-direction: column; align-items: center; gap: 10px;">
        ...
        <div style="display:flex; flex-direction:column; align-items:center; justify-content:center; padding:40px; opacity:0.6;">
           ...
           `,f.appendChild(y)}let S=c.filter(y=>!i.includes(y.id)),d=c.filter(y=>i.includes(y.id));if(S.forEach(y=>f.appendChild(j(y,!1))),d.length>0){let y=document.createElement("div");Object.assign(y.style,a.historyDivider),y.innerHTML=`<span>Visualizar ${d.length} avisos anteriores</span><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="transition:transform 0.3s"><polyline points="6 9 12 15 18 9"></polyline></svg>`;let k=document.createElement("div");Object.assign(k.style,a.historyContainer),d.forEach(T=>k.appendChild(j(T,!0)));let v=!1;y.onclick=()=>{J.playClick(),v=!v,k.style.display=v?"flex":"none",y.querySelector("svg").style.transform=v?"rotate(180deg)":"rotate(0deg)",y.querySelector("span").textContent=v?"Ocultar hist\xF3rico":`Visualizar ${d.length} avisos anteriores`},f.appendChild(y),f.appendChild(k)}}function j(i,c){let S=document.createElement("div");Object.assign(S.style,c?a.cardHistory:a.card);let d=r[i.type]||r.info,y=document.createElement("div");Object.assign(y.style,a.cardHeader,{background:d.bg,color:d.text,borderBottom:`1px solid ${d.border}`});let k=document.createElement("div");if(Object.assign(k.style,{display:"flex",alignItems:"center",gap:"6px"}),k.innerHTML=`${d.icon} <span>${i.type.toUpperCase()}</span>`,y.appendChild(k),c){let T=document.createElement("span");T.textContent=s(i.date),T.style.opacity="0.7",y.appendChild(T)}else{let T=document.createElement("button");T.title="Marcar como lido",Object.assign(T.style,a.dismissBtn),T.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>',T.onmouseenter=()=>{T.style.color="#1e8e3e",T.style.background="#e6f4ea",T.style.borderColor="#1e8e3e"},T.onmouseleave=()=>{T.style.color="#5f6368",T.style.background="#fff",T.style.borderColor="rgba(0,0,0,0.1)"},T.onclick=G=>{G.stopPropagation(),J.playClick(),S.style.transform="translateX(20px)",S.style.opacity="0",setTimeout(()=>{let M=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]");M.push(i.id),localStorage.setItem("cw_read_broadcasts",JSON.stringify(M)),R(),E()},250)},y.appendChild(T)}if(S.appendChild(y),i.title){let T=document.createElement("div");Object.assign(T.style,a.msgTitle),T.textContent=i.title,S.appendChild(T)}if(!c){let T=document.createElement("div");Object.assign(T.style,a.metaContainer),T.innerHTML=`<span style="font-weight:600">${i.author}</span> \u2022 <span>${s(i.date)}</span>`,S.appendChild(T)}let v=document.createElement("div");return Object.assign(v.style,a.cardBody),v.innerHTML=p(i.text),S.appendChild(v),S}R();let V=document.createElement("div");Object.assign(V.style,We),V.className="no-drag",g.appendChild(V),Ye(g,V),document.body.appendChild(g);let m=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]"),b=qe.some(i=>!m.includes(i.id));return{toggle:u,hasUnread:b}}function Eo(){if(window.techSolInitialized){wt();return}window.techSolInitialized=!0,console.log("\u{1F680} TechSol Suite Initializing...");try{Bt();try{J.initGlobalListeners(),J.playStartup()}catch(r){console.warn("\xC1udio n\xE3o p\xF4de ser iniciado automaticamente:",r)}Be.fetchTips(),wt();let e=io(),t=so(),n=lo(),o=co(),s=po();Qt({toggleNotes:e,toggleEmail:t,toggleScript:n,toggleLinks:o,broadcastControl:s})}catch(e){console.error("Erro fatal na inicializa\xE7\xE3o:",e),oe("Erro cr\xEDtico ao iniciar o Case Wizard.",{error:!0})}}Eo();})();
