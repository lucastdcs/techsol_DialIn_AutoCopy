...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
        ...
        `,document.head.appendChild(b)}let l=document.createElement("div");l.className="cw-zen-container";let g=document.createElement("div");Object.assign(g.style,{display:"none"});let u=document.createElement("div");u.className="cw-screens-container",g.appendChild(u),l.innerHTML=`
...
`).filter(B=>B.trim()!=="").map(B=>`<p style="margin: 0 0 8px 0;">${B}</p>`).join(""):W.tagName==="TEXTAREA"&&($=$.replace(/\n/g,"<br>"));let re=$.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(re===""||re==="\u2022"||re.toLowerCase()==="n/a"){let B=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${D}\\}(?:<br>\\s*)?`,"gi");B.test(E)?E=E.replace(B,""):E=E.replace(_,"")}else E=E.replace(_,$.replace(/\$/g,"$$$$"))}),E=E.replace(/{([A-Z0-9_]+)}/g,""),E=E.replace(/(<br>){3,}/g,"<br><br>"),typeof a<"u"&&a.getOutput&&(E+=a.getOutput()),E}Ke.onclick=()=>{let C=_t();C?(Ot(C),ee(P("copiado_sucesso"))):ee(P("selecione_substatus"),{error:!0})},Qe.onclick=async()=>{let C=w.value,M=_t();if(!M){ee(P("selecione_substatus"),{error:!0});return}Ot(M),ft();let E=Xe(),H=await po();if(H)try{if(H.focus(),H.innerHTML.trim()==="<p><br></p>"||H.innerHTML.trim()==="<br>"||H.innerText.trim()===""){let de=document.createRange();de.selectNodeContents(H);let K=window.getSelection();K.removeAllRanges(),K.addRange(de),document.execCommand("delete",!1,null)}else if(!H.innerHTML.endsWith("<br><br>")){let de=document.createRange();de.selectNodeContents(H),de.collapse(!1);let K=window.getSelection();K.removeAllRanges(),K.addRange(de),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,M),co(H),setTimeout(()=>{ee(P("inserido_copiado"))},600);let le=typeof ze<"u"&&ze?ze.checked:!0;if(C&&st[C]&&le){let de=st[C];await Zt(de),await new Promise(K=>setTimeout(K,500))}E(),bt(1.5),S.value="",w.innerHTML=`<option value="">${P("select_substatus")}</option>`,w.disabled=!0}catch(oe){console.error(oe),ee("Erro ao inserir.",{error:!0}),E()}};function bt(C=1.5){C<=1.5&&(Me.style.display="none",Ae.innerHTML=""),C<=2&&(l.reset(),pe.style.display="none"),C<=3&&(he.style.display="none",_e.innerHTML="",a.reset(),Ue.style.display="none",Fe.style.display="none")}function ft(){if(s=!s,s){let C=g.querySelector(".cw-expand-btn");C&&typeof C.resetState=="function"&&C.resetState()}Te(s,g,"cw-btn-notes")}return pt("bau"),ut("pt"),ft}var Ve={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
                    <p>Ol\xE1,</p>
                    <br>
