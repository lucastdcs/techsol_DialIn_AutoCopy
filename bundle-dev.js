...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
        ...
        `,document.head.appendChild(g)}let i=document.createElement("div");i.className="cw-zen-container";let s=document.createElement("div");Object.assign(s.style,{display:"none"});let u=document.createElement("div");u.className="cw-screens-container",s.appendChild(u),i.innerHTML=`
    ...
    `;let y=i.querySelector(".cw-hero-grid"),v=i.querySelector(".cw-acc-container"),b=i.querySelector(".cw-results-container"),w=i.querySelector(".cw-search-input"),C=i.querySelector(".cw-status-bar"),x=i.querySelector(".cw-status-text"),A=i.querySelector(".cw-footer-icons");o.forEach(([g,m])=>{let l=n(m.name),c=document.createElement("div");c.className="cw-hero-card",c.id=`hero-${g}`,c.style.setProperty("--hero-color",l.color),c.innerHTML=`
        ...
        `,c.onclick=p=>{if(p.target.closest(".cw-step-btn"))return;let h=t[g]?t[g].count:0;T(g,h>0?-h:1,m)},c.querySelector(".minus").onclick=()=>T(g,-1,m),c.querySelector(".plus").onclick=()=>T(g,1,m),c.dataset.color=l.color,y.appendChild(c)});function E(g,m){let l=n(m.name),c=document.createElement("div");return c.className="cw-task-item",c.dataset.id=g,c.innerHTML=`
        ...
        `,c.onclick=p=>{if(p.target.closest(".cw-step-btn"))return;let h=t[g]?t[g].count:0;T(g,h>0?-h:1,m)},c.querySelector(".minus").onclick=()=>T(g,-1,m),c.querySelector(".plus").onclick=()=>T(g,1,m),c}Object.entries(r).forEach(([g,m])=>{let l=document.createElement("div");l.className="cw-acc-group";let c=document.createElement("div");c.className="cw-acc-header",c.innerHTML=`
            <div class="cw-acc-title">
                <div class="cw-acc-dot" style="background:${m.brand.color}"></div>
                ${g}
            </div>
            <div class="cw-acc-icon">\u25BC</div>
        `,c.onclick=()=>{v.querySelectorAll(".cw-acc-group.open").forEach(h=>{h!==l&&h.classList.remove("open")}),l.classList.toggle("open")};let p=document.createElement("div");p.className="cw-acc-body",m.tasks.forEach(h=>{let S=E(h.key,h);p.appendChild(S)}),l.appendChild(c),l.appendChild(p),v.appendChild(l)});function T(g,m,l){t[g]||(t[g]={count:0,data:l,brand:n(l.name)}),t[g].count+=m,t[g].count<=0&&delete t[g],j(),V(),e&&e()}function j(){o.forEach(([p])=>{let h=y.querySelector(`#hero-${p}`);if(!h)return;let S=t[p];S?(h.classList.add("active"),h.querySelector(".cw-step-val").textContent=S.count,h.querySelector(".cw-step-val").style.color=h.dataset.color):h.classList.remove("active")}),i.querySelectorAll(".cw-task-item").forEach(p=>{let h=p.dataset.id,S=t[h];S?(p.classList.add("selected"),p.querySelector(".cw-step-val").textContent=S.count):p.classList.remove("selected")});let m=Object.keys(t),l=0,c=[];if(m.forEach(p=>{let h=t[p];l+=h.count;for(let S=0;S<h.count;S++)c.length<6&&c.push(h.brand)}),l>0){C.classList.add("visible");let p=l>1?"A\xE7\xF5es":"A\xE7\xE3o",h=l>1?"definidas":"definida";x.textContent=`${l} ${p} ${h}`,A.innerHTML="",c.forEach(S=>{let N=document.createElement("div");N.className="cw-mini-icon",N.innerHTML=_e[S.icon]||_e.default;let $=N.querySelector("svg");$&&($.style.width="14px",$.style.height="14px"),A.appendChild(N)})}else C.classList.remove("visible")}w.addEventListener("input",g=>{let m=g.target.value.toLowerCase();if(m.length>0){v.style.display="none",b.style.display="block",b.innerHTML="";let l=!1;Object.entries(Se).forEach(([c,p])=>{if(p.name.toLowerCase().includes(m)){l=!0;let h=E(c,p);t[c]&&(h.classList.add("selected"),h.querySelector(".cw-step-val").textContent=t[c].count),b.appendChild(h)}}),l||(b.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else v.style.display="block",b.style.display="none"});function V(){u.innerHTML="";let g=Object.keys(t),m=!1,l="implementation";if(g.length===0){u.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let c=document.createElement("div");c.className="cw-info-banner",c.innerHTML=`
...
`).filter(ee=>ee.trim()!=="").map(ee=>`<p style="margin: 0 0 8px 0;">${ee}</p>`).join(""):B.tagName==="TEXTAREA"&&(q=q.replace(/\n/g,"<br>"));let Z=q.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(Z===""||Z==="\u2022"||Z.toLowerCase()==="n/a"){let ee=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${L}\\}(?:<br>\\s*)?`,"gi");ee.test(f)?f=f.replace(ee,""):f=f.replace(k,"")}else f=f.replace(k,q.replace(/\$/g,"$$$$"))}),f=f.replace(/{([A-Z0-9_]+)}/g,""),f=f.replace(/(<br>){3,}/g,"<br><br>"),typeof i<"u"&&i.getOutput&&(f+=i.getOutput()),f}Ge.onclick=()=>{let d=Ct();d?(xt(d),Q(z("copiado_sucesso"))):Q(z("selecione_substatus"),{error:!0})},Fe.onclick=async()=>{let d=O.value,G=Ct();if(!G){Q(z("selecione_substatus"),{error:!0});return}xt(G),dt();let f=Je(),R=await jt();if(R)try{if(R.focus(),R.innerHTML.trim()==="<p><br></p>"||R.innerHTML.trim()==="<br>"||R.innerText.trim()===""){let ae=document.createRange();ae.selectNodeContents(R);let H=window.getSelection();H.removeAllRanges(),H.addRange(ae),document.execCommand("delete",!1,null)}else if(!R.innerHTML.endsWith("<br><br>")){let ae=document.createRange();ae.selectNodeContents(R),ae.collapse(!1);let H=window.getSelection();H.removeAllRanges(),H.addRange(ae),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,G),Pt(R),setTimeout(()=>{Q(z("inserido_copiado"))},600);let oe=typeof Ie<"u"&&Ie?Ie.checked:!0;if(d&&Ke[d]&&oe){let ae=Ke[d];await Lt(ae),await new Promise(H=>setTimeout(H,500))}f(),ct(1.5),S.value="",O.innerHTML=`<option value="">${z("select_substatus")}</option>`,O.disabled=!0}catch(X){console.error(X),Q("Erro ao inserir.",{error:!0}),f()}};function ct(d=1.5){d<=1.5&&(Te.style.display="none",me.innerHTML=""),d<=2&&(be.style.display="none",s.reset(),le.style.display="none"),d<=3&&(fe.style.display="none",ye.innerHTML="",i.reset(),Me.style.display="none",ke.style.display="none")}function dt(){if(r=!r,r){let d=u.querySelector(".cw-expand-btn");d&&typeof d.resetState=="function"&&d.resetState()}Ee(r,u,"cw-btn-links")}return it("bau"),st("pt"),dt}var Re={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
                    <p>Ol\xE1,</p>
                    <br>
