...
/* --- SCREENSHOTS: BRAND IMMERSION --- */
            ...
            .cw-screen-card {
                ...
                /* O segredo do "Fill": um pseudo-elemento invis\xEDvel que aparece no foco */
            }
            
            .cw-screen-card::before {
                content: ''; position: absolute; inset: 0;
                background: var(--brand-bg, #F8F9FA); /* Cor din\xE2mica */
                opacity: 0; transition: opacity 0.3s ease;
                z-index: 0; pointer-events: none;
            }

            /* EFEITO DE FOCO (Imers\xE3o) */
            ...
            .cw-card-title-input {
                font-family: ${I.font}; font-size: 16px; font-weight: 600; color: ${I.textMain};
            ...
            .cw-input-label {
                display: block; font-size: 11px; font-weight: 700; color: ${I.textSub};
...
`).filter(oe=>oe.trim()!=="").map(oe=>`<p style="margin: 0 0 8px 0;">${oe}</p>`).join(""):P.tagName==="TEXTAREA"&&(U=U.replace(/\n/g,"<br>"));let ee=U.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(ee===""||ee==="\u2022"||ee.toLowerCase()==="n/a"){let oe=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${G}\\}(?:<br>\\s*)?`,"gi");oe.test(f)?f=f.replace(oe,""):f=f.replace(V,"")}else f=f.replace(V,U.replace(/\$/g,"$$$$"))}),f=f.replace(/{([A-Z0-9_]+)}/g,""),f=f.replace(/(<br>){3,}/g,"<br><br>"),typeof s<"u"&&s.getOutput&&(f+=s.getOutput()),f}Me.onclick=()=>{let u=yt();u?(gt(u),Q(L("copiado_sucesso"))):Q(L("selecione_substatus"),{error:!0})},Le.onclick=async()=>{let u=N.value,R=yt();if(!R){Q(L("selecione_substatus"),{error:!0});return}gt(R),lt();let f=Ke(),D=await Ft();if(D)try{if(D.focus(),D.innerHTML.trim()==="<p><br></p>"||D.innerHTML.trim()==="<br>"||D.innerText.trim()===""){let z=document.createRange();z.selectNodeContents(D);let J=window.getSelection();J.removeAllRanges(),J.addRange(z),document.execCommand("delete",!1,null)}else if(!D.innerHTML.endsWith("<br><br>")){let z=document.createRange();z.selectNodeContents(D),z.collapse(!1);let J=window.getSelection();J.removeAllRanges(),J.addRange(z),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,R),Gt(D),setTimeout(()=>{Q(L("inserido_copiado"))},600);let se=typeof we<"u"&&we?we.checked:!0;if(u&&We[u]&&se){let z=We[u];await Nt(z),await new Promise(J=>setTimeout(J,500))}f(),rt(1.5),w.value="",N.innerHTML=`<option value="">${L("select_substatus")}</option>`,N.disabled=!0}catch(K){console.error(K),Q("Erro ao inserir.",{error:!0}),f()}};function rt(u=1.5){u<=1.5&&(Ce.style.display="none",ce.innerHTML=""),u<=2&&(de.style.display="none",i.reset(),re.style.display="none"),u<=3&&(pe.style.display="none",fe.innerHTML="",s.reset(),Ie.style.display="none",Ae.style.display="none")}function lt(){r=!r,ve(r,p,"cw-btn-notes")}return nt("bau"),at("pt"),lt}var Oe={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
      ...
      `;let H=h.querySelector("#csa-back-btn");H.onmouseover=()=>H.style.backgroundColor="#f1f3f4",H.onmouseout=()=>H.style.backgroundColor="transparent",H.onclick=T;let _=h.querySelector("#csa-insert-btn");_.onmouseover=()=>_.style.backgroundColor="#174ea6",_.onmouseout=()=>_.style.backgroundColor="#1a73e8",_.onclick=async()=>{_.style.transform="scale(0.96)",A();let te=Ke();try{await bt(M),te()}catch($){console.error($),te()}setTimeout(()=>{_.style.transform="scale(1)",T()},300)}}function T(){o="list",F.style.transform="translateX(0)"}function q(){c.innerHTML="",Object.keys(Oe).forEach(M=>{let Z=Oe[M],j=document.createElement("button");j.textContent=Z.title,Object.assign(j.style,S),t===M&&n===""&&Object.assign(j.style,y),j.onclick=()=>{t=M,n="",d.value="",q(),N()},c.appendChild(j)})}function N(){l.innerHTML="";let M=[];if(n.trim()!==""?Object.values(Oe).forEach(H=>{let _=H.emails.filter(te=>te.name.toLowerCase().includes(n.toLowerCase()));M=[...M,..._]}):Oe[t]&&(M=Oe[t].emails),M.length===0){l.innerHTML='<div style="text-align:center; padding:60px 20px; color:#9aa0a6;"><div style="font-size:24px; margin-bottom:8px;">\u{1F50D}</div><div style="font-size:14px;">Nenhum template encontrado.</div></div>';return}let Z='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="22" y1="2" x2="11" y2="13"></line><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon></svg>',j='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>';M.forEach(H=>{let _=document.createElement("div");Object.assign(_.style,m);let te=H.subject.length>50?H.subject.substring(0,50)+"...":H.subject;_.innerHTML=`
