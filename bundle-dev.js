...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let u=r.querySelector(".cw-hero-grid"),c=r.querySelector(".cw-acc-container"),g=r.querySelector(".cw-results-container"),y=r.querySelector(".cw-search-input"),x=r.querySelector(".cw-status-bar"),h=r.querySelector(".cw-status-text"),A=r.querySelector(".cw-footer-icons");i.forEach(([w,T])=>{let S=o(T.name),M=document.createElement("div");M.className="cw-hero-card",M.id=`hero-${w}`,M.style.setProperty("--hero-color",S.color),M.innerHTML=`
        ...
        `,M.onclick=I=>{if(I.target.closest(".cw-step-btn"))return;let C=t[w]?t[w].count:0;V(w,C>0?-C:1,T)},M.querySelector(".minus").onclick=()=>V(w,-1,T),M.querySelector(".plus").onclick=()=>V(w,1,T),M.dataset.color=S.color,u.appendChild(M)});function E(w,T){let S=o(T.name),M=document.createElement("div");return M.className="cw-task-item",M.dataset.id=w,M.innerHTML=`
        ...
        `,M.onclick=I=>{if(I.target.closest(".cw-step-btn"))return;let C=t[w]?t[w].count:0;V(w,C>0?-C:1,T)},M.querySelector(".minus").onclick=()=>V(w,-1,T),M.querySelector(".plus").onclick=()=>V(w,1,T),M}Object.entries(s).forEach(([w,T])=>{let S=document.createElement("div");S.className="cw-acc-group";let M=document.createElement("div");M.className="cw-acc-header",M.innerHTML=`
            <div class="cw-acc-title">
                <div class="cw-acc-dot" style="background:${T.brand.color}"></div>
                ${w}
            </div>
            <div class="cw-acc-icon">\u25BC</div>
        `,M.onclick=()=>{c.querySelectorAll(".cw-acc-group.open").forEach(C=>{C!==S&&C.classList.remove("open")}),S.classList.toggle("open")};let I=document.createElement("div");I.className="cw-acc-body",T.tasks.forEach(C=>{let q=E(C.key,C);I.appendChild(q)}),S.appendChild(M),S.appendChild(I),c.appendChild(S)});function V(w,T,S){t[w]||(t[w]={count:0,data:S,brand:o(S.name)}),t[w].count+=T,t[w].count<=0&&delete t[w],P(),B(),e&&e()}function P(){i.forEach(([I])=>{let C=u.querySelector(`#hero-${I}`);if(!C)return;let q=t[I];q?(C.classList.add("active"),C.querySelector(".cw-step-val").textContent=q.count,C.querySelector(".cw-step-val").style.color=C.dataset.color):C.classList.remove("active")}),r.querySelectorAll(".cw-task-item").forEach(I=>{let C=I.dataset.id,q=t[C];q?(I.classList.add("selected"),I.querySelector(".cw-step-val").textContent=q.count):I.classList.remove("selected")});let T=Object.keys(t),S=0,M=[];if(T.forEach(I=>{let C=t[I];S+=C.count;for(let q=0;q<C.count;q++)M.length<6&&M.push(C.brand)}),S>0){x.classList.add("visible");let I=S>1?"A\xE7\xF5es":"A\xE7\xE3o",C=S>1?"definidas":"definida";h.textContent=`${S} ${I} ${C}`,A.innerHTML="",M.forEach(q=>{let O=document.createElement("div");O.className="cw-mini-icon",O.innerHTML=Qe[q.icon]||Qe.default;let k=O.querySelector("svg");k&&(k.style.width="14px",k.style.height="14px"),A.appendChild(O)})}else x.classList.remove("visible")}y.addEventListener("input",w=>{let T=w.target.value.toLowerCase();if(T.length>0){c.style.display="none",g.style.display="block",g.innerHTML="";let S=!1;Object.entries(Le).forEach(([M,I])=>{if(I.name.toLowerCase().includes(T)){S=!0;let C=E(M,I);t[M]&&(C.classList.add("selected"),C.querySelector(".cw-step-val").textContent=t[M].count),g.appendChild(C)}}),S||(g.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else c.style.display="block",g.style.display="none"});function B(){d.innerHTML="";let w=Object.keys(t),T=!1,S=document.getElementById("sub-status"),M="implementation";if(S&&S.value.toLowerCase().includes("education")&&(M="education"),w.length===0){d.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}if(w.length===0){d.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let I=document.createElement("div");I.className="cw-info-banner",I.innerHTML=`
