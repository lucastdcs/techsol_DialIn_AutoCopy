...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let S=i.querySelector(".cw-hero-grid"),y=i.querySelector(".cw-acc-container"),b=i.querySelector(".cw-results-container"),C=i.querySelector(".cw-search-input"),g=i.querySelector(".cw-status-bar"),h=i.querySelector(".cw-status-text"),A=i.querySelector(".cw-footer-icons");o.forEach(([m,d])=>{let r=n(d.name),c=document.createElement("div");c.className="cw-hero-card",c.id=`hero-${m}`,c.style.setProperty("--hero-color",r.color),c.innerHTML=`
        ...
        `,c.onclick=v=>{if(v.target.closest(".cw-step-btn"))return;let x=t[m]?t[m].count:0;k(m,x>0?-x:1,d)},c.querySelector(".minus").onclick=()=>k(m,-1,d),c.querySelector(".plus").onclick=()=>k(m,1,d),c.dataset.color=r.color,S.appendChild(c)});function E(m,d){let r=n(d.name),c=document.createElement("div");return c.className="cw-task-item",c.dataset.id=m,c.innerHTML=`
        ...
        `,c.onclick=()=>{y.querySelectorAll(".cw-acc-group.open").forEach(x=>{x!==r&&x.classList.remove("open")}),r.classList.toggle("open")};let v=document.createElement("div");v.className="cw-acc-body",d.tasks.forEach(x=>{let w=E(x.key,x);v.appendChild(w)}),r.appendChild(c),r.appendChild(v),y.appendChild(r)});function k(m,d,r){t[m]||(t[m]={count:0,data:r,brand:n(r.name)}),t[m].count+=d,t[m].count<=0&&delete t[m],P(),$(),e&&e()}function P(){o.forEach(([v])=>{let x=S.querySelector(`#hero-${v}`);if(!x)return;let w=t[v];w?(x.classList.add("active"),x.querySelector(".cw-step-val").textContent=w.count,x.querySelector(".cw-step-val").style.color=x.dataset.color):x.classList.remove("active")}),i.querySelectorAll(".cw-task-item").forEach(v=>{let x=v.dataset.id,w=t[x];w?(v.classList.add("selected"),v.querySelector(".cw-step-val").textContent=w.count):v.classList.remove("selected")});let d=Object.keys(t),r=0,c=[];if(d.forEach(v=>{let x=t[v];r+=x.count;for(let w=0;w<x.count;w++)c.length<6&&c.push(x.brand)}),r>0){g.classList.add("visible");let v=r>1?"A\xE7\xF5es":"A\xE7\xE3o",x=r>1?"definidas":"definida";h.textContent=`${r} ${v} ${x}`,A.innerHTML="",c.forEach(w=>{let M=document.createElement("div");M.className="cw-mini-icon",M.innerHTML=_e[w.icon]||_e.default;let I=M.querySelector("svg");I&&(I.style.width="14px",I.style.height="14px"),A.appendChild(M)})}else g.classList.remove("visible")}C.addEventListener("input",m=>{let d=m.target.value.toLowerCase();if(d.length>0){y.style.display="none",b.style.display="block",b.innerHTML="";let r=!1;Object.entries(Se).forEach(([c,v])=>{if(v.name.toLowerCase().includes(d)){r=!0;let x=E(c,v);t[c]&&(x.classList.add("selected"),x.querySelector(".cw-step-val").textContent=t[c].count),b.appendChild(x)}}),r||(b.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else y.style.display="block",b.style.display="none"});function $(){p.innerHTML="";let m=Object.keys(t),d=!1,r="implementation";if(m.length===0){p.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let c=document.createElement("div");c.className="cw-info-banner",c.innerHTML=`
