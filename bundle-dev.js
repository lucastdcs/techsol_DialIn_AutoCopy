...
@keyframes googleBounce {
    ...
    `;let s=document.createElement("div");if(s.className="cw-focus-backdrop",document.body.appendChild(s),document.body.appendChild(o),o.querySelector(".notes").onclick=w=>{w.stopPropagation(),e.toggleNotes()},o.querySelector(".email").onclick=w=>{w.stopPropagation(),e.toggleEmail()},o.querySelector(".script").onclick=w=>{w.stopPropagation(),e.toggleScript()},o.querySelector(".links").onclick=w=>{w.stopPropagation(),e.toggleLinks()},o.querySelector(".broadcast").onclick=w=>{w.stopPropagation();let x=w.currentTarget.querySelector(".cw-badge");x&&(x.style.transform="scale(0)",setTimeout(()=>x.remove(),200)),e.broadcastControl&&e.broadcastControl.toggle()},e.broadcastControl&&e.broadcastControl.hasUnread){let w=document.createElement("div");w.className="cw-badge",o.querySelector(".broadcast").appendChild(w)}(async function(){await ct(2800),o.classList.add("docked"),await ct(300);let x=o.querySelectorAll(".cw-btn");o.querySelectorAll(".cw-sep").forEach(S=>S.classList.add("visible"));for(let S=0;S<x.length;S++)x[S].classList.add("popped"),await ct(90);await ct(200),o.classList.add("system-check")})();let i=!1,a,l,u,g,m=3;o.onmousedown=w=>{if(w.target.closest("button"))return;w.preventDefault(),a=w.clientX,l=w.clientY;let x=o.getBoundingClientRect();u=x.left,g=x.top,document.addEventListener("mousemove",p),document.addEventListener("mouseup",b)};function p(w){let x=w.clientX-a,E=w.clientY-l;!i&&Math.sqrt(x*x+E*E)>m&&(i=!0,o.style.transition="none"),i&&(o.style.left=`${u+x}px`,o.style.top=`${g+E}px`,o.style.right="auto",o.style.bottom="auto",o.style.transform="none")}function b(w){if(document.removeEventListener("mousemove",p),document.removeEventListener("mouseup",b),i){i=!1,o.style.transition="left 0.6s cubic-bezier(0.34, 1.56, 0.64, 1), top 0.6s cubic-bezier(0.34, 1.56, 0.64, 1), transform 0.4s ease";let x=window.innerWidth,E=window.innerHeight,S=o.getBoundingClientRect(),q=S.left+S.width/2,_;q<x/2?(_=24,o.classList.remove("side-right"),o.classList.add("side-left")):(_=x-S.width-24,o.classList.remove("side-left"),o.classList.add("side-right"));let V=S.top;V<24&&(V=24),V>E-S.height-24&&(V=E-S.height-24),o.style.left=`${_}px`,o.style.top=`${V}px`}else{let x=w.target.closest("button");x&&(x.style.transform="scale(0.9)",setTimeout(()=>x.style.transform="",150))}}}function dt(){let e=document.querySelector(".cw-pill"),t=document.querySelector(".cw-focus-backdrop");if(!e)return()=>{};let n=document.createElement("div");n.className="cw-center-stage",n.innerHTML=`
...
`).filter(B=>B.trim()!=="").map(B=>`<p style="margin: 0 0 8px 0;">${B}</p>`).join(""):W.tagName==="TEXTAREA"&&(U=U.replace(/\n/g,"<br>"));let de=U.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(de===""||de==="\u2022"||de.toLowerCase()==="n/a"){let B=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${D}\\}(?:<br>\\s*)?`,"gi");B.test(T)?T=T.replace(B,""):T=T.replace(I,"")}else T=T.replace(I,U.replace(/\$/g,"$$$$"))}),T=T.replace(/{([A-Z0-9_]+)}/g,""),T=T.replace(/(<br>){3,}/g,"<br><br>"),typeof a<"u"&&a.getOutput&&(T+=a.getOutput()),T}Ke.onclick=()=>{let C=Rt();C?(It(C),ae(j("copiado_sucesso"))):ae(j("selecione_substatus"),{error:!0})},Qe.onclick=async()=>{let C=A.value,F=Rt();if(!F){ae(j("selecione_substatus"),{error:!0});return}It(F),yt();let T=dt(),P=await po();if(P)try{if(P.focus(),P.innerHTML.trim()==="<p><br></p>"||P.innerHTML.trim()==="<br>"||P.innerText.trim()===""){let ue=document.createRange();ue.selectNodeContents(P);let Q=window.getSelection();Q.removeAllRanges(),Q.addRange(ue),document.execCommand("delete",!1,null)}else if(!P.innerHTML.endsWith("<br><br>")){let ue=document.createRange();ue.selectNodeContents(P),ue.collapse(!1);let Q=window.getSelection();Q.removeAllRanges(),Q.addRange(ue),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,F),co(P),setTimeout(()=>{ae(j("inserido_copiado"))},600);let pe=typeof ze<"u"&&ze?ze.checked:!0;if(C&&Pe[C]&&pe){let ue=Pe[C];await lt(ue),await new Promise(Q=>setTimeout(Q,500))}T(),xt(1.5),y.value="",A.innerHTML=`<option value="">${j("select_substatus")}</option>`,A.disabled=!0}catch(ie){console.error(ie),ae("Erro ao inserir.",{error:!0}),T()}};function xt(C=1.5){C<=1.5&&(Le.style.display="none",we.innerHTML=""),C<=2&&(l.reset(),ge.style.display="none"),C<=3&&(ye.style.display="none",_e.innerHTML="",a.reset(),We.style.display="none",De.style.display="none")}function yt(){if(s=!s,s){let C=u.querySelector(".cw-expand-btn");C&&typeof C.resetState=="function"&&C.resetState()}Te(s,u,"cw-btn-notes")}return gt("bau"),bt("pt"),yt}var $e={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
            ...
            <div style="padding:24px;">
                ...
                <div>
                    ...
                    <div style="font-size:13px; color:#3c4043; line-height:1.5; white-space: pre-wrap; word-break: break-word; padding:0 4px;">${M.body.replace(/\n{3,}/g,`
...
`).trim()}</div>
        ...
        `;let R=A.querySelector("#csa-back-btn");R.onmouseenter=()=>R.style.background="#f1f3f4",R.onmouseleave=()=>R.style.background="none",R.onclick=G;let $=A.querySelector("#csa-insert-btn");$.onmouseenter=()=>{$.style.transform="translateY(-1px)",$.style.boxShadow="0 6px 16px rgba(26,115,232,0.4)"},$.onmouseleave=()=>{$.style.transform="translateY(0)",$.style.boxShadow="0 4px 12px rgba(26,115,232,0.3)"},$.onclick=()=>{$.style.transform="scale(0.96)",O(M,"email"),setTimeout(()=>{$.style.transform="scale(1)",G()},300)}}function G(){s="list",f.style.transform="translateX(0)"}function L(M,K,re=null){let R=document.createElement("button"),$=re?`<span style="margin-right:6px; font-size:14px; opacity:0.9;">${re}</span>`:"";return R.innerHTML=`${$}${M}`,Object.assign(R.style,w),n===K&&o===""?Object.assign(R.style,x):(R.onmouseenter=()=>{R.style.background="#F1F3F4",R.style.borderColor="#DADCE0"},R.onmouseleave=()=>{R.style.background="#FFFFFF",R.style.borderColor="#DADCE0"}),R.onclick=()=>{n=K,o="",y.value="",ne(),J()},R}function ne(){d.innerHTML="",d.appendChild(L("Smart CRs",t,"\u26A1")),Object.keys($e).forEach(M=>{d.appendChild(L($e[M].title,M))})}function J(){h.innerHTML="";let M=[];if(o.trim()!==""){let ee=o.toLowerCase();Object.values($e).forEach(N=>{N.emails.forEach(Y=>{(Y.name.toLowerCase().includes(ee)||Y.subject.toLowerCase().includes(ee))&&M.push({type:"email",data:Y})})}),Object.entries(Pe).forEach(([N,Y])=>{if(!Y)return;(N.replace(/_/g," ").toLowerCase().includes(ee)||Y.toLowerCase().includes(ee))&&M.push({type:"cr",key:N,code:Y})})}else n===t?Object.entries(Pe).forEach(([ee,N])=>{N&&M.push({type:"cr",key:ee,code:N})}):$e[n]&&$e[n].emails.forEach(ee=>{M.push({type:"email",data:ee})});if(M.length===0){h.innerHTML=`
