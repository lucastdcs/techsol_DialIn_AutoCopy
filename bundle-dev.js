...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let S=i.querySelector(".cw-hero-grid"),x=i.querySelector(".cw-acc-container"),m=i.querySelector(".cw-results-container"),C=i.querySelector(".cw-search-input"),b=i.querySelector(".cw-status-bar"),h=i.querySelector(".cw-status-text"),A=i.querySelector(".cw-footer-icons");o.forEach(([g,d])=>{let l=n(d.name),c=document.createElement("div");c.className="cw-hero-card",c.id=`hero-${g}`,c.style.setProperty("--hero-color",l.color),c.innerHTML=`
        ...
        `,c.onclick=v=>{if(v.target.closest(".cw-step-btn"))return;let y=t[g]?t[g].count:0;O(g,y>0?-y:1,d)},c.querySelector(".minus").onclick=()=>O(g,-1,d),c.querySelector(".plus").onclick=()=>O(g,1,d),c.dataset.color=l.color,S.appendChild(c)});function E(g,d){let l=n(d.name),c=document.createElement("div");return c.className="cw-task-item",c.dataset.id=g,c.innerHTML=`
...
`).filter(te=>te.trim()!=="").map(te=>`<p style="margin: 0 0 8px 0;">${te}</p>`).join(""):F.tagName==="TEXTAREA"&&(H=H.replace(/\n/g,"<br>"));let J=H.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(J===""||J==="\u2022"||J.toLowerCase()==="n/a"){let te=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${G}\\}(?:<br>\\s*)?`,"gi");te.test(f)?f=f.replace(te,""):f=f.replace(W,"")}else f=f.replace(W,H.replace(/\$/g,"$$$$"))}),f=f.replace(/{([A-Z0-9_]+)}/g,""),f=f.replace(/(<br>){3,}/g,"<br><br>"),typeof i<"u"&&i.getOutput&&(f+=i.getOutput()),f}Ge.onclick=()=>{let u=St();u?(ht(u),Q(M("copiado_sucesso"))):Q(M("selecione_substatus"),{error:!0})},Fe.onclick=async()=>{let u=T.value,N=St();if(!N){Q(M("selecione_substatus"),{error:!0});return}ht(N),dt();let f=Je(),D=await zt();if(D)try{if(D.focus(),D.innerHTML.trim()==="<p><br></p>"||D.innerHTML.trim()==="<br>"||D.innerText.trim()===""){let q=document.createRange();q.selectNodeContents(D);let Z=window.getSelection();Z.removeAllRanges(),Z.addRange(q),document.execCommand("delete",!1,null)}else if(!D.innerHTML.endsWith("<br><br>")){let q=document.createRange();q.selectNodeContents(D),q.collapse(!1);let Z=window.getSelection();Z.removeAllRanges(),Z.addRange(q),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,N),Pt(D),setTimeout(()=>{Q(M("inserido_copiado"))},600);let se=typeof Oe<"u"&&Oe?Oe.checked:!0;if(u&&Ke[u]&&se){let q=Ke[u];await Lt(q),await new Promise(Z=>setTimeout(Z,500))}f(),ct(1.5),w.value="",T.innerHTML=`<option value="">${M("select_substatus")}</option>`,T.disabled=!0}catch(K){console.error(K),Q("Erro ao inserir.",{error:!0}),f()}};function ct(u=1.5){u<=1.5&&(we.style.display="none",me.innerHTML=""),u<=2&&(be.style.display="none",s.reset(),ce.style.display="none"),u<=3&&(ge.style.display="none",ye.innerHTML="",i.reset(),Le.style.display="none",Te.style.display="none")}function dt(){r=!r,Ae(r,p,"cw-btn-notes")}return it("bau"),st("pt"),dt}var Ne={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
      ...
      `;let B=y.querySelector("#csa-back-btn");B.onmouseover=()=>B.style.backgroundColor="#f1f3f4",B.onmouseout=()=>B.style.backgroundColor="transparent",B.onclick=k;let R=y.querySelector("#csa-insert-btn");R.onmouseover=()=>R.style.backgroundColor="#174ea6",R.onmouseout=()=>R.style.backgroundColor="#1a73e8",R.onclick=async()=>{R.style.transform="scale(0.96)",A();let ne=Je();try{await ft(I),ne()}catch(ae){console.error(ae),ne()}setTimeout(()=>{R.style.transform="scale(1)",k()},300)}}function k(){o="list",P.style.transform="translateX(0)"}function $(){l.innerHTML="",Object.keys(Ne).forEach(I=>{let ee=Ne[I],z=document.createElement("button");z.textContent=ee.title,Object.assign(z.style,S),t===I&&n===""&&Object.assign(z.style,x),z.onclick=()=>{t=I,n="",d.value="",$(),T()},l.appendChild(z)})}function T(){c.innerHTML="";let I=[];if(n.trim()!==""?Object.values(Ne).forEach(B=>{let R=B.emails.filter(ne=>ne.name.toLowerCase().includes(n.toLowerCase()));I=[...I,...R]}):Ne[t]&&(I=Ne[t].emails),I.length===0){c.innerHTML='<div style="text-align:center; padding:60px 20px; color:#9aa0a6;"><div style="font-size:24px; margin-bottom:8px;">\u{1F50D}</div><div style="font-size:14px;">Nenhum template encontrado.</div></div>';return}let ee='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="22" y1="2" x2="11" y2="13"></line><polygon points="22 2 15 22 11 13 2 9 22 2"></polygon></svg>',z='<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>';I.forEach(B=>{let R=document.createElement("div");Object.assign(R.style,m);let ne=B.subject.length>50?B.subject.substring(0,50)+"...":B.subject;R.innerHTML=`
