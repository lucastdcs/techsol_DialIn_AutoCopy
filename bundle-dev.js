...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
        ...
        `,document.head.appendChild(m)}let l=document.createElement("div");l.className="cw-zen-container";let u=document.createElement("div");Object.assign(u.style,{display:"none"});let g=document.createElement("div");g.className="cw-screens-container",u.appendChild(g),l.innerHTML=`
    ...
    `;let x=l.querySelector(".cw-hero-grid"),p=l.querySelector(".cw-acc-container"),S=l.querySelector(".cw-results-container"),h=l.querySelector(".cw-search-input"),f=l.querySelector(".cw-status-bar"),y=l.querySelector(".cw-status-text"),T=l.querySelector(".cw-footer-icons");i.forEach(([m,b])=>{let s=o(b.name),c=document.createElement("div");c.className="cw-hero-card",c.id=`hero-${m}`,c.style.setProperty("--hero-color",s.color),c.innerHTML=`
        ...
        `,c.onclick=w=>{if(w.target.closest(".cw-step-btn"))return;let d=t[m]?t[m].count:0;_(m,d>0?-d:1,b)},c.querySelector(".minus").onclick=()=>_(m,-1,b),c.querySelector(".plus").onclick=()=>_(m,1,b),c.dataset.color=s.color,x.appendChild(c)});function q(m,b){let s=o(b.name),c=document.createElement("div");return c.className="cw-task-item",c.dataset.id=m,c.innerHTML=`
        ...
        `,c.onclick=()=>{p.querySelectorAll(".cw-acc-group.open").forEach(d=>{d!==s&&d.classList.remove("open")}),s.classList.toggle("open")};let w=document.createElement("div");w.className="cw-acc-body",b.tasks.forEach(d=>{let v=q(d.key,d);w.appendChild(v)}),s.appendChild(c),s.appendChild(w),p.appendChild(s)});function _(m,b,s){t[m]||(t[m]={count:0,data:s,brand:o(s.name)}),t[m].count+=b,t[m].count<=0&&delete t[m],B(),H(),e&&e()}function B(){i.forEach(([w])=>{let d=x.querySelector(`#hero-${w}`);if(!d)return;let v=t[w];v?(d.classList.add("active"),d.querySelector(".cw-step-val").textContent=v.count,d.querySelector(".cw-step-val").style.color=d.dataset.color):d.classList.remove("active")}),l.querySelectorAll(".cw-task-item").forEach(w=>{let d=w.dataset.id,v=t[d];v?(w.classList.add("selected"),w.querySelector(".cw-step-val").textContent=v.count):w.classList.remove("selected")});let b=Object.keys(t),s=0,c=[];if(b.forEach(w=>{let d=t[w];s+=d.count;for(let v=0;v<d.count;v++)c.length<6&&c.push(d.brand)}),s>0){f.classList.add("visible");let w=s>1?"A\xE7\xF5es":"A\xE7\xE3o",d=s>1?"definidas":"definida";y.textContent=`${s} ${w} ${d}`,T.innerHTML="",c.forEach(v=>{let O=document.createElement("div");O.className="cw-mini-icon",O.innerHTML=Pe[v.icon]||Pe.default;let A=O.querySelector("svg");A&&(A.style.width="14px",A.style.height="14px"),T.appendChild(O)})}else f.classList.remove("visible")}h.addEventListener("input",m=>{let b=m.target.value.toLowerCase();if(b.length>0){p.style.display="none",S.style.display="block",S.innerHTML="";let s=!1;Object.entries(_e).forEach(([c,w])=>{if(w.name.toLowerCase().includes(b)){s=!0;let d=q(c,w);t[c]&&(d.classList.add("selected"),d.querySelector(".cw-step-val").textContent=t[c].count),S.appendChild(d)}}),s||(S.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else p.style.display="block",S.style.display="none"});function H(){g.innerHTML="";let m=Object.keys(t),b=!1,s=document.getElementById("sub-status"),c="implementation";if(s&&s.value.toLowerCase().includes("education")&&(c="education"),m.length===0){g.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}if(m.length===0){g.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let w=document.createElement("div");w.className="cw-info-banner",w.innerHTML=`
...
`).filter(re=>re.trim()!=="").map(re=>`<p style="margin: 0 0 8px 0;">${re}</p>`).join(""):$.tagName==="TEXTAREA"&&(U=U.replace(/\n/g,"<br>"));let se=U.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(se===""||se==="\u2022"||se.toLowerCase()==="n/a"){let re=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${D}\\}(?:<br>\\s*)?`,"gi");re.test(E)?E=E.replace(re,""):E=E.replace(R,"")}else E=E.replace(R,U.replace(/\$/g,"$$$$"))}),E=E.replace(/{([A-Z0-9_]+)}/g,""),E=E.replace(/(<br>){3,}/g,"<br><br>"),typeof a<"u"&&a.getOutput&&(E+=a.getOutput()),E}Ke.onclick=()=>{let C=It();C?(Tt(C),oe(j("copiado_sucesso"))):oe(j("selecione_substatus"),{error:!0})},Qe.onclick=async()=>{let C=A.value,M=It();if(!M){oe(j("selecione_substatus"),{error:!0});return}Tt(M),bt();let E=Xe(),V=await oo();if(V)try{if(V.focus(),V.innerHTML.trim()==="<p><br></p>"||V.innerHTML.trim()==="<br>"||V.innerText.trim()===""){let de=document.createRange();de.selectNodeContents(V);let X=window.getSelection();X.removeAllRanges(),X.addRange(de),document.execCommand("delete",!1,null)}else if(!V.innerHTML.endsWith("<br><br>")){let de=document.createRange();de.selectNodeContents(V),de.collapse(!1);let X=window.getSelection();X.removeAllRanges(),X.addRange(de),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,M),to(V),setTimeout(()=>{oe(j("inserido_copiado"))},600);let le=typeof Ge<"u"&&Ge?Ge.checked:!0;if(C&&at[C]&&le){let de=at[C];await $t(de),await new Promise(X=>setTimeout(X,500))}E(),gt(1.5),w.value="",A.innerHTML=`<option value="">${j("select_substatus")}</option>`,A.disabled=!0}catch(te){console.error(te),oe("Erro ao inserir.",{error:!0}),E()}};function gt(C=1.5){C<=1.5&&(Me.style.display="none",ye.innerHTML=""),C<=2&&(ve.style.display="none",l.reset(),be.style.display="none"),C<=3&&(ke.style.display="none",Ne.innerHTML="",a.reset(),Ue.style.display="none",De.style.display="none")}function bt(){if(i=!i,i){let C=u.querySelector(".cw-expand-btn");C&&typeof C.resetState=="function"&&C.resetState()}Se(i,u,"cw-btn-notes")}return dt("bau"),pt("pt"),bt}var Ve={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
                    <p>Ol\xE1,</p>
                    <br>
