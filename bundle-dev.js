...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let S=i.querySelector(".cw-hero-grid"),x=i.querySelector(".cw-acc-container"),b=i.querySelector(".cw-results-container"),C=i.querySelector(".cw-search-input"),g=i.querySelector(".cw-status-bar"),h=i.querySelector(".cw-status-text"),A=i.querySelector(".cw-footer-icons");o.forEach(([m,d])=>{let r=n(d.name),c=document.createElement("div");c.className="cw-hero-card",c.id=`hero-${m}`,c.style.setProperty("--hero-color",r.color),c.innerHTML=`
        ...
        `,c.onclick=v=>{if(v.target.closest(".cw-step-btn"))return;let y=t[m]?t[m].count:0;k(m,y>0?-y:1,d)},c.querySelector(".minus").onclick=()=>k(m,-1,d),c.querySelector(".plus").onclick=()=>k(m,1,d),c.dataset.color=r.color,S.appendChild(c)});function E(m,d){let r=n(d.name),c=document.createElement("div");return c.className="cw-task-item",c.dataset.id=m,c.innerHTML=`
