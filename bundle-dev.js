...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let u=r.querySelector(".cw-hero-grid"),l=r.querySelector(".cw-acc-container"),f=r.querySelector(".cw-results-container"),b=r.querySelector(".cw-search-input"),m=r.querySelector(".cw-status-bar"),E=r.querySelector(".cw-status-text"),R=r.querySelector(".cw-footer-icons");s.forEach(([S,w])=>{let y=o(w.name),_=document.createElement("div");_.className="cw-hero-card",_.id=`hero-${S}`,_.style.setProperty("--hero-color",y.color),_.innerHTML=`
        ...
        `,_.onclick=A=>{if(A.target.closest(".cw-step-btn"))return;let I=e[S]?e[S].count:0;z(S,I>0?-I:1,w)},_.querySelector(".minus").onclick=()=>z(S,-1,w),_.querySelector(".plus").onclick=()=>z(S,1,w),_.dataset.color=y.color,u.appendChild(_)});function N(S,w){let y=o(w.name),_=document.createElement("div");return _.className="cw-task-item",_.dataset.id=S,_.innerHTML=`
        ...
        `,_.onclick=A=>{if(A.target.closest(".cw-step-btn"))return;let I=e[S]?e[S].count:0;z(S,I>0?-I:1,w)},_.querySelector(".minus").onclick=()=>z(S,-1,w),_.querySelector(".plus").onclick=()=>z(S,1,w),_}Object.entries(i).forEach(([S,w])=>{let y=document.createElement("div");y.className="cw-acc-group";let _=document.createElement("div");_.className="cw-acc-header",_.innerHTML=`
            <div class="cw-acc-title">
                <div class="cw-acc-dot" style="background:${w.brand.color}"></div>
                ${S}
            </div>
            <div class="cw-acc-icon">\u25BC</div>
        `,_.onclick=()=>{l.querySelectorAll(".cw-acc-group.open").forEach(I=>{I!==y&&I.classList.remove("open")}),y.classList.toggle("open")};let A=document.createElement("div");A.className="cw-acc-body",w.tasks.forEach(I=>{let F=N(I.key,I);A.appendChild(F)}),y.appendChild(_),y.appendChild(A),l.appendChild(y)});function z(S,w,y){e[S]||(e[S]={count:0,data:y,brand:o(y.name)}),e[S].count+=w,e[S].count<=0&&delete e[S],$(),se(),t&&t()}function $(){s.forEach(([A])=>{let I=u.querySelector(`#hero-${A}`);if(!I)return;let F=e[A];F?(I.classList.add("active"),I.querySelector(".cw-step-val").textContent=F.count,I.querySelector(".cw-step-val").style.color=I.dataset.color):I.classList.remove("active")}),r.querySelectorAll(".cw-task-item").forEach(A=>{let I=A.dataset.id,F=e[I];F?(A.classList.add("selected"),A.querySelector(".cw-step-val").textContent=F.count):A.classList.remove("selected")});let w=Object.keys(e),y=0,_=[];if(w.forEach(A=>{let I=e[A];y+=I.count;for(let F=0;F<I.count;F++)_.length<6&&_.push(I.brand)}),y>0){m.classList.add("visible");let A=y>1?"A\xE7\xF5es":"A\xE7\xE3o",I=y>1?"definidas":"definida";E.textContent=`${y} ${A} ${I}`,R.innerHTML="",_.forEach(F=>{let q=document.createElement("div");q.className="cw-mini-icon",q.innerHTML=Ye[F.icon]||Ye.default;let k=q.querySelector("svg");k&&(k.style.width="14px",k.style.height="14px"),R.appendChild(q)})}else m.classList.remove("visible")}b.addEventListener("input",S=>{let w=S.target.value.toLowerCase();if(w.length>0){l.style.display="none",f.style.display="block",f.innerHTML="";let y=!1;Object.entries(Ge).forEach(([_,A])=>{if(A.name.toLowerCase().includes(w)){y=!0;let I=N(_,A);e[_]&&(I.classList.add("selected"),I.querySelector(".cw-step-val").textContent=e[_].count),f.appendChild(I)}}),y||(f.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else l.style.display="block",f.style.display="none"});function se(){p.innerHTML="";let S=Object.keys(e),w=!1,y=document.getElementById("sub-status"),_="implementation";if(y&&y.value.toLowerCase().includes("education")&&(_="education"),S.length===0){p.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}if(S.length===0){p.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let A=document.createElement("div");A.className="cw-info-banner",A.innerHTML=`
        ...
        `,p.appendChild(A),S.forEach(I=>{let F=e[I].data,q=e[I].count,k=e[I].brand,j=F.screenshots?F.screenshots[_]||[]:["Link da Evid\xEAncia"];if(j.length>0){w=!0;for(let g=1;g<=q;g++){let C=document.createElement("div");C.className="cw-screen-card",C.style.setProperty("--brand-color",k.color),C.style.setProperty("--brand-bg",k.bg),C.style.setProperty("--brand-shadow",k.color+"40");let h=document.createElement("div");h.className="cw-card-header";let c=document.createElement("div");c.className="cw-card-icon",c.innerHTML=Ye[k.icon]||Ye.default;let v=document.createElement("div");v.style.cssText="flex:1; display:flex; align-items:center; gap:8px;";let T=document.createElement("input");T.className="cw-card-title-input",T.id=`name-${I}-${g}`,T.value=`${F.name}${q>1?" #"+g:""}`,T.title="Clique para renomear esta task";let H=document.createElement("span");H.className="cw-edit-hint",H.innerHTML="\u270E Renomear",v.appendChild(T),v.appendChild(H),h.appendChild(c),h.appendChild(v),C.appendChild(h),j.forEach((D,M)=>{let G=document.createElement("div");G.className="cw-input-group";let U=document.createElement("label");U.className="cw-input-label",U.textContent=D.replace(/📷|:|•/g,"").trim();let O=document.createElement("input");O.className="cw-input-field",O.id=`screen-${I}-${g}-${M}`,O.placeholder="Cole o link aqui...",O.setAttribute("autocomplete","off"),O.addEventListener("input",()=>{O.value.trim().length>5?O.classList.add("filled"):O.classList.remove("filled")});let P=document.createElement("div");P.className="cw-input-check",P.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>',G.appendChild(U),G.appendChild(O),G.appendChild(P),C.appendChild(G)}),p.appendChild(C)}}}),d.style.display=w?"block":"none"}return{selectionElement:r,screenshotsElement:d,updateSubStatus:()=>se(),getCheckedElements:()=>Object.keys(e).map(S=>({value:S,closest:()=>({querySelector:()=>({textContent:e[S].count})})})),toggleTask:(S,w=!0)=>{let y=e[S];w&&!y?z(S,1,Ge[S]):!w&&y&&z(S,-y.count,Ge[S])},setMode:S=>{n=S,se()},reset:()=>{for(let S in e)delete e[S];b.value="",l.style.display="block",f.style.display="none",$(),se()}}}function vo(t){let e=document.createElement("div");e.style.cssText="display: flex; flex-direction: column; height: 100%; width: 100%; background: #F8F9FA; overflow: hidden;";let n=document.createElement("div");n.style.cssText="flex: 1; overflow-y: auto; padding: 20px 24px 100px 24px;";let o={sectionTitle:`
            font-family: 'Google Sans', Roboto, sans-serif;
            font-size: 11px; 
            font-weight: 700; 
            color: #5F6368; 
        ...
        `};function s(k,j,g="text",C=""){let h=document.createElement("div");h.style.cssText=o.inputWrapper;let c=document.createElement("label");c.style.cssText=o.label,c.textContent=j;let v;g==="textarea"?(v=document.createElement("textarea"),v.style.cssText=o.input+o.textarea):(v=document.createElement("input"),v.type=g,v.style.cssText=o.input);let T=v.style.cssText;return v.onfocus=()=>v.style.cssText=T+o.inputFocus,v.onblur=()=>v.style.cssText=T,v.id=`st-${k}`,v.placeholder=C,h.appendChild(c),h.appendChild(v),{wrapper:h,input:v}}function i(k,j){let g=document.createElement("div"),C=document.createElement("label");C.style.cssText=o.label,C.textContent=j,g.appendChild(C);let h=document.createElement("div");return h.style.cssText=o.radioGroup,["Yes","No"].forEach(c=>{let v=document.createElement("label");v.style.cssText=o.radioLabel;let T=document.createElement("input");T.type="radio",T.name=`st-${k}`,T.value=c==="Yes"?"Y":"N",T.style.display="none",c==="No"&&(T.checked=!0),v.onmousedown=()=>v.style.transform="scale(0.96)",v.onmouseup=()=>v.style.transform="scale(1)",v.onmouseleave=()=>v.style.transform="scale(1)",v.appendChild(T),v.appendChild(document.createTextNode(c)),T.addEventListener("change",()=>{h.querySelectorAll("label").forEach(H=>H.style.cssText=o.radioLabel),T.checked&&(v.style.cssText=o.radioLabel+o.radioActive)}),c==="No"&&(v.style.cssText=o.radioLabel+o.radioActive),h.appendChild(v)}),g.appendChild(h),{wrapper:g}}let a=document.createElement("div");a.style.cssText=o.banner,a.innerHTML=`
    ...
    `,n.appendChild(a);let r=document.createElement("button");r.style.cssText=o.magicBtn,r.innerHTML='<span style="font-size:16px">\u2728</span> Preencher Automaticamente',r.onmouseover=()=>r.style.backgroundColor="#F8F9FA",r.onmouseout=()=>r.style.backgroundColor="#FFFFFF",r.onmousedown=()=>r.style.transform="scale(0.98)",r.onmouseup=()=>r.style.transform="scale(1)",n.appendChild(r);let d=document.createElement("div");d.style.cssText=o.sectionTitle,d.style.marginTop="24px",d.innerHTML="<span>\u{1F6E0}\uFE0F</span> Dados T\xE9cnicos",n.appendChild(d);let p=s("cid","Ads CID","text","000-000-0000"),u=s("ga4","GA4 ID"),l=s("gtm","GTM ID"),f=i("access","Advertiser has access to GA4/GTM?"),b=s("access-email","If Yes, User Email"),m=i("ghost","Ghosting Access Available?");n.append(p.wrapper,u.wrapper,l.wrapper,f.wrapper,b.wrapper,m.wrapper);let E=document.createElement("div");E.style.cssText=o.sectionTitle,E.innerHTML="<span>\u{1F4DE}</span> Contato & Problema",n.appendChild(E);let R=s("name","Name of Advertiser"),N=s("url","Website Address"),z=s("phone","Phone Number"),$=s("email","Email Address"),se=s("callback","Preferred Call Back Time (w/ Timezone)"),S=s("desc","Detailed Issue Description","textarea","Descreva o problema t\xE9cnico em detalhes..."),w=s("checks","Checks Performed by Tech Team","textarea","Liste o troubleshooting j\xE1 realizado..."),y=s("screens","Uncropped Screenshots (Links)","textarea","https://...");n.append(R.wrapper,N.wrapper,z.wrapper,$.wrapper,se.wrapper,S.wrapper,w.wrapper,y.wrapper);let _=document.createElement("div");_.style.cssText=o.sectionTitle,_.innerHTML="<span>\u{1F4E7}</span> Contatos para C\xF3pia (CC)",n.appendChild(_);let A=s("c-adv","Advertiser Contact"),I=s("c-am","Account Manager");n.append(A.wrapper,I.wrapper);let F=document.createElement("div");F.style.cssText="padding: 16px 24px; background: rgba(255,255,255,0.9); backdrop-filter: blur(8px); border-top: 1px solid #E0E0E0; display: flex; justify-content: flex-end; position: absolute; bottom: 0; width: 100%; box-sizing: border-box; z-index: 100;";let q=document.createElement("button");return q.textContent="Gerar Nota S&T",q.style.cssText="padding: 10px 24px; background: #1a73e8; color: #fff; border: none; border-radius: 24px; font-size: 14px; font-weight: 600; cursor: pointer; box-shadow: 0 2px 6px rgba(26, 115, 232, 0.3); transition: transform 0.1s, box-shadow 0.2s;",q.onmousedown=()=>{q.style.transform="scale(0.96)",q.style.boxShadow="0 1px 3px rgba(26, 115, 232, 0.2)"},q.onmouseup=()=>{q.style.transform="scale(1)",q.style.boxShadow="0 2px 6px rgba(26, 115, 232, 0.3)"},F.appendChild(q),e.appendChild(n),e.appendChild(F),r.onclick=async()=>{r.innerHTML='<span style="font-size:16px">\u23F3</span> Buscando...';let k=await at();k.advertiserName&&(R.input.value=k.advertiserName),k.websiteUrl&&(N.input.value=k.websiteUrl),k.clientEmail&&($.input.value=k.clientEmail,A.input.value=k.clientEmail);let g=document.body.innerText.match(/\d{3}-\d{3}-\d{4}/);g&&(p.input.value=g[0]),r.innerHTML='<span style="font-size:16px; color:#188038">\u2705</span> Dados Preenchidos!',r.style.background="#E6F4EA",r.style.borderColor="#188038",setTimeout(()=>{r.innerHTML='<span style="font-size:16px">\u2728</span> Preencher Automaticamente',r.style.background="#FFFFFF",r.style.borderColor="#DADCE0"},2e3),Y("Dados capturados com sucesso!")},q.onclick=async()=>{let k=c=>{let v=e.querySelector(`#st-${c}`);return v?v.value:""},j=c=>{let v=e.querySelector(`input[name="st-${c}"]:checked`);return v?v.value:"N"},C=`Split & Transfer : Phone Note Format [Mandatory]
...
<b>If Yes, user access email to GA4/GTM:</b> ${k("access-email")}
<b>Ghosting Access Available (Y/N):</b> ${j("ghost")}
<b>Name of the advertiser:</b> ${k("name")}
<b>Website Address:</b> ${k("url")}
<b>Advertiser\u2019s preferred mode of communication:</b> Phone
<b>Advertiser/Web Master\u2019s Phone Number:</b> ${k("phone")}
<b>Preferred Call Back time with time zone and contact number:</b> ${k("callback")}
<b>Advertiser/Web Master\u2019s Email Address:</b> ${k("email")}
...
`).filter(Q=>Q.trim()!=="").map(Q=>`<p style="margin: 0 0 8px 0;">${Q}</p>`).join(""):te.tagName==="TEXTAREA"&&(ee=ee.replace(/\n/g,"<br>"));let ue=ee.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(ue===""||ue==="\u2022"||ue.toLowerCase()==="n/a"){let Q=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${W}\\}(?:<br>\\s*)?`,"gi");Q.test(L)?L=L.replace(Q,""):L=L.replace(B,"")}else L=L.replace(B,ee.replace(/\$/g,"$$$$"))}),L=L.replace(/{([A-Z0-9_]+)}/g,""),L=L.replace(/(<br>){3,}/g,"<br><br>"),typeof r<"u"&&r.getOutput&&(L+=r.getOutput()),L}tt.onclick=()=>{let x=Ht();x?(ct(x),Y(Z("copiado_sucesso"))):Y(Z("selecione_substatus"),{error:!0})},ot.onclick=async()=>{let x=c.value,V=Ht();if(!V){Y(Z("selecione_substatus"),{error:!0});return}ct(V),It();let L=xt(),K=await wt();if(K)try{if(K.focus(),K.innerHTML.trim()==="<p><br></p>"||K.innerHTML.trim()==="<br>"||K.innerText.trim()===""){let ge=document.createRange();ge.selectNodeContents(K);let ie=window.getSelection();ie.removeAllRanges(),ie.addRange(ge),document.execCommand("delete",!1,null)}else if(!K.innerHTML.endsWith("<br><br>")){let ge=document.createRange();ge.selectNodeContents(K),ge.collapse(!1);let ie=window.getSelection();ie.removeAllRanges(),ie.addRange(ge),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,V),vt(K),setTimeout(()=>{Y(Z("inserido_copiado"))},600);let me=typeof Ve<"u"&&Ve?Ve.checked:!0;if(x&&We[x]&&me){let ge=We[x];await ft(ge),await new Promise(ie=>setTimeout(ie,500))}L(),Ot(1.5),j.value="",c.innerHTML=`<option value="">${Z("select_substatus")}</option>`,c.disabled=!0}catch(re){console.error(re),Y("Erro ao inserir.",{error:!0}),L()}};function Ot(x=1.5){x<=1.5&&(Be.style.display="none",Ee.innerHTML=""),x<=2&&(d.reset(),he.style.display="none"),x<=3&&(Ae.style.display="none",De.innerHTML="",r.reset(),Ke.style.display="none",je.style.display="none")}function It(){if(s=!s,s){let x=p.querySelector(".cw-expand-btn");x&&typeof x.resetState=="function"&&x.resetState()}qe(s,p,"cw-btn-notes")}return St("bau"),Et("pt"),It}var Xe={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
            ...
            <div style="position:sticky; top:0; background:rgba(255,255,255,0.9); backdrop-filter:blur(12px); border-bottom:1px solid #eee; padding:16px 24px; z-index:10; display:flex; align-items:center; gap:12px;">
                ...
                <div style="font-weight:600; font-size:16px; color:#202124; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${T.name}</div>
            </div>
            ...
            <div style="padding:24px;">
                <div style="margin-bottom:24px;">
                    ...
                    <div style="font-size:14px; font-weight:500; color:#202124; padding:16px; background:#F8F9FA; border-radius:12px; border:1px solid #eee; box-shadow:inset 0 1px 2px rgba(0,0,0,0.02);">${T.subject}</div>
                </div>
        ...
        `;let M=k.querySelector("#csa-back-btn");M.onmouseenter=()=>M.style.background="#f1f3f4",M.onmouseleave=()=>M.style.background="none",M.onclick=C;let G=k.querySelector("#csa-insert-btn");G.onmouseenter=()=>{G.style.transform="translateY(-1px)",G.style.boxShadow="0 6px 16px rgba(26,115,232,0.4)"},G.onmouseleave=()=>{G.style.transform="translateY(0)",G.style.boxShadow="0 4px 12px rgba(26,115,232,0.3)"},G.onclick=()=>{G.style.transform="scale(0.96)",j(T,"email"),setTimeout(()=>{G.style.transform="scale(1)",C()},300)}}function C(){s="list",w.style.transform="translateX(0)"}function h(T,H,D=null){let M=document.createElement("button"),G=D?`<span style="margin-right:6px; font-size:14px; opacity:0.9;">${D}</span>`:"";return M.innerHTML=`${G}${T}`,Object.assign(M.style,b),n===H&&o===""?Object.assign(M.style,m):(M.onmouseenter=()=>{M.style.background="#F1F3F4",M.style.borderColor="#DADCE0"},M.onmouseleave=()=>{M.style.background="#FFFFFF",M.style.borderColor="#DADCE0"}),M.onclick=()=>{n=H,o="",A.value="",c(),v()},M}function c(){I.innerHTML="",I.appendChild(h("Smart CRs",e,"\u26A1")),Object.keys(Xe).forEach(T=>{I.appendChild(h(Xe[T].title,T))})}function v(){F.innerHTML="";let T=[];if(o.trim()!==""){let U=o.toLowerCase();Object.values(Xe).forEach(O=>{O.emails.forEach(P=>{(P.name.toLowerCase().includes(U)||P.subject.toLowerCase().includes(U))&&T.push({type:"email",data:P})})}),Object.entries(We).forEach(([O,P])=>{if(!P)return;(O.replace(/_/g," ").toLowerCase().includes(U)||P.toLowerCase().includes(U))&&T.push({type:"cr",key:O,code:P})})}else n===e?Object.entries(We).forEach(([U,O])=>{O&&T.push({type:"cr",key:U,code:O})}):Xe[n]&&Xe[n].emails.forEach(U=>{T.push({type:"email",data:U})});if(T.length===0){F.innerHTML=`
                ...
                `,O.onclick=()=>g(P)}else{let P=U.key.replace(/_/g," ").replace("AS ","AS - ").replace("NI ","NI - ");O.innerHTML=`
 ...
 D) Nombre del sitio web.`,"autenticar la cuenta del anunciante en el cases, si corresponde.","T\xE9rminos y condi\xE7\xF5es.","Informar las Task solicitadas y AM.","Cerrar contenido sensible.","Confirmaci\xF3n de copia de seguridad y acceso de administrador a las ferramentas.","Resumen de llamada."],fim:["Ofrecer ayuda adicional.","Dejar de compartir la pantalla.","Pasos siguientes  (Si se le har\xE1 seguimiento al caso).","Encuesta de Satisfa\xE7\xE3o.","Informar al cliente que el equipo de QA ir\xE1 a realizar pruebas en los siguientes dias."]},"EN BAU":{color:"#ff0011",inicio:["Example 1","Example 2"],fim:["Example 3","Example 4"]}};function So(){let t="v2.1 (Apple Motion)",e={progressBarContainer:{height:"4px",background:"#f1f3f4",width:"100%",position:"relative",overflow:"hidden"},progressBarFill:{height:"100%",background:"linear-gradient(90deg, #4285F4, #34A853)",width:"0%",transition:"width 0.5s cubic-bezier(0.25, 0.8, 0.25, 1)",borderRadius:"0 2px 2px 0"},contentArea:{padding:"16px",overflowY:"auto",flexGrow:"1",background:"#f8f9fa",scrollBehavior:"smooth"},card:{background:"#ffffff",border:"1px solid #dadce0",borderRadius:"12px",padding:"16px",marginBottom:"16px",transition:"transform 0.2s ease, box-shadow 0.2s ease",boxShadow:"0 1px 2px rgba(0,0,0,0.02)"},cardTitle:{fontSize:"13px",fontWeight:"700",color:"#5f6368",textTransform:"uppercase",letterSpacing:"0.6px",marginBottom:"12px",display:"flex",alignItems:"center",justifyContent:"space-between",userSelect:"none"},itemRow:{display:"flex",alignItems:"flex-start",padding:"10px 8px",cursor:"pointer",borderRadius:"8px",transition:"background-color 0.15s ease, opacity 0.3s ease",color:"#202124",fontSize:"14px",lineHeight:"1.5",marginBottom:"2px"},itemCompleted:{opacity:"0.5",textDecoration:"line-through",color:"#5f6368"},checkbox:{minWidth:"20px",height:"20px",borderRadius:"6px",border:"2px solid #dadce0",marginRight:"14px",marginTop:"1px",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)",background:"#fff"},footer:{padding:"12px 16px",borderTop:"1px solid #eee",background:"#fff",display:"flex",justifyContent:"space-between",alignItems:"center"},resetBtn:{background:"transparent",border:"none",color:"#d93025",fontSize:"12px",fontWeight:"600",cursor:"pointer",padding:"6px 12px",borderRadius:"20px",transition:"background 0.2s ease",display:"flex",alignItems:"center",gap:"4px"}},n={},o="PT",s="BAU",i=!1,a=document.createElement("div");a.id="call-script-popup",a.classList.add("cw-module-window"),Object.assign(a.style,Te,{right:"auto",left:"50%",width:"400px",height:"650px",display:"flex",flexDirection:"column",transform:"translateX(-50%) scale(0.05)"});let r={popup:a,googleLine:null};function d(){i=!i,qe(i,a,"cw-btn-script")}let p=Ie(a,"Call Script",t,"Guia interativo para condu\xE7\xE3o de chamadas.",r,()=>{d()});a.appendChild(p);let u=document.createElement("div");Object.assign(u.style,e.progressBarContainer);let l=document.createElement("div");Object.assign(l.style,e.progressBarFill),u.appendChild(l),a.appendChild(u);let f=document.createElement("div");f.id="csa-content",Object.assign(f.style,e.contentArea),a.appendChild(f);let b=document.createElement("div");Object.assign(b.style,e.footer);let m=document.createElement("span");m.textContent="by lucaste@",Object.assign(m.style,{fontSize:"10px",color:"#bdc1c6"});let E=document.createElement("button");E.innerHTML=`
  ...
  `,Object.assign(E.style,e.resetBtn),E.onmouseenter=()=>E.style.background="#fce8e6",E.onmouseleave=()=>E.style.background="transparent",E.onclick=()=>{E.style.transform="scale(0.9)",setTimeout(()=>E.style.transform="scale(1)",150);for(let q in n)delete n[q];_()},b.appendChild(m),b.appendChild(E),a.appendChild(b);let R=document.createElement("div");Object.assign(R.style,{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"16px",gap:"8px"});let N=document.createElement("div");Object.assign(N.style,{display:"flex",borderRadius:"8px",border:"1px solid #dadce0",overflow:"hidden",background:"#fff"});let z=document.createElement("div");z.textContent="BAU";let $=document.createElement("div");$.textContent="LT",Object.assign(z.style,ye),Object.assign($.style,ye),N.appendChild(z),N.appendChild($);let se=document.createElement("select");Object.assign(se.style,oo,{marginBottom:"0",width:"auto",minWidth:"90px",paddingTop:"6px",paddingBottom:"6px",paddingRight:"30px",height:"32px",backgroundPosition:"right 8px center"}),se.innerHTML='<option value="PT">PT</option><option value="ES">ES</option><option value="EN">EN</option>',se.value=o,R.appendChild(N),R.appendChild(se),f.appendChild(R);let S=document.createElement("div");S.id="csa-checklist-area",f.appendChild(S);let w=document.createElement("div");Object.assign(w.style,Qe),w.className="no-drag",w.title="Redimensionar",a.appendChild(w),Ze(a,w),document.body.appendChild(a);function y(q){return q}function _(){S.innerHTML="";let q=`${o} ${s}`,k=Co[q];if(!k){S.innerHTML=`<div style="padding: 30px; text-align: center; color: #bdc1c6; display: flex; flex-direction: column; align-items: center; gap: 10px;">
      ...
      `,m.querySelectorAll('input[name="cw-bc-type"]').forEach(c=>{c.addEventListener("change",()=>{m.querySelectorAll(".cw-radio-option").forEach(v=>v.classList.remove("checked")),c.parentElement.classList.add("checked")})}),setTimeout(()=>{let c=m.querySelector(".cw-radio-option.info");c&&c.classList.add("checked")},100);let g=m.querySelector("#cw-bc-cancel"),C=m.querySelector("#cw-bc-close-x"),h=m.querySelector("#cw-bc-send");g.onclick=$,C.onclick=$,h.onclick=se,p.appendChild(m)}function z(g=null){if(!m)return;let C=m.querySelector("#cw-editor-title-label"),h=m.querySelector("#cw-bc-title"),c=m.querySelector("#cw-bc-text"),v=m.querySelector("#cw-bc-send");if(g){o=g.id,C.textContent="Editar Aviso",h.value=g.title||"",c.value=g.text||"",v.textContent="Salvar Altera\xE7\xF5es";let T=g.type||"info",H=m.querySelector(`input[name="cw-bc-type"][value="${T}"]`);H&&H.click()}else{o=null,C.textContent="Novo Aviso",h.value="",c.value="",v.textContent="Publicar";let T=m.querySelector('input[name="cw-bc-type"][value="info"]');T&&T.click()}m.classList.add("active"),setTimeout(()=>h.focus(),300)}function $(){m&&m.classList.remove("active"),o=null}async function se(){let g=m.querySelector("#cw-bc-send"),C=m.querySelector("#cw-bc-title"),h=m.querySelector("#cw-bc-text"),c=m.querySelector('input[name="cw-bc-type"]:checked'),v=c?c.value:"info";if(!C.value.trim()||!h.value.trim()){Y("Preencha todos os campos!",{error:!0});return}g.textContent="Salvando...",g.style.opacity="0.7";let T=!1;o?T=await we.updateBroadcast(o,{title:C.value,text:h.value,type:v}):T=await we.sendBroadcast({title:C.value,text:h.value,type:v,author:window._cwCurrentUser||"admin"}),T?(Y(o?"Atualizado!":"Publicado!"),oe.playSuccess(),$(),setTimeout(()=>y(),1500)):(Y("Erro ao salvar. Verifique a conex\xE3o.",{error:!0}),g.textContent=o?"Salvar Altera\xE7\xF5es":"Publicar",g.style.opacity="1")}async function S(g){if(confirm("Confirma a exclus\xE3o deste aviso?"))if(await we.deleteBroadcast(g)){Y("Aviso removido."),oe.playClick();let h=ze.findIndex(c=>c.id===g);h>-1&&ze.splice(h,1),A(),setTimeout(()=>y(),1500)}else Y("Erro ao excluir.",{error:!0})}let w=document.createElement("div");w.className="cw-nice-scroll",Object.assign(w.style,i.feedContainer),p.appendChild(w);async function y(){e&&(R.style.display="block",R.innerHTML="\u{1F504} Sincronizando...");try{let g=await we.fetchData();g&&g.broadcast&&(Pt(g.broadcast),_(),e&&(A(),R.innerHTML='<span style="color:#137333">\u2713 Atualizado</span>',setTimeout(()=>{R.style.display="none"},1500)))}catch{e&&(R.innerHTML="\u26A0\uFE0F Offline")}}function _(){let g=document.getElementById("cw-btn-broadcast");if(!g)return;let C=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]");if(ze.some(c=>!C.includes(c.id))){if(g.classList.add("has-new"),!g.querySelector(".cw-badge")){let c=document.createElement("div");c.className="cw-badge",Object.assign(c.style,{position:"absolute",top:"8px",right:"8px",width:"8px",height:"8px",backgroundColor:"#d93025",borderRadius:"50%",border:"1px solid #fff",zIndex:"10"}),g.appendChild(c)}}else{g.classList.remove("has-new");let c=g.querySelector(".cw-badge");c&&c.remove()}}function A(){w.innerHTML="";let g=p.querySelector("#cw-bau-widget");g&&g.remove();let C=JSON.parse(localStorage.getItem("cw_read_broadcasts")||"[]"),h=[...ze].sort((D,M)=>{let G=new Date(D.date).getTime()||0;return(new Date(M.date).getTime()||0)-G}),c=h.findIndex(D=>D.title&&D.title.toLowerCase().includes("disponibilidade bau"));if(c!==-1){let D=h[c];h.splice(c,1);let M=document.createElement("div");M.id="cw-bau-widget",Object.assign(M.style,i.bauContainer);let G=[],U=(D.text||"").split(`
`),O=/\d{1,2}\/\d{1,2}/;if(U.forEach(ce=>{let fe=ce.match(O);if(fe){let Se=fe[0],_e="\u{1F4C5}";/🇧🇷|🇵🇹|PT|BR/i.test(ce)?_e="\u{1F1E7}\u{1F1F7}":/🇪🇸|🇲🇽|ES|LATAM/i.test(ce)&&(_e="\u{1F1EA}\u{1F1F8}"),G.some(Re=>Re.flag===_e&&Re.date===Se)||G.push({flag:_e,date:Se})}}),G.length===0){let ce=(D.text||"").match(/\d{1,2}\/\d{1,2}/g);ce&&[...new Set(ce)].forEach(fe=>G.push({flag:"\u{1F4C5}",date:fe}))}let P="",de='<button id="cw-bau-toggle-btn" class="cw-btn-interactive" style="background:rgba(255,255,255,0.7); border:1px solid rgba(139, 92, 246, 0.4); border-radius:12px; padding:8px 12px; color:#6D28D9; font-size:12px; font-weight:600;">Detalhes</button>';window._cwIsAdmin&&(de=`
