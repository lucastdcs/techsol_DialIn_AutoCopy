...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let u=r.querySelector(".cw-hero-grid"),c=r.querySelector(".cw-acc-container"),m=r.querySelector(".cw-results-container"),T=r.querySelector(".cw-search-input"),h=r.querySelector(".cw-status-bar"),g=r.querySelector(".cw-status-text"),S=r.querySelector(".cw-footer-icons");s.forEach(([C,E])=>{let q=o(E.name),z=document.createElement("div");z.className="cw-hero-card",z.id=`hero-${C}`,z.style.setProperty("--hero-color",q.color),z.innerHTML=`
        ...
        `,z.onclick=L=>{if(L.target.closest(".cw-step-btn"))return;let R=t[C]?t[C].count:0;G(C,R>0?-R:1,E)},z.querySelector(".minus").onclick=()=>G(C,-1,E),z.querySelector(".plus").onclick=()=>G(C,1,E),z.dataset.color=q.color,u.appendChild(z)});function _(C,E){let q=o(E.name),z=document.createElement("div");return z.className="cw-task-item",z.dataset.id=C,z.innerHTML=`
        ...
        `,z.onclick=L=>{if(L.target.closest(".cw-step-btn"))return;let R=t[C]?t[C].count:0;G(C,R>0?-R:1,E)},z.querySelector(".minus").onclick=()=>G(C,-1,E),z.querySelector(".plus").onclick=()=>G(C,1,E),z}Object.entries(i).forEach(([C,E])=>{let q=document.createElement("div");q.className="cw-acc-group";let z=document.createElement("div");z.className="cw-acc-header",z.innerHTML=`
            <div class="cw-acc-title">
                <div class="cw-acc-dot" style="background:${E.brand.color}"></div>
                ${C}
            </div>
            <div class="cw-acc-icon">\u25BC</div>
        `,z.onclick=()=>{c.querySelectorAll(".cw-acc-group.open").forEach(R=>{R!==q&&R.classList.remove("open")}),q.classList.toggle("open")};let L=document.createElement("div");L.className="cw-acc-body",E.tasks.forEach(R=>{let O=_(R.key,R);L.appendChild(O)}),q.appendChild(z),q.appendChild(L),c.appendChild(q)});function G(C,E,q){t[C]||(t[C]={count:0,data:q,brand:o(q.name)}),t[C].count+=E,t[C].count<=0&&delete t[C],Z(),oe(),e&&e()}function Z(){s.forEach(([L])=>{let R=u.querySelector(`#hero-${L}`);if(!R)return;let O=t[L];O?(R.classList.add("active"),R.querySelector(".cw-step-val").textContent=O.count,R.querySelector(".cw-step-val").style.color=R.dataset.color):R.classList.remove("active")}),r.querySelectorAll(".cw-task-item").forEach(L=>{let R=L.dataset.id,O=t[R];O?(L.classList.add("selected"),L.querySelector(".cw-step-val").textContent=O.count):L.classList.remove("selected")});let E=Object.keys(t),q=0,z=[];if(E.forEach(L=>{let R=t[L];q+=R.count;for(let O=0;O<R.count;O++)z.length<6&&z.push(R.brand)}),q>0){h.classList.add("visible");let L=q>1?"A\xE7\xF5es":"A\xE7\xE3o",R=q>1?"definidas":"definida";g.textContent=`${q} ${L} ${R}`,S.innerHTML="",z.forEach(O=>{let x=document.createElement("div");x.className="cw-mini-icon",x.innerHTML=Xe[O.icon]||Xe.default;let f=x.querySelector("svg");f&&(f.style.width="14px",f.style.height="14px"),S.appendChild(x)})}else h.classList.remove("visible")}T.addEventListener("input",C=>{let E=C.target.value.toLowerCase();if(E.length>0){c.style.display="none",m.style.display="block",m.innerHTML="";let q=!1;Object.entries(ze).forEach(([z,L])=>{if(L.name.toLowerCase().includes(E)){q=!0;let R=_(z,L);t[z]&&(R.classList.add("selected"),R.querySelector(".cw-step-val").textContent=t[z].count),m.appendChild(R)}}),q||(m.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else c.style.display="block",m.style.display="none"});function oe(){d.innerHTML="";let C=Object.keys(t),E=!1,q=document.getElementById("sub-status"),z="implementation";if(q&&q.value.toLowerCase().includes("education")&&(z="education"),C.length===0){d.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}if(C.length===0){d.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let L=document.createElement("div");L.className="cw-info-banner",L.innerHTML=`
        ...
        `,d.appendChild(L),C.forEach(R=>{let O=t[R].data,x=t[R].count,f=t[R].brand,N=O.screenshots?O.screenshots[z]||[]:["Link da Evid\xEAncia"];if(N.length>0){E=!0;for(let l=1;l<=x;l++){let v=document.createElement("div");v.className="cw-screen-card",v.style.setProperty("--brand-color",f.color),v.style.setProperty("--brand-bg",f.bg),v.style.setProperty("--brand-shadow",f.color+"40");let w=document.createElement("div");w.className="cw-card-header";let b=document.createElement("div");b.className="cw-card-icon",b.innerHTML=Xe[f.icon]||Xe.default;let A=document.createElement("div");A.style.cssText="flex:1; display:flex; align-items:center; gap:8px;";let I=document.createElement("input");I.className="cw-card-title-input",I.id=`name-${R}-${l}`,I.value=`${O.name}${x>1?" #"+l:""}`,I.title="Clique para renomear esta task";let j=document.createElement("span");j.className="cw-edit-hint",j.innerHTML="\u270E Renomear",A.appendChild(I),A.appendChild(j),w.appendChild(b),w.appendChild(A),v.appendChild(w),N.forEach((D,k)=>{let B=document.createElement("div");B.className="cw-input-group";let U=document.createElement("label");U.className="cw-input-label",U.textContent=D.replace(/📷|:|•/g,"").trim();let F=document.createElement("input");F.className="cw-input-field",F.id=`screen-${R}-${l}-${k}`,F.placeholder="Cole o link aqui...",F.setAttribute("autocomplete","off"),F.addEventListener("input",()=>{F.value.trim().length>5?F.classList.add("filled"):F.classList.remove("filled")});let H=document.createElement("div");H.className="cw-input-check",H.innerHTML='<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>',B.appendChild(U),B.appendChild(F),B.appendChild(H),v.appendChild(B)}),d.appendChild(v)}}}),p.style.display=E?"block":"none"}return{selectionElement:r,screenshotsElement:p,updateSubStatus:()=>oe(),getCheckedElements:()=>Object.keys(t).map(C=>({value:C,closest:()=>({querySelector:()=>({textContent:t[C].count})})})),toggleTask:(C,E=!0)=>{let q=t[C];E&&!q?G(C,1,ze[C]):!E&&q&&G(C,-q.count,ze[C])},setMode:C=>{n=C,oe()},reset:()=>{for(let C in t)delete t[C];T.value="",c.style.display="block",m.style.display="none",Z(),oe()}}}function Ao(e){let t=document.createElement("div");t.style.cssText="display: flex; flex-direction: column; height: 100%; width: 100%; background: #F8F9FA; overflow: hidden; position: relative;";let n=document.createElement("div");n.style.cssText="flex: 1; overflow-y: auto; padding: 20px 24px 80px 24px; min-height: 0;";let o={sectionTitle:`
            font-family: 'Google Sans', Roboto, sans-serif;
            font-size: 11px; 
            font-weight: 700; 
            color: #5F6368; 
        ...
        `};function s(f,N,l="text",v=""){let w=document.createElement("div");w.style.cssText=o.inputWrapper;let b=document.createElement("label");b.style.cssText=o.label,b.textContent=N;let A;l==="textarea"?(A=document.createElement("textarea"),A.style.cssText=o.input+o.textarea):(A=document.createElement("input"),A.type=l,A.style.cssText=o.input);let I=A.style.cssText;return A.onfocus=()=>A.style.cssText=I+o.inputFocus,A.onblur=()=>A.style.cssText=I,A.id=`st-${f}`,A.placeholder=v,w.appendChild(b),w.appendChild(A),{wrapper:w,input:A}}function i(f,N){let l=document.createElement("div"),v=document.createElement("label");v.style.cssText=o.label,v.textContent=N,l.appendChild(v);let w=document.createElement("div");return w.style.cssText=o.radioGroup,["Yes","No"].forEach(b=>{let A=document.createElement("label");A.style.cssText=o.radioLabel;let I=document.createElement("input");I.type="radio",I.name=`st-${f}`,I.value=b==="Yes"?"Y":"N",I.style.display="none",b==="No"&&(I.checked=!0),A.onmousedown=()=>A.style.transform="scale(0.96)",A.onmouseup=()=>A.style.transform="scale(1)",A.onmouseleave=()=>A.style.transform="scale(1)",A.appendChild(I),A.appendChild(document.createTextNode(b)),I.addEventListener("change",()=>{w.querySelectorAll("label").forEach(j=>j.style.cssText=o.radioLabel),I.checked&&(A.style.cssText=o.radioLabel+o.radioActive)}),b==="No"&&(A.style.cssText=o.radioLabel+o.radioActive),w.appendChild(A)}),l.appendChild(w),{wrapper:l}}let a=document.createElement("div");a.style.cssText=o.banner,a.innerHTML=`
    ...
    `,n.appendChild(a);let r=document.createElement("button");r.style.cssText=o.magicBtn,r.innerHTML='<span style="font-size:16px">\u2728</span> Preencher Automaticamente',r.onmouseover=()=>r.style.backgroundColor="#F8F9FA",r.onmouseout=()=>r.style.backgroundColor="#FFFFFF",r.onmousedown=()=>r.style.transform="scale(0.98)",r.onmouseup=()=>r.style.transform="scale(1)",n.appendChild(r);let p=document.createElement("div");p.style.cssText=o.sectionTitle,p.style.marginTop="24px",p.innerHTML="<span>\u{1F6E0}\uFE0F</span> Dados T\xE9cnicos",n.appendChild(p);let d=s("cid","Ads CID","text","000-000-0000"),u=s("ga4","GA4 ID"),c=s("gtm","GTM ID"),m=i("access","Advertiser has access to GA4/GTM?"),T=s("access-email","If Yes, User Email"),h=i("ghost","Ghosting Access Available?");n.append(d.wrapper,u.wrapper,c.wrapper,m.wrapper,T.wrapper,h.wrapper);let g=document.createElement("div");g.style.cssText=o.sectionTitle,g.innerHTML="<span>\u{1F4DE}</span> Contato & Problema",n.appendChild(g);let S=s("name","Name of Advertiser"),_=s("url","Website Address"),G=s("phone","Phone Number"),Z=s("email","Email Address"),oe=s("callback","Preferred Call Back Time (w/ Timezone)"),C=s("desc","Detailed Issue Description","textarea","Descreva o problema t\xE9cnico em detalhes..."),E=s("checks","Checks Performed by Tech Team","textarea","Liste o troubleshooting j\xE1 realizado..."),q=s("screens","Uncropped Screenshots (Links)","textarea","https://...");n.append(S.wrapper,_.wrapper,G.wrapper,Z.wrapper,oe.wrapper,C.wrapper,E.wrapper,q.wrapper);let z=document.createElement("div");z.style.cssText=o.sectionTitle,z.innerHTML="<span>\u{1F4E7}</span> Contatos para C\xF3pia (CC)",n.appendChild(z);let L=s("c-adv","Advertiser Contact"),R=s("c-am","Account Manager");n.append(L.wrapper,R.wrapper);let O=document.createElement("div");O.style.cssText="padding: 16px 24px; background: rgba(255,255,255,0.9); backdrop-filter: blur(8px); border-top: 1px solid #E0E0E0; display: flex; justify-content: flex-end; position: absolute; bottom: 0; left: 0; width: 100%; box-sizing: border-box; z-index: 100;";let x=document.createElement("button");return x.textContent="Gerar Nota S&T",x.style.cssText="padding: 10px 24px; background: #1a73e8; color: #fff; border: none; border-radius: 24px; font-size: 14px; font-weight: 600; cursor: pointer; box-shadow: 0 2px 6px rgba(26, 115, 232, 0.3); transition: transform 0.1s, box-shadow 0.2s;",x.onmousedown=()=>{x.style.transform="scale(0.96)",x.style.boxShadow="0 1px 3px rgba(26, 115, 232, 0.2)"},x.onmouseup=()=>{x.style.transform="scale(1)",x.style.boxShadow="0 2px 6px rgba(26, 115, 232, 0.3)"},O.appendChild(x),t.appendChild(n),t.appendChild(O),r.onclick=async()=>{r.innerHTML='<span style="font-size:16px">\u23F3</span> Buscando...';let f=await it();f.advertiserName&&(S.input.value=f.advertiserName),f.websiteUrl&&(_.input.value=f.websiteUrl),f.clientEmail&&(Z.input.value=f.clientEmail,L.input.value=f.clientEmail);let l=document.body.innerText.match(/\d{3}-\d{3}-\d{4}/);l&&(d.input.value=l[0]),r.innerHTML='<span style="font-size:16px; color:#188038">\u2705</span> Dados Preenchidos!',r.style.background="#E6F4EA",r.style.borderColor="#188038",setTimeout(()=>{r.innerHTML='<span style="font-size:16px">\u2728</span> Preencher Automaticamente',r.style.background="#FFFFFF",r.style.borderColor="#DADCE0"},2e3),V("Dados capturados com sucesso!")},x.onclick=async()=>{let f=b=>{let A=t.querySelector(`#st-${b}`);return A?A.value:""},N=b=>{let A=t.querySelector(`input[name="st-${b}"]:checked`);return A?A.value:"N"},v=`Split & Transfer : Phone Note Format [Mandatory]
...
`).filter(Q=>Q.trim()!=="").map(Q=>`<p style="margin: 0 0 8px 0;">${Q}</p>`).join(""):ne.tagName==="TEXTAREA"&&(te=te.replace(/\n/g,"<br>"));let ue=te.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(ue===""||ue==="\u2022"||ue.toLowerCase()==="n/a"){let Q=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${W}\\}(?:<br>\\s*)?`,"gi");Q.test(M)?M=M.replace(Q,""):M=M.replace(P,"")}else M=M.replace(P,te.replace(/\$/g,"$$$$"))}),M=M.replace(/{([A-Z0-9_]+)}/g,""),M=M.replace(/(<br>){3,}/g,"<br><br>"),typeof r<"u"&&r.getOutput&&(M+=r.getOutput()),M}ot.onclick=()=>{let y=Ut();y?(dt(y),V(J("copiado_sucesso"))):V(J("selecione_substatus"),{error:!0})},nt.onclick=async()=>{let y=b.value,$=Ut();if(!$){V(J("selecione_substatus"),{error:!0});return}dt($),Ft();let M=wt(),K=await St();if(K)try{if(K.focus(),K.innerHTML.trim()==="<p><br></p>"||K.innerHTML.trim()==="<br>"||K.innerText.trim()===""){let ge=document.createRange();ge.selectNodeContents(K);let se=window.getSelection();se.removeAllRanges(),se.addRange(ge),document.execCommand("delete",!1,null)}else if(!K.innerHTML.endsWith("<br><br>")){let ge=document.createRange();ge.selectNodeContents(K),ge.collapse(!1);let se=window.getSelection();se.removeAllRanges(),se.addRange(ge),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,$),Ct(K),setTimeout(()=>{V(J("inserido_copiado"))},600);let me=typeof $e<"u"&&$e?$e.checked:!0;if(y&&Ye[y]&&me){let ge=Ye[y];await xt(ge),await new Promise(se=>setTimeout(se,500))}M(),_t(1.5),N.value="",b.innerHTML=`<option value="">${J("select_substatus")}</option>`,b.disabled=!0}catch(re){console.error(re),V("Erro ao inserir.",{error:!0}),M()}};function _t(y=1.5){y<=1.5&&(Pe.style.display="none",Ee.innerHTML=""),y<=2&&(p.reset(),he.style.display="none"),y<=3&&(Ae.style.display="none",De.innerHTML="",r.reset(),Qe.style.display="none",He.style.display="none")}function Ft(){if(s=!s,s){let y=d.querySelector(".cw-expand-btn");y&&typeof y.resetState=="function"&&y.resetState()}Ie(s,d,"cw-btn-notes")}return kt("bau"),Ot("pt"),Ft}var Ke={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
        ...
        `;let k=f.querySelector("#csa-back-btn");k.onmouseenter=()=>k.style.background="#f1f3f4",k.onmouseleave=()=>k.style.background="none",k.onclick=v;let B=f.querySelector("#csa-insert-btn");B.onmouseenter=()=>{B.style.transform="translateY(-1px)",B.style.boxShadow="0 6px 16px rgba(26,115,232,0.4)"},B.onmouseleave=()=>{B.style.transform="translateY(0)",B.style.boxShadow="0 4px 12px rgba(26,115,232,0.3)"},B.onclick=()=>{B.style.transform="scale(0.96)",N(I,"email"),setTimeout(()=>{B.style.transform="scale(1)",v()},300)}}function v(){s="list",E.style.transform="translateX(0)"}function w(I,j,D=null){let k=document.createElement("button"),B=D?`<span style="margin-right:6px; font-size:14px; opacity:0.9;">${D}</span>`:"";return k.innerHTML=`${B}${I}`,Object.assign(k.style,T),n===j&&o===""?Object.assign(k.style,h):(k.onmouseenter=()=>{k.style.background="#F1F3F4",k.style.borderColor="#DADCE0"},k.onmouseleave=()=>{k.style.background="#FFFFFF",k.style.borderColor="#DADCE0"}),k.onclick=()=>{n=j,o="",L.value="",b(),A()},k}function b(){R.innerHTML="",R.appendChild(w("Smart CRs",t,"\u26A1")),Object.keys(Ke).forEach(I=>{R.appendChild(w(Ke[I].title,I))})}function A(){O.innerHTML="";let I=[];if(o.trim()!==""){let U=o.toLowerCase();Object.values(Ke).forEach(F=>{F.emails.forEach(H=>{(H.name.toLowerCase().includes(U)||H.subject.toLowerCase().includes(U))&&I.push({type:"email",data:H})})}),Object.entries(Ye).forEach(([F,H])=>{if(!H)return;(F.replace(/_/g," ").toLowerCase().includes(U)||H.toLowerCase().includes(U))&&I.push({type:"cr",key:F,code:H})})}else n===t?Object.entries(Ye).forEach(([U,F])=>{F&&I.push({type:"cr",key:U,code:F})}):Ke[n]&&Ke[n].emails.forEach(U=>{I.push({type:"email",data:U})});if(I.length===0){O.innerHTML=`
                ...
                `,F.onclick=()=>l(H)}else{let H=U.key.replace(/_/g," ").replace("AS ","AS - ").replace("NI ","NI - ");F.innerHTML=`
 ...
 D) Nombre del sitio web.`,"autenticar la cuenta del anunciante en el cases, si corresponde.","T\xE9rminos y condi\xE7\xF5es.","Informar las Task solicitadas y AM.","Cerrar contenido sensible.","Confirmaci\xF3n de copia de seguridad y acceso de administrador a las ferramentas.","Resumen de llamada."],fim:["Ofrecer ayuda adicional.","Dejar de compartir la pantalla.","Pasos siguientes  (Si se le har\xE1 seguimiento al caso).","Encuesta de Satisfa\xE7\xE3o.","Informar al cliente que el equipo de QA ir\xE1 a realizar pruebas en los siguientes dias."]},"EN BAU":{color:"#ff0011",inicio:["Example 1","Example 2"],fim:["Example 3","Example 4"]}};function To(){let e="v2.1 (Apple Motion)",t={progressBarContainer:{height:"4px",background:"#f1f3f4",width:"100%",position:"relative",overflow:"hidden"},progressBarFill:{height:"100%",background:"linear-gradient(90deg, #4285F4, #34A853)",width:"0%",transition:"width 0.5s cubic-bezier(0.25, 0.8, 0.25, 1)",borderRadius:"0 2px 2px 0"},contentArea:{padding:"16px",overflowY:"auto",flexGrow:"1",background:"#f8f9fa",scrollBehavior:"smooth"},card:{background:"#ffffff",border:"1px solid #dadce0",borderRadius:"12px",padding:"16px",marginBottom:"16px",transition:"transform 0.2s ease, box-shadow 0.2s ease",boxShadow:"0 1px 2px rgba(0,0,0,0.02)"},cardTitle:{fontSize:"13px",fontWeight:"700",color:"#5f6368",textTransform:"uppercase",letterSpacing:"0.6px",marginBottom:"12px",display:"flex",alignItems:"center",justifyContent:"space-between",userSelect:"none"},itemRow:{display:"flex",alignItems:"flex-start",padding:"10px 8px",cursor:"pointer",borderRadius:"8px",transition:"background-color 0.15s ease, opacity 0.3s ease",color:"#202124",fontSize:"14px",lineHeight:"1.5",marginBottom:"2px"},itemCompleted:{opacity:"0.5",textDecoration:"line-through",color:"#5f6368"},checkbox:{minWidth:"20px",height:"20px",borderRadius:"6px",border:"2px solid #dadce0",marginRight:"14px",marginTop:"1px",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1)",background:"#fff"},footer:{padding:"12px 16px",borderTop:"1px solid #eee",background:"#fff",display:"flex",justifyContent:"space-between",alignItems:"center"},resetBtn:{background:"transparent",border:"none",color:"#d93025",fontSize:"12px",fontWeight:"600",cursor:"pointer",padding:"6px 12px",borderRadius:"20px",transition:"background 0.2s ease",display:"flex",alignItems:"center",gap:"4px"}},n={},o="PT",s="BAU",i=!1,a=document.createElement("div");a.id="call-script-popup",a.classList.add("cw-module-window"),Object.assign(a.style,Te,{right:"auto",left:"50%",width:"400px",height:"650px",display:"flex",flexDirection:"column",transform:"translateX(-50%) scale(0.05)"});let r={popup:a,googleLine:null};function p(){i=!i,Ie(i,a,"cw-btn-script")}let d=qe(a,"Call Script",e,"Guia interativo para condu\xE7\xE3o de chamadas.",r,()=>{p()});a.appendChild(d);let u=document.createElement("div");Object.assign(u.style,t.progressBarContainer);let c=document.createElement("div");Object.assign(c.style,t.progressBarFill),u.appendChild(c),a.appendChild(u);let m=document.createElement("div");m.id="csa-content",Object.assign(m.style,t.contentArea),a.appendChild(m);let T=document.createElement("div");Object.assign(T.style,t.footer);let h=document.createElement("span");h.textContent="by lucaste@",Object.assign(h.style,{fontSize:"10px",color:"#bdc1c6"});let g=document.createElement("button");g.innerHTML=`
  ...
  `,h.onfocus=()=>{m.style.background="#FFF",m.style.boxShadow="0 2px 8px rgba(0,0,0,0.05)",m.style.borderColor=s.primary},h.onblur=()=>{m.style.background="#F1F3F4",m.style.boxShadow="none",m.style.borderColor=s.borderSubtle},m.appendChild(T),m.appendChild(h),c.appendChild(m),u.appendChild(c);let g=document.createElement("div");g.style.cssText="flex: 1; overflow-y: auto; padding: 0 24px 40px 24px; scroll-behavior: smooth;",u.appendChild(g);let S=null;function _(){if(S)return;S=document.createElement("div"),S.style.cssText=`
          position: absolute; bottom: 0; left: 0; width: 100%; height: 100%;
          background: rgba(255,255,255,0.98); z-index: 20;
          display: flex; flex-direction: column;
          transform: translateY(100%); transition: transform 0.3s cubic-bezier(0.2, 0.8, 0.2, 1);
          box-shadow: 0 -4px 20px rgba(0,0,0,0.1);
      `;let O=document.createElement("div");O.style.cssText="padding: 16px 24px; display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #F1F3F4;",O.innerHTML='<span style="font-size: 16px; font-weight: 700; color: #202124;">\u{1F552} Hist\xF3rico Recente</span>';let x=document.createElement("button");x.innerHTML='<svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>',x.style.cssText="background: none; border: none; cursor: pointer; color: #5f6368;",x.onclick=Z,O.appendChild(x),S.appendChild(O);let f=document.createElement("div");f.id="cw-history-list",f.style.cssText="flex: 1; overflow-y: auto; padding: 20px;",S.appendChild(f),u.appendChild(S)}function G(){S||_();let O=S.querySelector("#cw-history-list");O.innerHTML="";let x=Yo();x.length===0?O.innerHTML='<div style="text-align: center; color: #999; margin-top: 40px;">Nenhum link acessado recentemente.</div>':x.forEach(f=>{let N=L(f,Ge[f._originalCat],!0);O.appendChild(N)}),requestAnimationFrame(()=>{S.style.transform="translateY(0)"})}function Z(){S&&(S.style.transform="translateY(100%)")}function oe(){d.innerHTML="";let O=C("history","Recentes",Ge.history);O.onclick=()=>{Y.playClick(),G()};let x=document.createElement("div");x.style.cssText="width: 40px; height: 1px; background: rgba(0,0,0,0.06); margin: 8px 0;",d.appendChild(O),d.appendChild(x),Object.keys(Et).forEach(f=>{let N=Et[f],l=C(f,N.label,Ge[f]);l.onclick=()=>{Y.playClick(),E(f)},d.appendChild(l)})}function C(O,x,f){let N=document.createElement("div");N.style.cssText=`
          width: 64px; height: 56px; border-radius: 12px;
          display: flex; flex-direction: column; align-items: center; justify-content: center;
          cursor: pointer; color: ${s.textSecondary}; 
          transition: all 0.2s cubic-bezier(0.2, 0.0, 0.2, 1);
          margin-bottom: 4px; position: relative;
      `,N.title=x,N.dataset.key=O;let l=document.createElement("div");l.style.cssText="width: 24px; height: 24px; margin-bottom: 4px; transition: transform 0.2s;",l.innerHTML=f||Ge.tasks;let v=document.createElement("div");return v.style.cssText="font-size: 10px; font-weight: 500; opacity: 0.8;",v.textContent=x,N.appendChild(l),N.appendChild(v),N.onmouseenter=()=>{o!==O&&(N.style.background="rgba(0,0,0,0.04)",l.style.transform="scale(1.1)")},N.onmouseleave=()=>{o!==O&&(N.style.background="transparent",l.style.transform="scale(1)")},N}function E(O){let x=document.getElementById(`cat-anchor-${O}`);x&&(x.scrollIntoView({behavior:"smooth",block:"start"}),q(O))}function q(O){o=O,Array.from(d.children).forEach(x=>{if(!x.dataset.key)return;let f=x.querySelector("div:first-child");x.dataset.key===O?(x.style.background="#E8F0FE",x.style.color="#1967D2",x.style.fontWeight="600",f&&(f.style.transform="scale(1.1)")):(x.style.background="transparent",x.style.color=s.textSecondary,x.style.fontWeight="500",f&&(f.style.transform="scale(1)"))})}function z(){if(g.innerHTML="",t.trim()!==""){let x=[];if(Object.entries(Et).forEach(([N,l])=>{let v=l.links.filter(w=>w.name.toLowerCase().includes(t.toLowerCase())||w.desc.toLowerCase().includes(t.toLowerCase()));x.push(...v.map(w=>({...w,_cat:N})))}),x.length===0){g.innerHTML='<div style="text-align:center; padding: 60px; color:#999; font-size:13px;">Nenhum link encontrado.</div>';return}let f=document.createElement("div");f.innerHTML="Resultados da busca",f.style.cssText="font-size:12px; font-weight:700; color:#5f6368; margin:20px 0 10px; text-transform:uppercase;",g.appendChild(f),x.forEach(N=>{let l=L(N,Ge[N._cat],!1);g.appendChild(l)});return}Object.entries(Et).forEach(([x,f])=>{let N=document.createElement("div"),l=document.createElement("div");l.id=`cat-anchor-${x}`,l.innerHTML=`<span style="opacity:0.6; margin-right:8px;">${Ge[x]}</span> ${f.label}`,l.style.cssText=`
              display: flex; align-items: center;
              font-size: 12px; font-weight: 700; color: #1a73e8; 
              text-transform: uppercase; margin: 32px 0 12px 0;
              padding-top: 10px;
          `,N.appendChild(l),f.links.forEach(v=>{let w=L(v,Ge[x],!1,x);N.appendChild(w)}),g.appendChild(N)});let O=document.createElement("div");O.style.height="60px",g.appendChild(O)}function L(O,x,f,N){let l=document.createElement("div"),v=f?"#FFF8E1":"#FFFFFF",w=f?"1px solid #FFE082":"1px solid rgba(0,0,0,0.08)";l.style.cssText=`
