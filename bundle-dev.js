...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
        ...
        `,document.head.appendChild(f)}let l=document.createElement("div");l.className="cw-zen-container";let u=document.createElement("div");Object.assign(u.style,{display:"none"});let b=document.createElement("div");b.className="cw-screens-container",u.appendChild(b),l.innerHTML=`
    ...
    `;let h=l.querySelector(".cw-hero-grid"),d=l.querySelector(".cw-acc-container"),S=l.querySelector(".cw-results-container"),m=l.querySelector(".cw-search-input"),g=l.querySelector(".cw-status-bar"),y=l.querySelector(".cw-status-text"),T=l.querySelector(".cw-footer-icons");i.forEach(([f,x])=>{let s=o(x.name),c=document.createElement("div");c.className="cw-hero-card",c.id=`hero-${f}`,c.style.setProperty("--hero-color",s.color),c.innerHTML=`
        ...
        `,c.onclick=w=>{if(w.target.closest(".cw-step-btn"))return;let p=t[f]?t[f].count:0;_(f,p>0?-p:1,x)},c.querySelector(".minus").onclick=()=>_(f,-1,x),c.querySelector(".plus").onclick=()=>_(f,1,x),c.dataset.color=s.color,h.appendChild(c)});function q(f,x){let s=o(x.name),c=document.createElement("div");return c.className="cw-task-item",c.dataset.id=f,c.innerHTML=`
        ...
        `,c.onclick=w=>{if(w.target.closest(".cw-step-btn"))return;let p=t[f]?t[f].count:0;_(f,p>0?-p:1,x)},c.querySelector(".minus").onclick=()=>_(f,-1,x),c.querySelector(".plus").onclick=()=>_(f,1,x),c}Object.entries(r).forEach(([f,x])=>{let s=document.createElement("div");s.className="cw-acc-group";let c=document.createElement("div");c.className="cw-acc-header",c.innerHTML=`
            <div class="cw-acc-title">
                <div class="cw-acc-dot" style="background:${x.brand.color}"></div>
                ${f}
            </div>
            <div class="cw-acc-icon">\u25BC</div>
        `,c.onclick=()=>{d.querySelectorAll(".cw-acc-group.open").forEach(p=>{p!==s&&p.classList.remove("open")}),s.classList.toggle("open")};let w=document.createElement("div");w.className="cw-acc-body",x.tasks.forEach(p=>{let A=q(p.key,p);w.appendChild(A)}),s.appendChild(c),s.appendChild(w),d.appendChild(s)});function _(f,x,s){t[f]||(t[f]={count:0,data:s,brand:o(s.name)}),t[f].count+=x,t[f].count<=0&&delete t[f],B(),H(),e&&e()}function B(){i.forEach(([w])=>{let p=h.querySelector(`#hero-${w}`);if(!p)return;let A=t[w];A?(p.classList.add("active"),p.querySelector(".cw-step-val").textContent=A.count,p.querySelector(".cw-step-val").style.color=p.dataset.color):p.classList.remove("active")}),l.querySelectorAll(".cw-task-item").forEach(w=>{let p=w.dataset.id,A=t[p];A?(w.classList.add("selected"),w.querySelector(".cw-step-val").textContent=A.count):w.classList.remove("selected")});let x=Object.keys(t),s=0,c=[];if(x.forEach(w=>{let p=t[w];s+=p.count;for(let A=0;A<p.count;A++)c.length<6&&c.push(p.brand)}),s>0){g.classList.add("visible");let w=s>1?"A\xE7\xF5es":"A\xE7\xE3o",p=s>1?"definidas":"definida";y.textContent=`${s} ${w} ${p}`,T.innerHTML="",c.forEach(A=>{let O=document.createElement("div");O.className="cw-mini-icon",O.innerHTML=Pe[A.icon]||Pe.default;let v=O.querySelector("svg");v&&(v.style.width="14px",v.style.height="14px"),T.appendChild(O)})}else g.classList.remove("visible")}m.addEventListener("input",f=>{let x=f.target.value.toLowerCase();if(x.length>0){d.style.display="none",S.style.display="block",S.innerHTML="";let s=!1;Object.entries(Re).forEach(([c,w])=>{if(w.name.toLowerCase().includes(x)){s=!0;let p=q(c,w);t[c]&&(p.classList.add("selected"),p.querySelector(".cw-step-val").textContent=t[c].count),S.appendChild(p)}}),s||(S.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else d.style.display="block",S.style.display="none"});function H(){b.innerHTML="";let f=Object.keys(t),x=!1,s=document.getElementById("sub-status"),c="implementation";if(s&&s.value.toLowerCase().includes("education")&&(c="education"),f.length===0){b.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}if(f.length===0){b.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let w=document.createElement("div");w.className="cw-info-banner",w.innerHTML=`
