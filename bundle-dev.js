...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let y=a.querySelector(".cw-hero-grid"),m=a.querySelector(".cw-acc-container"),l=a.querySelector(".cw-results-container"),T=a.querySelector(".cw-search-input"),f=a.querySelector(".cw-status-bar"),x=a.querySelector(".cw-status-text"),b=a.querySelector(".cw-footer-icons");o.forEach(([g,d])=>{let p=n(d.name),u=document.createElement("div");u.className="cw-hero-card",u.id=`hero-${g}`,u.style.setProperty("--hero-color",p.color),u.innerHTML=`
        ...
        `,u.onclick=v=>{if(v.target.closest(".cw-step-btn"))return;let C=e[g]?e[g].count:0;E(g,C>0?-C:1,d)},u.querySelector(".minus").onclick=()=>E(g,-1,d),u.querySelector(".plus").onclick=()=>E(g,1,d),u.dataset.color=p.color,y.appendChild(u)});function A(g,d){let p=n(d.name),u=document.createElement("div");return u.className="cw-task-item",u.dataset.id=g,u.innerHTML=`
        ...
        `,u.onclick=()=>{m.querySelectorAll(".cw-acc-group.open").forEach(C=>{C!==p&&C.classList.remove("open")}),p.classList.toggle("open")};let v=document.createElement("div");v.className="cw-acc-body",d.tasks.forEach(C=>{let k=A(C.key,C);v.appendChild(k)}),p.appendChild(u),p.appendChild(v),m.appendChild(p)});function E(g,d,p){e[g]||(e[g]={count:0,data:p,brand:n(p.name)}),e[g].count+=d,e[g].count<=0&&delete e[g],j(),H(),t&&t()}function j(){o.forEach(([v])=>{let C=y.querySelector(`#hero-${v}`);if(!C)return;let k=e[v];k?(C.classList.add("active"),C.querySelector(".cw-step-val").textContent=k.count,C.querySelector(".cw-step-val").style.color=C.dataset.color):C.classList.remove("active")}),a.querySelectorAll(".cw-task-item").forEach(v=>{let C=v.dataset.id,k=e[C];k?(v.classList.add("selected"),v.querySelector(".cw-step-val").textContent=k.count):v.classList.remove("selected")});let d=Object.keys(e),p=0,u=[];if(d.forEach(v=>{let C=e[v];p+=C.count;for(let k=0;k<C.count;k++)u.length<6&&u.push(C.brand)}),p>0){f.classList.add("visible");let v=p>1?"A\xE7\xF5es":"A\xE7\xE3o",C=p>1?"definidas":"definida";x.textContent=`${p} ${v} ${C}`,b.innerHTML="",u.forEach(k=>{let U=document.createElement("div");U.className="cw-mini-icon",U.innerHTML=Ge[k.icon]||Ge.default;let _=U.querySelector("svg");_&&(_.style.width="14px",_.style.height="14px"),b.appendChild(U)})}else f.classList.remove("visible")}T.addEventListener("input",g=>{let d=g.target.value.toLowerCase();if(d.length>0){m.style.display="none",l.style.display="block",l.innerHTML="";let p=!1;Object.entries(ke).forEach(([u,v])=>{if(v.name.toLowerCase().includes(d)){p=!0;let C=A(u,v);e[u]&&(C.classList.add("selected"),C.querySelector(".cw-step-val").textContent=e[u].count),l.appendChild(C)}}),p||(l.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else m.style.display="block",l.style.display="none"});function H(){c.innerHTML="";let g=Object.keys(e),d=!1,p="implementation";if(g.length===0){c.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let u=document.createElement("div");u.className="cw-info-banner",u.innerHTML=`
...
\u2022 `)?(R.preventDefault(),this.value=q.substring(0,S-3)+this.value.substring(this.selectionEnd),this.selectionStart=this.selectionEnd=S-3):q==="\u2022 "&&(R.preventDefault(),this.value="",this.selectionStart=this.selectionEnd=0)}}}}function lt(){let h=typeof fe<"u"?fe:document.getElementById("snippet-container");if(!h)return;let R=h.querySelectorAll('input[type="checkbox"]:checked, input[type="radio"]:checked'),S={},q=new Set;R.forEach(W=>{let V=W.id,L=xe[V];if(L)for(let N in L)N==="linkedTask"?q.add(L.linkedTask):N!=="type"&&(S[N]||(S[N]=[]),S[N].includes(L[N])||S[N].push(L[N]))});let te=new Set;Object.values(xe).forEach(W=>{Object.keys(W).forEach(V=>{V!=="linkedTask"&&V!=="type"&&te.add(V)})}),te.forEach(W=>{let V=document.getElementById(W);if(V){let L=S[W]||[],N="";We.includes(W.replace("field-",""))?(N=L.map($=>$.startsWith("\u2022 ")?$:"\u2022 "+$).join(`
`),N===""?N="\u2022 ":N.endsWith(`
\u2022 `)||(N+=`
\u2022 `)):N=L.join(`

`),N.trim()!=="\u2022"&&N.trim()!==""?V.value=N:We.includes(W.replace("field-",""))?V.value="\u2022 ":V.value="",V.tagName==="TEXTAREA"&&typeof rt=="function"&&rt(V)}});let re=new Set,me=new Set;h.querySelectorAll('input[type="checkbox"], input[type="radio"]').forEach(W=>{let V=xe[W.id];V&&V.linkedTask&&(W.checked?re.add(V.linkedTask):me.add(V.linkedTask))}),me.forEach(W=>{re.has(W)||i.toggleTask(W,!1)}),re.forEach(W=>{i.toggleTask(W,!0)})}C.onchange=()=>{let h=C.value;if(ct(1.5),w.innerHTML=`<option value="">${z("select_substatus")}</option>`,!h){w.disabled=!0;return}for(let R in Ue){let S=Ue[R];if(S.status===h){let q=document.createElement("option");q.value=R,q.textContent=S.name,w.appendChild(q)}}w.disabled=!1},w.onchange=()=>{let h=w.value;if(ct(1.5),!h)return;i.updateSubStatus(h);let R=Ue[h];fe.innerHTML="";let S=(L,N,$)=>{let ae=document.createElement("label");Object.assign(ae.style,s.checkboxLabel),ae.onmouseover=()=>ae.style.backgroundColor="#e8eaed",ae.onmouseout=()=>ae.style.backgroundColor="#f8f9fa";let Q=document.createElement("input");return Q.type=N,Q.id=L.id,Object.assign(Q.style,s.checkboxInput),ae.appendChild(Q),ae.appendChild(document.createTextNode(` ${L.text}`)),$.appendChild(ae),Q},q=[],te="radio";if(h==="NI_Awaiting_Inputs")q=[{id:"quickfill-ni-inicio-manual",text:"In\xEDcio 2/6 (Manual)"},{id:"quickfill-ni-cms-access",text:"In\xEDcio 2/6 (ADV sem acesso ao CMS)"},{id:"quickfill-ni-followup-bau",text:"Follow-up 2/6 (BAU)"},{id:"quickfill-ni-followup-lm",text:"Follow-up 2/6 (LM)"}];else if(h.startsWith("SO_"))te="checkbox",q=[{id:"quickfill-gtm-install",text:"Instala\xE7\xE3o do GTM"},{id:"quickfill-whatsapp",text:"Convers\xE3o de WhatsApp"},{id:"quickfill-form",text:"Convers\xE3o de Formul\xE1rio (Padr\xE3o)"},{id:"quickfill-ecw4-close",text:"Fechamento ECW4 (P\xF3s 7 dias)"}];else if(h.startsWith("AS_")){te="checkbox";let L=document.createElement("label");L.textContent=z("cenarios_comuns"),Object.assign(L.style,s.label),fe.appendChild(L),q=[{id:"quickfill-as-no-show",text:"Anunciante n\xE3o compareceu (respondeu e-mail)"},{id:"quickfill-as-insufficient-time",text:"Tempo insuficiente"},{id:"quickfill-as-no-access",text:"Anunciante sem acessos necess\xE1rios"}]}else h.startsWith("IN_")&&(q=[{id:"quickfill-in-nrp-bau",text:"NRP (BAU - 3 tentativas)"},{id:"quickfill-in-nrp-lm",text:"NRP (LM - Sem tentativas)"},{id:"quickfill-in-no-show-bau",text:"No-Show (BAU - 3 tentativas)"},{id:"quickfill-in-2-6-final",text:"Finaliza\xE7\xE3o 2/6 (Sem Resposta)"},{id:"quickfill-in-manual",text:"Outro (Manual)"}]);let re=q.filter(L=>{let N=xe[L.id];return!N.type||N.type==="all"||N.type===e});re.forEach((L,N)=>{let $=S(L,te,fe);te==="radio"&&($.name="scenario-radio-group",N===0&&($.checked=!0))}),re.length>0&&(Re.style.display="block"),R.requiresTasks?(pe.style.display="none",Pe.style.display="block",i.selectionElement.style.display="block",he.style.display="block"):(pe.style.display="block",Pe.style.display="none",i.selectionElement.style.display="none",he.style.display="block"),Ee.innerHTML="";let me=R.template.match(/{([A-Z0-9_]+)}/g)||[];[...new Set(me)].forEach(L=>{if(["{TAGS_IMPLEMENTED}","{SCREENSHOTS_LIST}","{CONSENTIU_GRAVACAO}","{CASO_PORTUGAL}"].includes(L))return;let N=L.slice(1,-1),$=document.createElement("label"),ae=z(N.toLowerCase());$.textContent=ae!==N.toLowerCase()?ae:N.replace(/_/g," ").replace(/\b\w/g,ie=>ie.toUpperCase())+":",Object.assign($.style,s.label);let Q;We.includes(N)?(Q=document.createElement("textarea"),Object.assign(Q.style,s.textarea),Q.classList.add("bullet-textarea"),rt(Q)):ft.includes(N)?(Q=document.createElement("textarea"),Object.assign(Q.style,s.textarea)):(Q=document.createElement("input"),Q.type="text",Object.assign(Q.style,s.input),N==="REASON_COMMENTS"&&(h.startsWith("NI_")||h.startsWith("IN_"))&&(Object.assign($.style,{display:"none"}),Object.assign(Q.style,{display:"none"}))),N==="ON_CALL"&&e==="lm"&&(Object.assign($.style,{display:"none"}),Object.assign(Q.style,{display:"none"}),Q.value="N/A"),Q.id=`field-${N}`,Ee.appendChild($),Ee.appendChild(Q)});let W=fe.querySelectorAll('input[type="checkbox"], input[type="radio"]');W.length>0&&(W.forEach(L=>{L.removeEventListener("change",lt),L.addEventListener("change",lt)}),lt()),Ce.style.display="block",Ze[h]?Le.style.display="block":Le.style.display="none",je.style.display="flex";let V=i.getCheckedElements().map(L=>L.value);a.updateVisibility(h,V)},pe.onclick=()=>{pe.style.display="none",Pe.style.display="block",i.selectionElement.style.display="block"};function Ct(){let h=w.value;if(!h)return null;let S=Ue[h].template.replace(/\n/g,"<br>"),q='style="margin-bottom: 12px; padding-left: 30px;"',te=[],re="",me=i.getCheckedElements();me.length>0&&me.forEach(V=>{let L=V.value,N=ke[L],$=V.closest().querySelector(".stepper-count"),ae=$?parseInt($.textContent):1;ae>1?te.push(`${N.name} (x${ae})`):te.push(N.name)});let le=i.screenshotsElement;if(le){let V=Array.from(le.querySelectorAll('input[id^="name-"]'));V.length>0&&V.forEach(L=>{let N=L.value,$=L.closest(".cw-screen-card");if($){let ae=$.querySelectorAll('input[id^="screen-"]'),Q=!1,ie="";ae.forEach(ue=>{let At=ue.closest(".cw-input-group"),wt=At?At.querySelector(".cw-input-label"):null,Qt=wt?wt.textContent:"Evid\xEAncia",Et=ue.value.trim(),Zt=Et?` ${Et}`:"";ie+=`<li>${Qt} -${Zt}</li>`,Q=!0}),Q&&(re+=`<b>${N}</b>`,re+=`<ul ${q}>${ie}</ul>`)}})}if(S.includes("{TAGS_IMPLEMENTED}")?S=S.replace(/{TAGS_IMPLEMENTED}/g,te.join(", ")||"N/A"):te.length>0&&(S+=`<br><b>Tags:</b> ${te.join(", ")}<br>`),S.includes("{SCREENSHOTS_LIST}")?S=S.replace(/{SCREENSHOTS_LIST}/g,re?`${re}`:"N/A"):re!==""&&(S+=`<br>${re}`),n==="pt"&&o){let V=ge.checked?z("sim"):z("nao");S=S.replace(/{CONSENTIU_GRAVACAO}/g,`<br><b>${z("consentiu_gravacao")}</b> ${V}<br><br>`),S=S.replace(/{CASO_PORTUGAL}/g,`<br><b>${z("caso_portugal")}</b> ${z("sim")}<br>`)}else n==="pt"&&!o?(S=S.replace(/{CASO_PORTUGAL}/g,`<br><b>${z("caso_portugal")}</b> ${z("nao")}<br>`),S=S.replace(/{CONSENTIU_GRAVACAO}/g,"")):(S=S.replace(/{CASO_PORTUGAL}/g,""),S=S.replace(/{CONSENTIU_GRAVACAO}/g,""));return Ee.querySelectorAll("input, textarea").forEach(V=>{let L=V.id.replace("field-",""),N=new RegExp(`{${L}}`,"g"),$=V.value;if(L==="REASON_COMMENTS"&&(h.startsWith("NI_")||h.startsWith("IN_"))){let ie=fe.querySelector('input[type="radio"]:checked');ie&&xe[ie.id]&&($=xe[ie.id]["field-REASON_COMMENTS"])}if(We.includes(L)&&$.trim()!==""){let ie=$.split(`
...
`).filter(ie=>ie.trim()!=="").map(ie=>`<p style="margin: 0 0 8px 0;">${ie}</p>`).join(""):V.tagName==="TEXTAREA"&&($=$.replace(/\n/g,"<br>"));let ae=$.replace(/<[^>]*>/g,"").replace(/&nbsp;/g," ").trim();if(ae===""||ae==="\u2022"||ae.toLowerCase()==="n/a"){let ie=new RegExp(`(?:<br>\\s*)?<[b|strong]+>[^<]+:\\s*<\\/[b|strong]+>\\s*\\{${L}\\}(?:<br>\\s*)?`,"gi");ie.test(S)?S=S.replace(ie,""):S=S.replace(N,"")}else S=S.replace(N,$.replace(/\$/g,"$$$$"))}),S=S.replace(/{([A-Z0-9_]+)}/g,""),S=S.replace(/(<br>){3,}/g,"<br><br>"),typeof a<"u"&&a.getOutput&&(S+=a.getOutput()),S}Be.onclick=()=>{let h=Ct();h?(yt(h),oe(z("copiado_sucesso"))):oe(z("selecione_substatus"),{error:!0})},qe.onclick=async()=>{let h=w.value,R=Ct();if(!R){oe(z("selecione_substatus"),{error:!0});return}yt(R),dt();let S=tt(),q=await qt();if(q)try{if(q.focus(),q.innerHTML.trim()==="<p><br></p>"||q.innerHTML.trim()==="<br>"||q.innerText.trim()===""){let le=document.createRange();le.selectNodeContents(q);let W=window.getSelection();W.removeAllRanges(),W.addRange(le),document.execCommand("delete",!1,null)}else if(!q.innerHTML.endsWith("<br><br>")){let le=document.createRange();le.selectNodeContents(q),le.collapse(!1);let W=window.getSelection();W.removeAllRanges(),W.addRange(le),document.execCommand("insertHTML",!1,"<br><br>")}document.execCommand("insertHTML",!1,R),Bt(q),setTimeout(()=>{oe(z("inserido_copiado"))},600);let re=typeof De<"u"&&De?De.checked:!0;if(h&&Ze[h]&&re){let le=Ze[h];await Mt(le),await new Promise(W=>setTimeout(W,500))}S(),ct(1.5),C.value="",w.innerHTML=`<option value="">${z("select_substatus")}</option>`,w.disabled=!0}catch(te){console.error(te),oe("Erro ao inserir.",{error:!0}),S()}};function ct(h=1.5){h<=1.5&&(Re.style.display="none",fe.innerHTML=""),h<=2&&(he.style.display="none",i.reset(),pe.style.display="none"),h<=3&&(Ce.style.display="none",Ee.innerHTML="",a.reset(),je.style.display="none",Le.style.display="none")}function dt(){if(r=!r,r){let h=c.querySelector(".cw-expand-btn");h&&typeof h.resetState=="function"&&h.resetState()}Ne(r,c,"cw-btn-notes")}return it("bau"),st("pt"),dt}var Fe={CONTACT:{title:"Tentativas & Agendamento",emails:[{id:"attempt_10min",name:"Tentativa de Contato (Antes dos 10min)",subject:"Implementa\xE7\xE3o com o Time de Solu\xE7\xF5es T\xE9cnicas do Google - Tentativa de Contato",body:`
                    <p>Ol\xE1,</p>
                    <br>
