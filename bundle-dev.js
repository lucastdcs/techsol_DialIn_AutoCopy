...
/* --- SCREENSHOTS: FINE & ELEGANT (Step 3) --- */
    ...
    `;let u=r.querySelector(".cw-hero-grid"),d=r.querySelector(".cw-acc-container"),f=r.querySelector(".cw-results-container"),x=r.querySelector(".cw-search-input"),h=r.querySelector(".cw-status-bar"),b=r.querySelector(".cw-status-text"),A=r.querySelector(".cw-footer-icons");s.forEach(([y,v])=>{let C=o(v.name),F=document.createElement("div");F.className="cw-hero-card",F.id=`hero-${y}`,F.style.setProperty("--hero-color",C.color),F.innerHTML=`
            <div class="cw-hero-main">
                <div class="cw-hero-icon">${Ke[C.icon]}</div>
        ...
        `,F.onclick=q=>{if(q.target.closest(".cw-step-btn"))return;let w=e[y]?e[y].count:0;z(y,w>0?-w:1,v)},F.querySelector(".minus").onclick=()=>z(y,-1,v),F.querySelector(".plus").onclick=()=>z(y,1,v),F.dataset.color=C.color,u.appendChild(F)});function k(y,v){let C=o(v.name),F=document.createElement("div");return F.className="cw-task-item",F.dataset.id=y,F.innerHTML=`
            <div class="cw-task-left">
                <div class="cw-list-icon" style="background:${C.bg}; color:${C.color}">
                    ${Ke[C.icon]||Ke.default}
                </div>
        ...
        `,F.onclick=q=>{if(q.target.closest(".cw-step-btn"))return;let w=e[y]?e[y].count:0;z(y,w>0?-w:1,v)},F.querySelector(".minus").onclick=()=>z(y,-1,v),F.querySelector(".plus").onclick=()=>z(y,1,v),F}Object.entries(i).forEach(([y,v])=>{let C=document.createElement("div");C.className="cw-acc-group";let F=document.createElement("div");F.className="cw-acc-header",F.innerHTML=`
            <div class="cw-acc-title">
                <div class="cw-acc-dot" style="background:${v.brand.color}"></div>
                ${y}
            </div>
            <div class="cw-acc-icon">\u25BC</div>
        `,F.onclick=()=>{d.querySelectorAll(".cw-acc-group.open").forEach(w=>{w!==C&&w.classList.remove("open")}),C.classList.toggle("open")};let q=document.createElement("div");q.className="cw-acc-body",v.tasks.forEach(w=>{let O=k(w.key,w);q.appendChild(O)}),C.appendChild(F),C.appendChild(q),d.appendChild(C)});function z(y,v,C){e[y]||(e[y]={count:0,data:C,brand:o(C.name)}),e[y].count+=v,e[y].count<=0&&delete e[y],G(),j(),t&&t()}function G(){s.forEach(([q])=>{let w=u.querySelector(`#hero-${q}`);if(!w)return;let O=e[q];O?(w.classList.add("active"),w.querySelector(".cw-step-val").textContent=O.count,w.querySelector(".cw-step-val").style.color=w.dataset.color):w.classList.remove("active")}),r.querySelectorAll(".cw-task-item").forEach(q=>{let w=q.dataset.id,O=e[w];O?(q.classList.add("selected"),q.querySelector(".cw-step-val").textContent=O.count):q.classList.remove("selected")});let v=Object.keys(e),C=0,F=[];if(v.forEach(q=>{let w=e[q];C+=w.count;for(let O=0;O<w.count;O++)F.length<6&&F.push(w.brand)}),C>0){h.classList.add("visible");let q=C>1?"A\xE7\xF5es":"A\xE7\xE3o",w=C>1?"definidas":"definida";b.textContent=`${C} ${q} ${w}`,A.innerHTML="",F.forEach(O=>{let S=document.createElement("div");S.className="cw-mini-icon",S.innerHTML=Ke[O.icon]||Ke.default;let L=S.querySelector("svg");L&&(L.style.width="14px",L.style.height="14px"),A.appendChild(S)})}else h.classList.remove("visible")}x.addEventListener("input",y=>{let v=y.target.value.toLowerCase();if(v.length>0){d.style.display="none",f.style.display="block",f.innerHTML="";let C=!1;Object.entries(Ge).forEach(([F,q])=>{if(q.name.toLowerCase().includes(v)){C=!0;let w=k(F,q);e[F]&&(w.classList.add("selected"),w.querySelector(".cw-step-val").textContent=e[F].count),f.appendChild(w)}}),C||(f.innerHTML='<div style="padding:20px; text-align:center; font-size:13px; color:#999">Nenhum resultado.</div>')}else d.style.display="block",f.style.display="none"});function j(){m.innerHTML="";let y=Object.keys(e),v=!1,C=document.getElementById("sub-status"),F="implementation";if(C&&C.value.toLowerCase().includes("education")&&(F="education"),y.length===0){m.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}if(y.length===0){m.innerHTML='<div class="cw-empty-state">Selecione tarefas para ver os campos.</div>';return}let q=document.createElement("div");q.className="cw-info-banner",q.innerHTML=`
