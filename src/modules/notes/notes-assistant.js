...
export function initCaseNotesAssistant() {
    ...
    // Step 2 (Tasks) + Botão de Redundância
    ...
    step2Div.id = "step-2-tasks"; Object.assign(step2Div.style, { ...styleStepBlock, display: 'none' }); Object.assign(step2Title.style, styleH3); taskCheckboxesContainer.id = "task-checkboxes-container"; 
    ...
    step3Div.id = "step-3-form"; Object.assign(step3Div.style, { ...styleStepBlock, display: 'none' }); Object.assign(step3Title.style, styleH3); dynamicFormFieldsContainer.id = "dynamic-form-fields-container"; step3Div.appendChild(step3Title); step3Div.appendChild(dynamicFormFieldsContainer);
    ...
    // Helper para popular as checkboxes (extraído para reuso)
    ...
    optionalTaskBtn.onclick = () => {
        optionalTaskBtn.style.display = 'none'; // Esconde o botão
        step2Title.style.display = 'block';     // Mostra título
        taskCheckboxesContainer.style.display = 'block'; // Mostra lista
        populateTaskCheckboxes(); // Popula
    };
    ...
    function renderScreenshotInputs() {
        ...
        selectedCheckboxes.forEach(checkbox => {
            ...
            if (screenshotList.length > 0) {
                ...
                // Header com campo de nome personalizado
                ...
                for (let i = 1; i <= count; i++) {
                    ...
                    // Campo para nome da task (Personalização)
                    const nameInput = document.createElement('input');
                    nameInput.type = 'text';
                    const suffix = count > 1 ? ` #${i}` : '';
                    nameInput.value = `${task.name}${suffix}`;
                    nameInput.id = `name-${taskKey}-${i}`; // ID Único para recuperar
    ...
    function generateOutputHtml() {
        ...
        // ... (Lógica de substituição de placeholders padrão mantida) ...
    ...
    subStatusSelect.onchange = () => {
        ...
        // ... (Lógica de criar snippets mantida)
        ...
        // ===== LÓGICA DE REDUNDÂNCIA DE TASKS =====
        if (templateData.requiresTasks) {
            optionalTaskBtn.style.display = 'none'; // Esconde botão
            step2Title.style.display = 'block';
            taskCheckboxesContainer.style.display = 'block';
            populateTaskCheckboxes();
            step2Div.style.display = 'block';
        } else {
            // Não requer task, mostra o botão de redundância
            optionalTaskBtn.style.display = 'block'; // Mostra botão
            step2Title.style.display = 'none';
            taskCheckboxesContainer.style.display = 'none'; // Esconde lista
            step2Div.style.display = 'block'; // Mostra o container do passo 2
        }
        // ==========================================
    ...
    // ... (Resto do código: generateButton.onclick que você já tem, resetSteps, togglePopup...)
