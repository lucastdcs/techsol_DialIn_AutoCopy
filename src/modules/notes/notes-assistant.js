...
export function initCaseNotesAssistant() {
    const CURRENT_VERSION = "v3.4.0"; // Versão com Tasks Opcionais
    ...
    // --- NOVO ESTILO: Botão Opcional ---
    const styleOptionalBtn = { 
        width: '100%', padding: '10px', background: 'white', 
        border: '1px dashed #1a73e8', color: '#1a73e8', 
        borderRadius: '6px', cursor: 'pointer', fontWeight: '500', 
        fontSize: '13px', marginBottom: '10px', transition: 'all 0.2s' 
    };
    ...
    function updateFieldsFromScenarios() {
        ...
        taskCheckboxes.forEach(taskCheckbox => {
            // Não resetamos se for linkedTask
            if (!activeLinkedTasks.has(taskCheckbox.value)) {
                 // Só desmarca se não estiver na lista de ativos e se o usuário não tiver marcado manualmente
                 // Na verdade, para simplificar e seguir a lógica original:
    ...
    typeBAU.textContent = "BAU"; typeBAU.classList.add('no-drag');
    typeLM.textContent = "LM"; typeLM.classList.add('no-drag');
    Object.assign(typeBAU.style, typeBtnStyle); Object.assign(typeLM.style, typeBtnStyle);
    function setCaseType(type) {
        currentCaseType = type; const newActiveStyle = getRandomGoogleStyle();
        Object.assign(typeBAU.style, typeBtnStyle); Object.assign(typeLM.style, typeBtnStyle);
    ...
    step0Div.appendChild(typeContainer); popupContent.appendChild(step0Div);
    setCaseType('bau'); 
    ...
    step1Div.appendChild(mainStatusLabel); step1Div.appendChild(mainStatusSelect);
    step1Div.appendChild(subStatusLabel); step1Div.appendChild(subStatusSelect);
    ...
    portugalRadioSim.type = 'radio'; portugalRadioSim.id = 'portugal-sim'; portugalRadioSim.name = 'portugal-group'; portugalRadioSim.value = 'sim';
    Object.assign(portugalRadioSim.style, styleCheckboxInput); portugalLabelSim.htmlFor = 'portugal-sim'; Object.assign(portugalLabelSim.style, { cursor: 'pointer' });
    ...
    portugalRadioNao.type = 'radio'; portugalRadioNao.id = 'portugal-nao'; portugalRadioNao.name = 'portugal-group'; portugalRadioNao.value = 'nao'; portugalRadioNao.checked = true; 
    Object.assign(portugalRadioNao.style, styleCheckboxInput); portugalLabelNao.htmlFor = 'portugal-nao'; Object.assign(portugalLabelNao.style, { cursor: 'pointer' });
    divPtNao.appendChild(portugalRadioNao); divPtNao.appendChild(portugalLabelNao);
    portugalRadioGroup.appendChild(divPtSim); portugalRadioGroup.appendChild(divPtNao);
    stepPortugalDiv.appendChild(portugalRadioGroup); popupContent.appendChild(stepPortugalDiv);
    ...
    consentRadioSim.type = 'radio'; consentRadioSim.id = 'consent-sim'; consentRadioSim.name = 'consent-group'; consentRadioSim.value = 'Sim'; consentRadioSim.checked = true;
    Object.assign(consentRadioSim.style, styleCheckboxInput); consentLabelSim.htmlFor = 'consent-sim'; Object.assign(consentLabelSim.style, { cursor: 'pointer' });
    divSim.appendChild(consentRadioSim); divSim.appendChild(consentLabelSim);
    const divNao = document.createElement('div'); Object.assign(divNao.style, { display: 'flex', alignItems: 'center' });
    consentRadioNao.type = 'radio'; consentRadioNao.id = 'consent-nao'; consentRadioNao.name = 'consent-group'; consentRadioNao.value = 'Não';
    Object.assign(consentRadioNao.style, styleCheckboxInput); consentLabelNao.htmlFor = 'consent-nao'; Object.assign(consentLabelNao.style, { cursor: 'pointer' });
    divNao.appendChild(consentRadioNao); divNao.appendChild(consentLabelNao);
    consentRadioGroup.appendChild(divSim); consentRadioGroup.appendChild(divNao);
    stepConsentDiv.appendChild(consentRadioGroup); popupContent.appendChild(stepConsentDiv);
    ...
    // --- ADICIONA O BOTÃO OPCIONAL AQUI ---
    ...
    function resetSteps(startFrom = 1.5) {
        ...
        if (startFrom <= 2) {
            ...
            // Reseta estado do botão opcional
    ...
    // --- Lógica de População de Tasks (Extraída para reuso) ---
    function populateTasks() {
        ...
        for (const taskKey in TASKS_DB) {
            ...
            btnMinus.onclick = (e) => {
                ...
                else { checkbox.checked = false; checkbox.dispatchEvent(new Event('change')); }
            };
    ...
    optionalTaskBtn.onclick = () => {
        ...
        populateTasks(); // Força popular as tasks
    };
    ...
    subStatusSelect.onchange = () => {
        ...
        // --- MUDANÇA AQUI (REDUNDÂNCIA) ---
        // Sempre popula as tasks, mas controla a visibilidade
        populateTasks();
        ...
        if (templateData.requiresTasks) {
            ...
            // Não requer: mostra botão opcional
    ...
    function generateOutputHtml() {
        ...
        // --- MUDANÇA AQUI: Verifica se tem checkboxes marcados, mesmo se não for required ---
        ...
        if (templateData.requiresTasks || selectedCheckboxes.length > 0) {
            ...
            // Se não tem tasks e não é required, limpa os placeholders
