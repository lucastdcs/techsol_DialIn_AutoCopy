...
export function initCaseNotesAssistant() {
    ...
    // =========================================================================
    // 1. ESTILOS LOCAIS (DEFINIDOS NO TOPO PARA EVITAR ERRO DE INICIALIZAÇÃO)
    // =========================================================================
    const styleInput = { width: "100%", padding: "8px", borderRadius: "8px", border: "1px solid #dadce0", fontSize: "14px", marginBottom: "12px", boxSizing: "border-box", fontFamily: "'Poppins', sans-serif", transition: "border-color 0.2s ease, box-shadow 0.2s ease" };
    const styleTextarea = { ...styleInput, height: "100px", resize: "vertical" };
    const styleH3 = { fontSize: "14px", fontWeight: "600", color: "#202124", margin: "0 0 12px 0" };
    const styleCheckboxLabel = { display: "flex", alignItems: "center", marginBottom: "10px", fontSize: "14px", fontWeight: "400", cursor: "pointer", padding: "8px", background: "#f8f9fa", borderRadius: "6px", transition: "background-color 0.2s ease, box-shadow 0.2s ease", userSelect: "none" };
    const styleCheckboxInput = { width: "auto", marginRight: "10px", marginBottom: "0", cursor: "pointer", accentColor: "#1a73e8" };
    const styleButtonBase = { flex: "1 1 0", padding: "10px 0", color: "#fff", border: "none", borderRadius: "8px", fontSize: "14px", fontWeight: "500", cursor: "pointer", marginTop: "16px" };
    const styleStepper = { display: 'none', alignItems: 'center', gap: '5px', marginLeft: 'auto' };
    const styleStepperBtn = { width: '24px', height: '24px', border: '1px solid #dadce0', borderRadius: '50%', backgroundColor: '#f8f9fa', color: '#3c4043', cursor: 'pointer', padding: '0', fontSize: '16px', lineHeight: '22px', textAlign: 'center', userSelect: 'none' };
    ...
    const styleTagSupportContainer = { marginTop: "12px", marginBottom: "12px", padding: "10px", background: "#fff8e1", borderRadius: "6px", border: "1px solid #ffecb3", display: "none" };
    const styleWarningText = { fontSize: "12px", color: "#e37400", marginTop: "4px" };

    // --- NOVO ESTILO DO LINK DE AJUDA (Declarado aqui para não dar erro) ---
    const styleHelpLink = {
        fontSize: "12px", color: "#1a73e8", textDecoration: "none", cursor: "pointer",
    ...
    // --- 3. HELPERS DE DOM / ABERTURA AUTOMÁTICA ---
    ...
    Object.assign(btn.style, { width: "48px", height: "48px", borderRadius: "50%", background: "#1a73e8", color: "white", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", boxShadow: "0 4px 12px rgba(26, 115, 232, 0.4)", transition: "transform 0.2s cubic-bezier(0.25, 0.8, 0.25, 1), box-shadow 0.2s" });
    ...
    expandBtn.onmouseover = () => expandBtn.style.backgroundColor = '#e8eaed'; expandBtn.onmouseout = () => expandBtn.style.backgroundColor = 'transparent';
    ...
    closeBtn.onmouseover = () => closeBtn.style.backgroundColor = '#e8eaed'; closeBtn.onmouseout = () => closeBtn.style.backgroundColor = 'transparent';
    ...
    const langToggleDiv = document.createElement("div"); langToggleDiv.id = "step-lang-type"; Object.assign(langLabel.style, styleLabel); langToggleDiv.appendChild(langLabel);
    ...
    const typeLabel = document.createElement("label"); Object.assign(typeLabel.style, styleLabel); step0Div.appendChild(typeLabel);
    ...
    const mainStatusLabel = document.createElement("label"); Object.assign(mainStatusLabel.style, styleLabel);
    ...
    // --- SUBSTATUS HEADER (Label + Link de Ajuda) ---
    ...
    const subStatusLabel = document.createElement("label"); Object.assign(subStatusLabel.style, styleLabel, { marginTop: "0", marginBottom: "0" });
    ...
    Object.assign(subStatusHelpLink.style, styleHelpLink); // Usa o estilo definido no topo
    subStatusHelpLink.onmouseover = () => subStatusHelpLink.style.textDecoration = "underline";
    subStatusHelpLink.onmouseout = () => subStatusHelpLink.style.textDecoration = "none";
    ...
    step1Div.appendChild(subStatusHeader); // Adiciona o header com o link
    step1Div.appendChild(subStatusSelect); 
    popupContent.appendChild(step1Div);

    const stepPortugalDiv = document.createElement("div"); stepPortugalDiv.id = "step-1-1-portugal"; Object.assign(stepPortugalDiv.style, { ...styleStepBlock, display: 'none' });
    const portugalLabel = document.createElement("label"); Object.assign(portugalLabel.style, styleLabel); stepPortugalDiv.appendChild(portugalLabel);
    const portugalRadioGroup = document.createElement("div"); Object.assign(portugalRadioGroup.style, styleRadioContainer);
    const divPtSim = document.createElement('div'); Object.assign(divPtSim.style, { display: 'flex', alignItems: 'center' });
    const portugalRadioSim = document.createElement("input"); portugalRadioSim.type = 'radio'; portugalRadioSim.name = 'portugal-group'; portugalRadioSim.value = 'sim'; Object.assign(portugalRadioSim.style, styleCheckboxInput);
    ...
    const portugalRadioNao = document.createElement("input"); portugalRadioNao.type = 'radio'; portugalRadioNao.name = 'portugal-group'; portugalRadioNao.value = 'nao'; portugalRadioNao.checked = true; Object.assign(portugalRadioNao.style, styleCheckboxInput);
    ...
    const stepConsentDiv = document.createElement("div"); stepConsentDiv.id = "step-1-2-consent"; Object.assign(stepConsentDiv.style, { ...styleStepBlock, display: 'none' });
    const consentLabel = document.createElement("label"); Object.assign(consentLabel.style, styleLabel); stepConsentDiv.appendChild(consentLabel);
    const consentRadioGroup = document.createElement("div"); Object.assign(consentRadioGroup.style, styleRadioContainer);
    const divCSim = document.createElement('div'); Object.assign(divCSim.style, { display: 'flex', alignItems: 'center' });
    const consentRadioSim = document.createElement("input"); consentRadioSim.type = 'radio'; consentRadioSim.name = 'consent-group'; consentRadioSim.value = 'Sim'; consentRadioSim.checked = true; Object.assign(consentRadioSim.style, styleCheckboxInput);
    ...
    const consentRadioNao = document.createElement("input"); consentRadioNao.type = 'radio'; consentRadioNao.name = 'consent-group'; consentRadioNao.value = 'Não'; Object.assign(consentRadioNao.style, styleCheckboxInput);
    ...
    const stepSnippetsDiv = document.createElement("div"); stepSnippetsDiv.id = "step-1-5-snippets"; Object.assign(stepSnippetsDiv.style, { ...styleStepBlock, display: 'none' });
    const stepSnippetsTitle = document.createElement("h3"); Object.assign(stepSnippetsTitle.style, styleH3); stepSnippetsDiv.appendChild(stepSnippetsTitle);
    const snippetContainer = document.createElement("div"); snippetContainer.id = "snippet-container"; stepSnippetsDiv.appendChild(snippetContainer); popupContent.appendChild(stepSnippetsDiv);

    const step2Div = document.createElement("div"); step2Div.id = "step-2-tasks"; Object.assign(step2Div.style, { ...styleStepBlock, display: 'none' });
    const optionalTaskBtn = document.createElement("button"); optionalTaskBtn.textContent = "+ Gostaria de selecionar uma task?"; Object.assign(optionalTaskBtn.style, styleOptionalBtn);
    ...
    const step2Title = document.createElement("h3"); Object.assign(step2Title.style, styleH3);
    ...
    const step3Div = document.createElement("div"); step3Div.id = "step-3-form"; Object.assign(step3Div.style, { ...styleStepBlock, display: 'none' });
    const step3Title = document.createElement("h3"); Object.assign(step3Title.style, styleH3); step3Div.appendChild(step3Title);
    ...
    // Tag Support
    const tagSupportDiv = document.createElement("div"); tagSupportDiv.id = "tag-support-container"; Object.assign(tagSupportDiv.style, styleTagSupportContainer);
    const tsLabel = document.createElement("label"); tsLabel.textContent = "Utilizou o Tag Support para criar/verificar?"; Object.assign(tsLabel.style, styleLabel, { marginTop: "0" });
    const tsRadioContainer = document.createElement("div"); Object.assign(tsRadioContainer.style, styleRadioContainer);
    const tsSim = document.createElement("input"); tsSim.type = "radio"; tsSim.name = "ts_usage"; tsSim.value = "Sim"; Object.assign(tsSim.style, styleCheckboxInput);
    const tsNao = document.createElement("input"); tsNao.type = "radio"; tsNao.name = "ts_usage"; tsNao.value = "Não"; tsNao.checked = true; Object.assign(tsNao.style, styleCheckboxInput);
    ...
    const tsReasonLabel = document.createElement("label"); tsReasonLabel.textContent = "Qual foi o Motivo?"; Object.assign(tsReasonLabel.style, styleLabel, { fontSize: "12px" });
    const tsReasonInput = document.createElement("input"); tsReasonInput.type = "text"; Object.assign(tsReasonInput.style, styleInput);
    const tsWarning = document.createElement("div"); tsWarning.innerHTML = `⚠️ <strong>Lembre-se de preencher o Form!</strong> <a href="https://docs.google.com/forms/d/e/1FAIpQLSeP_JM8D-6qHa5ZC93aTzj38WiO5zx8nyrWNPvbZhjJj6CpkA/viewform" target="_blank" style="color:#e37400">Link aqui</a>`; Object.assign(tsWarning.style, styleWarningText);
    ...
    const screenshotsTitle = document.createElement("h4"); screenshotsTitle.textContent = "Evidências / Screenshots"; Object.assign(screenshotsTitle.style, styleH3);
    ...
    const emailCheckbox = document.createElement("input"); emailCheckbox.type = "checkbox"; emailCheckbox.checked = true; Object.assign(emailCheckbox.style, styleCheckboxInput);
    ...
    Object.assign(copyButton.style, { ...styleButtonBase, backgroundColor: "#5f6368" }); buttonContainer.appendChild(copyButton);
    Object.assign(generateButton.style, { ...styleButtonBase, backgroundColor: "#1a73e8" }); buttonContainer.appendChild(generateButton);
    ...
    // Atualiza o link de ajuda baseado no tipo (BAU/LM)
    ...
    function populateTaskCheckboxes() {
        ...
        for (const taskKey in TASKS_DB) {
            ...
            const checkbox = document.createElement('input'); checkbox.type = 'checkbox'; checkbox.value = taskKey; checkbox.className = 'task-checkbox'; Object.assign(checkbox.style, styleCheckboxInput);
            const taskName = document.createElement('span'); taskName.textContent = task.name; Object.assign(taskName.style, { flexGrow: '1' }); 
            const stepperDiv = document.createElement('div'); stepperDiv.className = 'stepper-container'; Object.assign(stepperDiv.style, styleStepper);
            const btnMinus = document.createElement('button'); btnMinus.type = 'button'; btnMinus.textContent = '−'; btnMinus.classList.add('no-drag'); Object.assign(btnMinus.style, styleStepperBtn);
            const countSpan = document.createElement('span'); countSpan.className = 'stepper-count'; countSpan.textContent = '1'; Object.assign(countSpan.style, styleStepperCount);
            const btnPlus = document.createElement('button'); btnPlus.type = 'button'; btnPlus.textContent = '+'; btnPlus.classList.add('no-drag'); Object.assign(btnPlus.style, styleStepperBtn);
    ...
    subStatusSelect.onchange = () => {
        ...
        const addSnippetInput = (scenario, type, container) => {
            const label = document.createElement('label'); Object.assign(label.style, styleCheckboxLabel);
            label.onmouseover = () => label.style.backgroundColor = '#e8eaed'; label.onmouseout = () => label.style.backgroundColor = '#f8f9fa';
            const input = document.createElement('input'); input.type = type; input.id = scenario.id; Object.assign(input.style, styleCheckboxInput);
        ...
        if (selectedSubStatusKey === 'NI_Awaiting_Inputs') {
            ...
            inputType = 'checkbox'; const reasonTitle = document.createElement('label'); reasonTitle.textContent = t('cenarios_comuns'); Object.assign(reasonTitle.style, styleLabel); snippetContainer.appendChild(reasonTitle);
        ...
        uniquePlaceholders.forEach(placeholder => {
            ...
            if (textareaListFields.includes(fieldName)) {
                field = document.createElement('textarea'); Object.assign(field.style, styleTextarea); field.classList.add('bullet-textarea'); enableAutoBullet(field);
            } else if (textareaParagraphFields.includes(fieldName)) {
                field = document.createElement('textarea'); Object.assign(field.style, styleTextarea);
            } else {
                field = document.createElement('input'); field.type = 'text'; Object.assign(field.style, styleInput);
