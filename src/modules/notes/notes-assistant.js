...
export function initCaseNotesAssistant() {
    ...
    function getVisibleEditor() {
        // Procura o Card que está no topo da pilha (visível)
        ...
        // Dentro do card ativo, busca o editor
    ...
    // --- Funções de Tradução e UI ---
    // (Mesmas funções de t(), updateUIText(), setLanguage() do arquivo anterior...)
    function t(key) {
    ...
    // --- MONTAGEM DA UI (Reduzida para brevidade, mas completa na execução) ---
    // (Aqui vai toda a montagem de divs, selects, buttons que já validamos)
    ...
    langPT.textContent = "Português"; langPT.classList.add('no-drag');
    langES.textContent = "Español"; langES.classList.add('no-drag');
    ...
    langToggleDiv.appendChild(langContainer); popupContent.appendChild(langToggleDiv);
    ...
    typeContainer.appendChild(typeBAU); typeContainer.appendChild(typeLM);
    step0Div.appendChild(typeContainer); popupContent.appendChild(step0Div); setCaseType('bau');

    step1Div.id = "step-1-selection"; Object.assign(mainStatusLabel.style, styleLabel);
    mainStatusSelect.id = "main-status"; mainStatusSelect.innerHTML = `<option value="">-- Selecione --</option><option value="NI">NI - Need Info</option><option value="SO">SO - Solution Offered</option><option value="IN">IN - Inactive</option><option value="AS">AS - Assigned</option>`; Object.assign(mainStatusSelect.style, styleSelect);
    ...
    // Steps 1.5 a 3
    stepSnippetsDiv.id = "step-1-5-snippets"; Object.assign(stepSnippetsDiv.style, { ...styleStepBlock, display: 'none' }); Object.assign(stepSnippetsTitle.style, styleH3); snippetContainer.id = "snippet-container"; stepSnippetsDiv.appendChild(stepSnippetsTitle); stepSnippetsDiv.appendChild(snippetContainer); popupContent.appendChild(stepSnippetsDiv);
    step2Div.id = "step-2-tasks"; Object.assign(step2Div.style, { ...styleStepBlock, display: 'none' }); Object.assign(step2Title.style, styleH3); taskCheckboxesContainer.id = "task-checkboxes-container"; step2Div.appendChild(step2Title); step2Div.appendChild(taskCheckboxesContainer); popupContent.appendChild(step2Div);
    step3Div.id = "step-3-form"; Object.assign(step3Div.style, { ...styleStepBlock, display: 'none' }); Object.assign(step3Title.style, styleH3); dynamicFormFieldsContainer.id = "dynamic-form-fields-container"; step3Div.appendChild(step3Title); step3Div.appendChild(dynamicFormFieldsContainer); popupContent.appendChild(step3Div);
    ...
    emailCheckbox.type = "checkbox"; emailCheckbox.checked = true; Object.assign(emailCheckbox.style, styleCheckboxInput);
    ...
    // --- LÓGICA (updateFields, enableAutoBullet, generateOutputHtml) ---
    // ... (Lógicas de preenchimento e stepper mantidas conforme versão anterior) ...
    // ... (Apenas vou colar as funções principais para garantir que não se percam)

    function updateFieldsFromScenarios() {
         // ... (Lógica de cenário mantida)
         ...
         activeScenarioInputs.forEach(input => {
             ...
             if (snippets) {
                 for (const fieldId in snippets) {
                     ...
                     } else if (fieldId === 'linkedTask') activeLinkedTasks.add(snippets.linkedTask);
                 }
             }
         });
    ...
    // --- (HTML Generator e Eventos de Clique) ---
            ...
            selectedCheckboxes.forEach(checkbox => {
                ...
                if (screenshotList.length > 0) {
                    for (let i = 1; i <= count; i++) {
                        if (count > 1) screenshotsText += `<b>${task.name} - Implementação #${i}</b>`;
                        else screenshotsText += `<b>${task.name}</b>`;
    ...
    // === O BOTÃO DE AÇÃO PRINCIPAL (CORRIGIDO COM RANGE) ===
