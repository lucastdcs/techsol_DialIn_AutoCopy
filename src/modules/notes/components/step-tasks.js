...
export function createStepTasksComponent(onUpdateCallback) {
    ...
    function getBrandStyle(name) {
        ...
        if (n.includes('ads') || n.includes('conversion') || n.includes('remarketing')) return { ...DS.brands.ads, icon: ICONS.ads };
        if (n.includes('ga4') || n.includes('analytics')) return { ...DS.brands.ga4, icon: ICONS.ga4 };
        if (n.includes('gtm') || n.includes('tag manager')) return { ...DS.brands.gtm, icon: ICONS.gtm };
        if (n.includes('merchant') || n.includes('shopping')) return { ...DS.brands.gmc, icon: ICONS.gmc };
    ...
    if (!document.getElementById(styleId)) {
        ...
        style.innerHTML = `
            ...
            .cw-hero-card {
                ...
                display: flex; flex-direction: column; gap: 12px; height: 90px;
    ...
    const renderList = (filter = "") => {
        ...
        otherTasks.forEach(([key, task]) => {
            if (filter && !task.name.toLowerCase().includes(filter.toLowerCase())) return;
            ...
            row.onclick = () => toggleTask(key, selection[key] ? -selection[key].count : 1); // Toggle simples na lista
    ...
    searchInput.addEventListener('input', (e) => {
        ...
        container.querySelector('.cw-section-title').style.display = e.target.value ? 'none' : 'block';
    ...
    function toggleTask(key, delta) {
        if (!selection[key]) {
            if (delta > 0) selection[key] = { count: delta, data: TASKS_DB[key], brand: getBrandStyle(TASKS_DB[key].name) };
        } else {
    ...
    function updateUI() {
        ...
        if (keys.length > 0) {
            ...
            footerCount.textContent = `${keys.length} Task${keys.length > 1 ? 's' : ''}`;
        ...
        keys.forEach(key => {
            ...
            if (prints.length > 0) {
                ...
                for(let i=1; i<=count; i++) {
                    ...
                    Object.assign(input.style, { width: "100%", border: "none", borderBottom: "1px dashed #ccc", fontSize: "12px", fontWeight: "600", color: "#1a73e8", background: "transparent", outline: "none" });
                    ...
                    prints.forEach((req, idx) => {
                        ...
                        Object.assign(pInput.style, { width: "100%", border: "none", borderBottom: "1px dashed #ccc", fontSize: "12px", marginBottom: "4px", background: "transparent", outline: "none" });
