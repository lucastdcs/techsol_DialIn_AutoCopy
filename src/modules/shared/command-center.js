...
// Ícones SVG (Funcionais e Limpos)
const ICONS = {
    ...
    // Grip agora é desenhado via CSS (mais limpo)
};
...
export function initCommandCenter(actions) {
    ...
    if (!document.getElementById('cw-command-center-style')) {
        ...
        style.innerHTML = `
            ...
            /* SYSTEM READY (Pulso Verde Suave) */
            .cw-pill.system-ready { animation: systemReadyPulse 1s ease-out forwards; }
            ...
            .cw-pill.dragging .cw-grip-bar { background-color: ${COLORS.gripActive}; width: 24px; }
            ...
            .cw-grip-bar { 
                ...
                border-radius: 10px; /* Arredondado perfeito */
                transition: all 0.3s cubic-bezier(0.4, 0.0, 0.2, 1);
            }
            
            /* --- ÍCONES (Hover Encaixado) --- */
            .cw-btn {
            ...
            /* Cores Específicas (Texto e Glow) */
            .cw-btn.notes:hover { color: ${COLORS.blue}; text-shadow: 0 0 12px ${COLORS.blue}; }
            .cw-btn.email:hover { color: ${COLORS.red}; text-shadow: 0 0 12px ${COLORS.red}; }
            .cw-btn.script:hover { color: ${COLORS.purple}; text-shadow: 0 0 12px ${COLORS.purple}; }
            .cw-btn.links:hover { color: ${COLORS.green}; text-shadow: 0 0 12px ${COLORS.green}; }

            /* Divisor */
            .cw-sep { width: 24px; height: 1px; background: rgba(255,255,255,0.15); margin: 2px 0; opacity: 0; transition: opacity 0.5s; }
    ...
    // Grip Limpo (Apenas a div da barra, sem SVG)
    pill.innerHTML = `
