...
const COLORS = {
    ...
    iconIdle: '#E8EAED', 
    iconActive: '#FFFFFF',
    gripColor: '#5F6368', // Cor do ícone de drag
...
export function initCommandCenter(actions) {
    ...
    if (!document.getElementById('cw-command-center-style')) {
        ...
        style.innerHTML = `
            ...
            .cw-pill {
                ...
                /* Transições Suaves para Hover/Entrada */
                transition: 
                    background 0.3s ease,
                    box-shadow 0.3s ease,
                    border-color 0.3s ease,
                    opacity 0.4s ease-out,
                    transform 0.5s cubic-bezier(0.19, 1, 0.22, 1), /* Movimento de entrada */
            ...
            /* DRAGGING - A CORREÇÃO ESTÁ AQUI */
            .cw-pill.dragging {
                cursor: grabbing; 
                background: ${COLORS.glassActive}; 
                transform: scale(1.05) !important; /* Scale fixo, sem transition de transform */
                box-shadow: 0 20px 50px rgba(0,0,0,0.6);
                border-color: rgba(255,255,255,0.3);
                
                /* MATAMOS TODAS AS TRANSIÇÕES DURANTE O ARRASTO */
                /* Isso impede que o CSS tente animar a posição enquanto o JS move */
    ...
    // 2. DOM
    const ICONS = {
        grip: `<svg viewBox="0 0 24 24"><path d="M11 18c0 1.1-.9 2-2 2s-2-.9-2-2 .9-2 2-2 2 .9 2 2zm-2-8c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0-6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm6 4c1.1 0 2-.9 2-2s-.9-2-2-2-2 .9-2 2 .9 2 2 2zm0 2c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2zm0 6c-1.1 0-2 .9-2 2s.9 2 2 2 2-.9 2-2-.9-2-2-2z"/></svg>`,
    ...
    // 5. PHYSICS ENGINE (Drag)
    let isDragging = false;
    let startX, startY, initialLeft, initialTop;
    const DRAG_THRESHOLD = 3; 
    const grip = pill.querySelector('.cw-grip-area');

    // Inicia drag em qualquer lugar (exceto botões)
    pill.onmousedown = (e) => {
        ...
        pill.classList.remove('snapping', 'docked', 'system-ready'); // Remove classes com transition longa
        
        // Reseta transform para movimento 1:1 com o mouse
