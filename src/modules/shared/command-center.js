...
const COLORS = {
    ...
    glassActive: 'rgba(255, 255, 255, 0.95)',
    glassHighlight: 'rgba(0, 0, 0, 0.05)',
    iconIdle: '#5F6368', 
    iconActive: '#202124',
    gripColor: '#DADCE0',
    gripActive: '#1A73E8',
    blue: '#1A73E8', red: '#D93025', purple: '#A142F4', green: '#1E8E3E',
    readyGlow: 'rgba(52, 168, 83, 0.6)'
};
...
export function initCommandCenter(actions) {
    ...
    if (!document.getElementById('cw-command-center-style')) {
        ...
        style.innerHTML = `
            ...
            .cw-pill {
                ...
                box-shadow: 0 2px 6px rgba(60, 64, 67, 0.15), 0 8px 24px rgba(60, 64, 67, 0.10);
                ...
                transition: 
                    ...
                    transform 0.5s cubic-bezier(0.19, 1, 0.22, 1), /* Entrada Firme */
            ...
            @keyframes systemReadyPulse {
                ...
                100% { border-color: ${COLORS.glassBorder}; box-shadow: 0 0 0 10px rgba(52, 168, 83, 0); }
            }

            /* DRAGGING */
            .cw-pill.dragging {
                cursor: grabbing; background: ${COLORS.glassActive}; transform: scale(1.05);
                box-shadow: 0 15px 30px rgba(60,64,67, 0.25);
                transition: transform 0.1s ease, box-shadow 0.1s ease;
            }
            .cw-pill.dragging .cw-grip-bar { background-color: ${COLORS.gripActive}; width: 16px; }
            ...
            .cw-grip-area:hover .cw-grip-bar { background-color: #9AA0A6; }
            ...
            .cw-btn.notes:hover { color: ${COLORS.blue}; background: #E8F0FE; }
            .cw-btn.email:hover { color: ${COLORS.red}; background: #FCE8E6; }
            .cw-btn.script:hover { color: ${COLORS.purple}; background: #F3E8FD; }
            .cw-btn.links:hover { color: ${COLORS.green}; background: #E6F4EA; }

            /* SEP */
            .cw-sep { width: 24px; height: 1px; background: #DADCE0; margin: 2px 0; opacity: 0; transition: opacity 0.5s; }
            .cw-sep.visible { opacity: 1; }

            /* TOOLTIPS */
            .cw-btn::after { content: attr(data-label); position: absolute; top: 50%; transform: translateY(-50%) scale(0.8); padding: 6px 10px; border-radius: 4px; background: #3C4043; color: #fff; font-family: 'Google Sans', sans-serif; font-size: 12px; font-weight: 500; opacity: 0; pointer-events: none; transition: all 0.2s cubic-bezier(0.4, 0.0, 0.2, 1); box-shadow: 0 2px 6px rgba(0,0,0,0.2); white-space: nowrap; }
    ...
    // Drag começa no Grip ou na área vazia
    pill.onmousedown = (e) => {
    ...
    // 4. ANIMAÇÃO DE ENTRADA (Sequência "System Ready")
    async function animateEntry() {
        // Espera a splash sair (~2.5s)
        await esperar(3100);
        ...
        for (let i = 0; i < items.length; i++) {
            ...
            await esperar(60); // Ritmo rápido
        }

        // 3. System Ready (Pulso Verde na Borda)
        await esperar(200);
