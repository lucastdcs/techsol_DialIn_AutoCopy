...
const COLORS = {
    ...
    glassActive: 'rgba(40, 40, 40, 0.98)', // Mais opaco no drag
...
export function initCommandCenter(actions) {
    ...
    // 1. ESTILOS (Com correções de Opacidade e Transição)
    if (!document.getElementById('cw-command-center-style')) {
        ...
        style.innerHTML = `
            ...
            /* ESTADO VISÍVEL (Quando parado) */
            .cw-pill.docked { 
                opacity: 1; 
                transform: translateX(0) scale(1); 
            }

            /* ESTADO ARRASTANDO (Correções Críticas) */
            .cw-pill.dragging {
                cursor: grabbing; 
                background: ${COLORS.glassActive}; 
                
                /* FIX 1: Força visibilidade máxima */
                opacity: 1 !important; 
                
                /* FIX 2: Scale fixo para estabilidade */
                ...
                /* FIX 3: Mata todas as transições de movimento */
            ...
            /* SNAP (Movimento Elástico ao Soltar) */
            .cw-pill.snapping { 
                transition: 
            ...
            .cw-btn::after { 
                ...
                padding: 6px 12px; border-radius: 6px; background: #E8EAED; color: #202124; 
    ...
    let startX, startY, shiftX, shiftY; // Variáveis de tracking
    const DRAG_THRESHOLD = 3; 
    
    const grip = pill.querySelector('.cw-grip-area');

    // Drag inicia ao clicar (se não for botão)
    pill.onmousedown = (e) => {
        ...
        // Calcula Shift (distância do mouse até o canto top-left da pílula)
        // Isso impede que a pílula "pule" para o centro do mouse
        shiftX = e.clientX - rect.left;
        shiftY = e.clientY - rect.top;

        // --- PREPARAÇÃO DO DRAG ---
        // 1. Trava transições (o Fix principal)
        pill.style.transition = 'none';
        
        // 2. Fixa posição absoluta nas coordenadas atuais
        pill.style.left = rect.left + 'px';
        pill.style.top = rect.top + 'px';
        
        // 3. Limpa âncoras relativas e transforms
        pill.style.right = 'auto';
        pill.style.bottom = 'auto';
        pill.style.transform = 'scale(1.05)'; // Aplica o scale diretamente aqui se quiser, ou via classe CSS
    ...
    function onMouseMove(e) {
        ...
        if (isDragging) {
            // Move usando o shift calculado
            // Isso mantém o mouse exatamente no mesmo ponto da pílula onde você clicou
            let newLeft = e.clientX - shiftX;
            let newTop = e.clientY - shiftY;
            
            // Limites de Tela (Não sair)
    ...
    function onMouseUp(e) {
        if (isDragging) {
            ...
            // Restaura animações para o snap
