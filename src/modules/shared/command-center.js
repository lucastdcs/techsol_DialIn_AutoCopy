...
.cw-pill {
    ...
    gap: 8px; /* Espaço entre itens quando aberto */
    
    /* Tamanho e Forma (Estado Aberto Padrão) */
    width: 56px; /* Largura fixa para evitar balanço lateral */
    padding: 8px 0 16px 0; /* Padding assimétrico para acomodar Grip e conteúdo */
    max-height: 500px; /* Altura suficiente para todo conteúdo */
    ...
    overflow: hidden; /* Corta conteúdo no colapso */
    z-index: 2147483647;

    /* TRANSIÇÕES CRÍTICAS UNIFICADAS */
    transition: 
        max-height 0.6s cubic-bezier(0.34, 1.56, 0.64, 1), /* Colapso Vertical Elástico */
        width 0.5s cubic-bezier(0.2, 0.8, 0.2, 1),      /* Largura (importante p/ Ilha Dinâmica) */
...
/* Isso faz os ícones sumirem rápido antes da pílula fechar */
...
/* Gira o ícone principal ao fechar (Opcional, mas fica legal) */
...
brandBtn.onclick = (e) => {
    // Se estiver arrastando (isDragging do seu código original), não faz nada
    ...
    if (isCollapsed) {
        ...
        brandBtn.title = "Expandir";
    } else {
        pill.classList.remove("collapsed");
        brandBtn.title = "Minimizar";
    }
};
