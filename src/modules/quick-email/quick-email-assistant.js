...
export function initQuickEmailAssistant() {
    ...
    // --- 1. DESIGN SYSTEM & ESTILOS HD (Revisão de Espaçamentos e Definição) ---

    // Paleta de Cores e Sombras Padronizadas
    const COLORS = {
        bgApp: "#F8F9FA", // Fundo geral mais leve
        bgSurface: "#FFFFFF",
    ...
    const styleNavView = {
        ...
        transition: "transform 0.5s cubic-bezier(0.19, 1, 0.22, 1)", // Spring Physics
    ...
    // Barra de Ferramentas (Fixa no topo da lista)
    const styleToolbar = {
        padding: "20px 24px 12px 24px", // Espaçamento lateral maior para respiro
        flexShrink: "0", 
        background: COLORS.bgApp, // Mantém a cor do fundo para parecer nativo
        zIndex: "10",
        display: "flex", flexDirection: "column", gap: "16px",
        borderBottom: `1px solid ${COLORS.borderSubtle}` // Separação sutil
    };

    // Input de Busca HD
    const styleSearchInput = {
        width: "100%", height: "44px", // Altura de toque confortável
        padding: "0 16px 0 48px", // Espaço para ícone
        borderRadius: "12px", 
        ...
        boxShadow: "0 2px 5px rgba(0,0,0,0.03)", // Sombra sutil em repouso
    ...
    const styleTabBtn = {
        padding: "8px 16px", 
        borderRadius: "100px", // Pill shape perfeita
    ...
    const styleTabActive = {
        ...
        borderColor: "transparent", // Remove borda para look mais limpo
        fontWeight: "600",
    ...
    const styleListContent = {
        padding: "16px 24px 80px 24px", // Padding inferior grande para scroll
        overflowY: "auto", 
        flexGrow: "1",
        display: "flex", flexDirection: "column", gap: "12px" // Gap vertical consistente
    };

    // Cartão da Lista (Row)
    const styleRow = {
        display: "flex", alignItems: "center", justifyContent: "space-between",
        padding: "16px", // Mais respiro interno
        borderRadius: "16px", // Bordas mais arredondadas (Modern UI)
        background: COLORS.bgSurface, 
        border: `1px solid transparent`, // Borda transparente p/ hover não pular layout
    ...
    Object.assign(popup.style, stylePopup, {
        right: "100px", width: "440px", height: "640px", // Largura mais compacta, altura maior
        borderRadius: "20px", // Popup mais arredondado
        boxShadow: "0 24px 64px rgba(0,0,0,0.2)", // Sombra profunda de "Float"
    ...
    function createChip(text, key, icon = null) {
        ...
        // Adiciona um wrapper para o ícone para alinhamento perfeito
    ...
    function renderTabs() {
        ...
        // Aba CRs com ícone de raio para destacar velocidade
        ...
        itemsToRender.forEach(item => {
            ...
            if (item.type === 'email') {
                ...
                row.innerHTML = `
                    ...
                    <div style="flex-grow:1; min-width:0; display:flex; flex-direction:column; gap:2px;">
                        <div style="font-size:14px; font-weight:600; color:#202124; margin-bottom:0;">${cleanName}</div>
                ...
                `;
                row.onclick = () => {
                    // Feedback de clique instantâneo
    ...
    searchInput.addEventListener("input", (e) => {
        ...
        if (searchTerm !== "") {
            Array.from(tabsContainer.children).forEach(c => {
                // Visual de 'desativado' durante busca
